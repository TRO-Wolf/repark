...
U8 WRITE-SQL PR1 round 4 (2026-09-25, critic r3): the oracle adds the critic's `r3/…` .. `r3e/…` and `r3fix/…` (572 cases, sha256 e7fab83c…); a case whose RePark answer is a named residue carries `residue = {id, repark: {step, rows}}`. The facade replay covers 26 prefixes (439 cases, `r2fix/A-` and the `r3…` keys added) and skips none: every refusal compares its exact class, condition, SQLSTATE and message (R-1 by base class and Iceberg's `Cannot delete file …` text), and a residue case is held to RePark's recorded answer (an R-2 case also to Spark's rows). DECIMAL and DATE cells compare as their text. pins: u8-write-sql/C-015, C-019, C-021, C-022, C-023, C-024
