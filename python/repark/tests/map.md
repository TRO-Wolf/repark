...
U8 WRITE-SQL PR2 (2026-09-25): `u8_write_sql_nested_spark_oracle.json` is the PR's Spark oracle (93 keys `pr2/…`, sha256 d02c6ed7…, generator `target/probe-u8-pr2/build_oracle.py`, sources `target/probe-u8-pr2/spark*.json`). It covers nested struct-field assignment in UPDATE and MERGE: the two scoreboard cells, casts, refusals, conflicts, map and array targets, MoR tables and the whole-struct near misses. Its `provenance` block names the normalization and the residue keys (R-13 .. R-17, 17 keys). Each residue key carries RePark's recorded answer. `test_ice_write_sql_1.py` pins the cells `W-UPDATE-NESTED-FIELD` and `W-MERGE-NESTED` (rows, schema, snapshot summary). `test_the_nested_assignment_measurements_replay_as_spark_answered` replays every key through `spark.sql` and compares the exception class name, condition, SQLSTATE, normalized message and rows. pins: u8-write-sql/C-025, C-026, C-027, C-028, C-029, C-030, C-031
