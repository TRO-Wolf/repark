...
ICE-VIEWS-1 (2026-09-20): `test_ice_views_1.py` is the views PR1 battery — the catalog door and the read path on the memory catalog: create+select `[[1,"d1"],[2,"d2"]]`, OR REPLACE second wins, SHOW VIEWS with LIKE plus DROP, IF NOT EXISTS noop, version-log JSON pin, VERSION AS OF branch and snapshot pins, view-over-view, stored-namespace resolution, the 100-deep typed guard, listTables exclusion, the full D-9 contract, ALTER AS refusal, view-only DROP NAMESPACE, V-DROP-IF-EXISTS. `test_f1_sql_expander.py` pins durable CREATE VIEW bodies verbatim (TEMPORARY still expands). R2 (2026-09-21): the D-9 contract pins full sentences for INSERT/DROP TABLE/DELETE/UPDATE/BY NAME plus a post-DROP readback, and one-part DROP VIEW round-trips the current namespace. pins: ice-views-1/C-004, C-006, C-007, C-008, C-009, C-011, C-012, C-013, C-014, C-015, C-016
