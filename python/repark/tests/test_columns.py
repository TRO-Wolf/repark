"""Facade tests for the Column / expression surface.
...
Parity inputs use ``createDataFrame``, never inline SQL ``VALUES``: a pin is genuine only when BOTH
engines infer the pinned type/nullability, and Spark types ``VALUES`` differently (int32/non-null,
DECIMAL literals), which would record repark's own shape as "Spark".
"""
...
def test_lit_rejects_unsupported_type(spark: ReparkSession) -> None:
    # lit(list) is supported (F.array expansion); pin a still-unsupported type.
...
def test_division_is_float(spark: ReparkSession) -> None:
    # SQL literals `7.0`/`2.0` are DECIMAL(2,1); Column `/` still yields float64 (the analyzer's
    # SQL `/` rewrite is not this path).
...
def test_division_of_integers_is_double(spark: ReparkSession) -> None:
    # PySpark `/` is ALWAYS true division, never integer-truncating (that is `//` / `div`);
    # integer operands must promote to double.
...
def test_python_bool_on_column_raises() -> None:
    # Without the PySpark __bool__ guard, `and`/`or`/`not`/`if` on a Column resolve via object
    # truthiness and silently DROP predicates — wrong rows, no error.
    with pytest.raises(ValueError, match="Cannot convert column into bool"):
        bool(F.col("a") > 1)
    with pytest.raises(ValueError, match="Cannot convert column into bool"):
        _ = (F.col("a") > 1) and (F.col("b") > 2)
    with pytest.raises(ValueError, match="Cannot convert column into bool"):
...
def test_cast_accepts_long_and_bigint_type_spec(spark: ReparkSession) -> None:
    # No `types` object emits "long"/"bigint", but they are PySpark cast idioms and must map to
    # Int64. A float source truncates toward zero (Arrow cast), like Spark.
...
def test_concat_propagates_null(spark: ReparkSession) -> None:
    ...
    # nulls, so the engine must guard against that divergence.
...
def test_sql_concat_coalesce_null_returns_utf8_not_view(spark: ReparkSession) -> None:
    """SQL ``concat(coalesce(NULL, ''), …)`` must yield Utf8, never Utf8View.

    The DF kernel returns Utf8View after coalesce folds a null to Utf8View(""); SparkConcat
    coerces args to Utf8 and always emits Arrow string.
    """
    table = spark.sql(
...
def test_sql_concat_array_any_null_propagates_per_row(spark: ReparkSession) -> None:
    ...
    Column API embeds DF concat + CASE (separate from the UDF overwrite); this pin mutation-proofs
    the per-row null mask on the shipping SQL surface.
    """
    table = spark.sql(
        """
...
def test_parity_coalesce_cast_chain(spark: ReparkSession) -> None:
    ...
    # createDataFrame so both engines genuinely infer the golden types (see module docstring).
    ...
    # `clean_name` is non-nullable: coalesce with a non-null literal fallback can never be null,
    # and the harness compares field nullability as part of the schema signature.
    golden = pa.table(
        [
...
def test_parity_filter_orderby_ordered(spark: ReparkSession) -> None:
    ...
    # genuinely infer the golden types (see module docstring).
...
def test_parity_integer_division_is_double(spark: ReparkSession) -> None:
    # Spark `/` on integers yields DoubleType (true division); integer inputs are the class that
    # would truncate under DataFusion's native integer `/`.
...
def test_parity_end_to_end_chain(spark: ReparkSession) -> None:
    # Full chain: withColumn, filter, join, orderBy — a genuine cross-engine pin. createDataFrame
    # so both engines genuinely infer the golden types (see module docstring). The frames are
    # re-selected via temp views so the by-name join gets distinct qualifiers: two inline
    # createDataFrame frames share a qualifier and cannot be joined directly yet.
