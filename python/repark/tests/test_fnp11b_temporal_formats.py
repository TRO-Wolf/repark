...
def _session(ansi: bool, tz: str) -> ReparkSession:
    ...
    if _LIVE.get("key") != key or cached is None or cached._inner is None:
        ...
        session.sql(
            f"SELECT *, CASE WHEN y IS NULL THEN CAST(NULL AS TIME)"
            f" ELSE TIME'12:34:56.789' END AS tm FROM {FRAME_VIEW}"
...
def _frame_for_python(cell: dict[str, Any]) -> str:
    """Frame view for a Python-door cell: the TIME frame only for valued tm reads."""
...
def _assert_interval_rows_as_string(session: ReparkSession, cell: dict[str, Any]) -> None:
    ...
    if cell["door"] == "sql":
        table = session.sql(f"SELECT CAST(({cell['expr']}) AS STRING)").toArrow()
    else:
...
def _run_block2_cell(cell: dict[str, Any]) -> None:
    ...
    if "error_condition" in cell:
        try:
            if cell["door"] == "sql":
                table = session.sql(cell["expr"].replace("FRAME", f"({O245_VIEW})")).toArrow()
            else:
                column = eval(cell["expr"], dict(_eval_namespace()))
