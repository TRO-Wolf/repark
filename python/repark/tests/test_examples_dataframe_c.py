"""Divergence pins for the EX-18 DataFrame-c example batch (registry §7 EX-DF-11..17)."""
...
def test_sampleby_seeded_fraction_divergence(spark: ReparkSession) -> None:
    """sampleBy 0.5/0.5 at seed 0 keeps three rows where Spark keeps two (EX-DF-14)."""
    frame = spark.createDataFrame(SIX_ROWS, ["g", "k", "v"])
...
def test_show_rendering_divergence(spark: ReparkSession) -> None:
    """show(3) renders the repark grid without Spark's 'only showing top' trailer (EX-DF-16)."""
...
def test_tojson_refuses(spark: ReparkSession) -> None:
    """toJSON refuses loudly; Spark answers one JSON object string per row (EX-DF-17)."""
