...
def test_max_num_rows_edge_shapes_preserved(spark: ReparkSession) -> None:
    ...
    try:
        ...
        spark.conf.set("spark.sql.repl.eagerEval.maxNumRows", "0")
        assert repr(_sized_frame(spark, 3)).endswith(_footer_text(0))
        assert "only showing top" not in repr(_sized_frame(spark, 0))
        spark.conf.set("spark.sql.repl.eagerEval.maxNumRows", "-3")
        assert repr(_sized_frame(spark, 3)).endswith(_footer_text(0))
        spark.conf.set("spark.sql.repl.eagerEval.maxNumRows", "abc")
        assert repr(_sized_frame(spark, 21)).endswith(_footer_text(20))
        assert "only showing top" not in repr(_sized_frame(spark, 20))
        spark.conf.set("spark.sql.repl.eagerEval.maxNumRows", "1")
        spark.conf.set("spark.sql.repl.eagerEval.truncate", "3")
        rendered = repr(_sized_frame(spark, 2))
        assert rendered.endswith(_footer_text(1))
        html = _sized_frame(spark, 2)._repr_html_()
