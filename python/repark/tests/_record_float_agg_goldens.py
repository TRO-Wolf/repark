...
and runs each row's own recipe (the same `run_row` the assertions use) on live PySpark, so the
recorded golden and the asserted recipe cannot drift apart. Run it (needs a JVM and `pyspark`,
i.e. `uv sync --extra record`). Hold the conductor JVM lock (`/tmp/grok-jvm-record.lock`) first::
