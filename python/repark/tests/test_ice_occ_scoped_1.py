...
def test_insert_storm_loses_only_to_the_retry_budget(
    ...
    Over repetitions Spark commits 7-16 of 16 (Hadoop 9-16, all sixteen in 4 of 32
    repetitions; InMemory 7-9 — fixture `spark_occ_oracle4.json`); RePark's losers exhaust
    the fork's commit-retry budget — registry row ICE-OCC-SCOPED-1-INSERT-STORM (BACKLOG).
    """
    session, catalog = _session(tmp_path)
