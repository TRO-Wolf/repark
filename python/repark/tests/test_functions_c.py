...
def test_fn_c_deferred_names_are_absent() -> None:
    ...
    The list RATCHETS DOWN. ``sum_distinct`` stays here for a reason worth recording: it is
    **not a kernel** — DataFusion spells it ``sum(DISTINCT x)``, a modifier on the aggregate
    call, so it needs the facade's DISTINCT path rather than a dispatch arm. The camelCase
    spellings are PySpark's deprecated aliases and follow their snake_case originals.
    """
    deferred = (
        "sum_distinct",
        "sumDistinct",
