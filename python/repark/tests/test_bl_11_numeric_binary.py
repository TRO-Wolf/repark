...
def _session() -> ReparkSession:
    ...
    return (
        ReparkSession.builder.appName("bl-11-numeric-binary").config(ANSI_KEY, "true").getOrCreate()
    )
...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            "B11-tinyint", "CAST(CAST(1 AS TINYINT) AS BINARY)", b"\x01", id="B11-tinyint"
        ),
        pytest.param(
            "B11-smallint", "CAST(CAST(1 AS SMALLINT) AS BINARY)", b"\x00\x01", id="B11-smallint"
        ),
        pytest.param("B11-int", "CAST(1 AS BINARY)", b"\x00\x00\x00\x01", id="B11-int"),
        pytest.param("B11-int-neg", "CAST(-1 AS BINARY)", b"\xff\xff\xff\xff", id="B11-int-neg"),
        pytest.param(
            "B11-int-big",
...
def test_c001_sql_door_integrals_encode_big_endian(cell: str, expr: str, expected: bytes) -> None:
...
def test_c001_python_door_small_types_encode() -> None:
    ...
    small = _table(spark.range(1).select(F.lit(1).cast("smallint").cast("binary").alias("b")))
...
@pytest.mark.parametrize(
    ("cell", "expr", "source"),
    [
        pytest.param("B11-float", "CAST(CAST(1.5 AS FLOAT) AS BINARY)", "FLOAT", id="B11-float"),
        pytest.param(
        ...
        pytest.param("B11-date", "CAST(DATE'2024-01-01' AS BINARY)", "DATE", id="B11-date"),
        pytest.param(
            "B11-ts",
...
def test_c002_sql_door_never_castable_refuses_ansi_off(cell: str, expr: str, source: str) -> None:
...
def test_c002_python_door_double_refuses_ansi_off() -> None:
    ...
    spark = _session()
    _ansi_off(spark)
    frame = spark.range(1).select(F.lit(1.5).cast("binary").alias("b"))
    with pytest.raises(_REFUSAL) as excinfo:
        _table(frame)
    message = str(excinfo.value)
    assert "DATATYPE_MISMATCH.CAST_WITHOUT_SUGGESTION" in message
...
@pytest.mark.parametrize(
    ...
    [
        ...
        pytest.param("B11-int-big", "CAST(305419896 AS BINARY)", "INT", id="B11-int-big"),
        pytest.param(
            "B11-bigint",
            "CAST(CAST(1 AS BIGINT) AS BINARY)",
            "BIGINT",
...
def test_c003_sql_door_never_castable_refuses_ansi_on(cell: str, expr: str, source: str) -> None:
...
def test_c004_python_door_stale_frame_keeps_refusal() -> None:
    """C-004 on the Python door: a frame selected under ANSI on still refuses after the SET."""
    spark = _session()
    frame = spark.range(1).select(F.lit(1).cast("binary").alias("b"))
    _ansi_off(spark)
