...
def test_nested_struct_stdout_ends_with_blank_line(spark: ReparkSession) -> None:
    """Nested tree plus Spark's blank line. pins: df-printschema-1-trailing-newline/C-001, C-002"""
...
def test_level_1_stdout_ends_with_blank_line(spark: ReparkSession) -> None:
    """Level-1 tree plus Spark's blank line. pins: df-printschema-1-trailing-newline/C-001, C-002"""
    frame = _shapes(spark)["nested"]
    buf = io.StringIO()
    with redirect_stdout(buf):
        frame.printSchema(1)
