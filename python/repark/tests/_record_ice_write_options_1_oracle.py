...
def _record(cell_id: str, spark: Any, table: str, write: Any) -> None:
    ...
    CELLS.append(entry)
    print(f"cell {cell_id}: " + ("ERROR " + str(entry["error"])[:160] if entry["error"] else f"summary_keys={sorted(entry['summary'])} files={len(entry['files'])}"), flush=True)
...
def main() -> None:
    ...
    try:
        ...
        _record("SNAP-00-baseline-append", spark, "snap_base", lambda: (
            _frame(spark, 2).writeTo(f"{t}.snap_base").append()
        ))

        fresh("snap_prop")
        _frame(spark).writeTo(f"{t}.snap_prop").append()
        _record("SNAP-01-run-id-append", spark, "snap_prop", lambda: (
            _frame(spark, 2).writeTo(f"{t}.snap_prop")
        ...
        _record("SNAP-02-two-props", spark, "snap_two", lambda: (
            _frame(spark, 2).writeTo(f"{t}.snap_two")
            .option("snapshot-property.run_id", "abc-123")
        ...
        _record("SNAP-03-dyn-overwrite", spark, "snap_part", lambda: (
            _frame(spark, 2).writeTo(f"{t}.snap_part")
            .option("snapshot-property.run_id", "dyn-1")
            .overwritePartitions()
        ))

        _record("SNAP-04-create-replace", spark, "snap_create", lambda: (
            _frame(spark).writeTo(f"{t}.snap_create")
            .option("snapshot-property.run_id", "ctas-1")
            .createOrReplace()
        ))

        _record("SNAP-05-v1-saveastable", spark, "snap_v1ctas", lambda: (
        ...
        _record("SNAP-06-v1-insertinto", spark, "snap_v1ins", lambda: (
        ...
        _record("SNAP-07-no-prop-after", spark, "snap_stale", lambda: (
            _frame(spark, 2).writeTo(f"{t}.snap_stale").append()
        ))

        fresh("fmt_parquet")
        _record("FORMAT-01-parquet", spark, "fmt_parquet", lambda: (
            _frame(spark).writeTo(f"{t}.fmt_parquet")
            .option("write-format", "parquet")
            .append()
        ))

        fresh("fmt_orc")
        _record("FORMAT-02-orc", spark, "fmt_orc", lambda: (
            _frame(spark).writeTo(f"{t}.fmt_orc")
            .option("write-format", "orc")
            .append()
        ))

        fresh("fmt_avro")
        _record("FORMAT-03-avro", spark, "fmt_avro", lambda: (
            _frame(spark).writeTo(f"{t}.fmt_avro")
            .option("write-format", "avro")
            .append()
        ))

        fresh("fmt_bogus")
        _record("FORMAT-04-bogus", spark, "fmt_bogus", lambda: (
            _frame(spark).writeTo(f"{t}.fmt_bogus")
            .option("write-format", "bogus")
            .append()
        ))

        fresh("opt_size")
        big = spark.range(20000).withColumnRenamed("id", "id").withColumn(
            "name", __import__("pyspark.sql.functions", fromlist=["lit"]).lit("pad")
        )
        big.writeTo(f"{t}.opt_size").append()
        _record("OPT-01-target-size", spark, "opt_size", lambda: (
            big.limit(20000).writeTo(f"{t}.opt_size")
            .option("target-file-size-bytes", "65536")
            .append()
        ))

        fresh("opt_codec")
        _record("OPT-02-codec", spark, "opt_codec", lambda: (
            _frame(spark).writeTo(f"{t}.opt_codec")
            .option("compression-codec", "gzip")
            .append()
        ))

        fresh("opt_level")
        _record("OPT-03-level", spark, "opt_level", lambda: (
            _frame(spark).writeTo(f"{t}.opt_level")
            .option("compression-codec", "gzip")
        ...
        fresh("opt_dist")
        _record("OPT-04-dist", spark, "opt_dist", lambda: (
            _frame(spark).writeTo(f"{t}.opt_dist")
            .option("distribution-mode", "hash")
            .append()
        ))

        fresh("opt_fanout")
        _record("OPT-05-fanout", spark, "opt_fanout", lambda: (
            _frame(spark).writeTo(f"{t}.opt_fanout")
        ...
        _record("OPT-06-isolation", spark, "opt_iso", lambda: (
            _frame(spark, 2).writeTo(f"{t}.opt_iso")
        ...
        _record("OPT-07-isolation-bad", spark, "opt_iso_bad", lambda: (
            _frame(spark, 2).writeTo(f"{t}.opt_iso_bad")
            .option("isolation-level", "bogus")
            .overwritePartitions()
        ))

        fresh("opt_null")
        _record("OPT-08-nullability", spark, "opt_null", lambda: (
            _frame(spark).writeTo(f"{t}.opt_null")
            .option("check-nullability", "false")
            .append()
        ))

        fresh("opt_order", partitioned=True)
        _record("OPT-09-ordering", spark, "opt_order", lambda: (
            _frame(spark).writeTo(f"{t}.opt_order")
            .option("check-ordering", "false")
            .append()
        ))

        fresh("opt_unknown")
        _record("UNKNOWN-01", spark, "opt_unknown", lambda: (
            _frame(spark).writeTo(f"{t}.opt_unknown")
        ...
        _record("SQL-00-baseline-insert", spark, "sql_base", lambda: (
            spark.sql(f"INSERT INTO {t}.sql_base SELECT 2 AS id, 'b' AS name")
        ))
        ...
        _record("SQL-01-set-conf", spark, "sql_conf", lambda: (
            spark.sql(f"INSERT INTO {t}.sql_conf SELECT 2 AS id, 'b' AS name")
        ))
