...
SQL door with DataFrame-door reads beside every move. Live tier
(REPARK_PARITY_LIVE=1): Spark replays the truth (drift detector) and the engines
cross-read each other's moved tables. The Rust ANSI door carries the same move
...
def _fresh_facade(
    tmp_path: Path, test_name: str, table: str, ddl: str, seed: str, *, allow_v3: bool = False
) -> tuple[ReparkSession, Path]:
    """Facade session plus seeded table; caller stops the session."""
    catalog = _catalog(test_name)
    warehouse = tmp_path / "wh"
    builder = ReparkSession.builder.appName(f"reorder-{test_name}")
    if allow_v3:
...
def test_move_first_matches_oracle(tmp_path: Path) -> None:
    """ALTER COLUMN b FIRST reorders with ids intact (SQL plus DataFrame read)."""
...
def test_move_after_matches_oracle(tmp_path: Path) -> None:
    """ALTER COLUMN b AFTER id reorders with ids intact (SQL plus DataFrame read)."""
...
def test_after_unknown_column_refuses(tmp_path: Path) -> None:
    """AFTER an unknown column raises Spark's UNRESOLVED_COLUMN and writes nothing."""
...
def test_move_unknown_column_refuses(tmp_path: Path) -> None:
    """Moving an unknown column raises Spark's UNRESOLVED_COLUMN and writes nothing."""
...
def test_nested_field_move_matches_oracle(tmp_path: Path) -> None:
    ...
    session, warehouse = _fresh_facade(tmp_path, "nested", "t", _NESTED_CTAS, "")
    try:
        catalog = _catalog("nested")
        qualified = f"{catalog}.{_NAMESPACE}.t"
        root = _table_root(warehouse, catalog, "t")
        _, repark_before = _current_order_ids(root)
        assert repark_before == before_ids
        ...
        assert arrow.column("s").to_pylist() == [{"a": 1, "b": "x"}]
    finally:
        session.stop()


def test_struct_top_move_matches_oracle(tmp_path: Path) -> None:
    """ALTER COLUMN s FIRST moves the whole struct with ids intact."""
    case = _TRUTH["cases"]["struct_top_v2"]
    session, warehouse = _fresh_facade(tmp_path, "structtop", "t", _NESTED_CTAS, "")
    try:
...
def test_positional_insert_and_select_after_move(tmp_path: Path) -> None:
    """INSERT in the new positional order lands rows the moved table reads back."""
...
def test_move_on_v3_matches_oracle(tmp_path: Path) -> None:
    ...
    session, warehouse = _fresh_facade(tmp_path, "v3", "t", ddl, _PLAIN_SEED, allow_v3=True)
    try:
        catalog = _catalog("v3")
        qualified = f"{catalog}.{_NAMESPACE}.t"
        session.sql(f"ALTER TABLE {qualified} ALTER COLUMN b FIRST")
