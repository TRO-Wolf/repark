...
def test_register_function_returns_udf_object(spark: ReparkSession) -> None:
    """registerFunction answers the UserDefinedFunction object where Spark's alias returns f (EX-SES-1)."""
...
def test_new_session_action_promotes_active(spark: ReparkSession) -> None:
    """An action on a newSession() result promotes it active where Spark keeps the caller (EX-SES-2)."""
