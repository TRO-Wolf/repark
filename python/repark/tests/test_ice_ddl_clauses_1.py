...
def test_alter_table_set_location_moves_the_metadata_location(spark: Any, tmp_path: Path) -> None:
    ...
    assert sorted(old_table.rglob("*.parquet")) == pre_move
    ...
    assert len(list(old_table.glob("metadata/*.metadata.json"))) == 2
    assert len(list((moved / "metadata").glob("*.metadata.json"))) == 2
    post_move = sorted(moved.rglob("*.parquet"))
    assert len(post_move) == 1
    assert post_move[0].name != pre_move[0].name
    assert sorted(old_table.rglob("*.parquet")) == pre_move
    arrow = spark.sql(f"SELECT id, data FROM {table} ORDER BY id").to_arrow()
    assert arrow.to_pylist() == [{"id": 1, "data": "a"}, {"id": 2, "data": "b"}]
