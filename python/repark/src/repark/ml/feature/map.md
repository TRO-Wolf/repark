# map — python/repark/src/repark/ml/feature

## Purpose

Plan-built `pyspark.ml.feature` transformers under `repark.ml.feature` (M2 + Q1
quantile unlock). `fit` = session aggregates; `transform` = SQL/plan expressions only.
