...
Near-drop-in `pyspark.ml` surface under `repark.spark.ml` (Q1 re-home, 2026-08-14;
no `pyspark.ml` alias shim). Pipeline skeleton (M1), feature transformers (M2),
native estimators (M3), delegated backends + tuning (M4).
