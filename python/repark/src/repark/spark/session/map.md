...
| `reader_support.py` | Reader option sets and integer bounds, JDBC integer options, reader paths, JSON empty-input checks, CSV promotion (bigint/double/boolean/timestamp after nullValue; timestamp only when a cell has `:`), inferred-timestamp cast to instant `timestamp` via CAST(str AS TIMESTAMP) so `spark.sql.session.timeZone` localizes the wall clock (round 3). JSON also runs that cast; DataFusion infers Utf8 so it is a no-op. pins: nullability-2/C-006 |
