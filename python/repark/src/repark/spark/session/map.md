...
| `session_configuration.py` | SQLConf defaults, DataFusion validation/forwarding, display-style normalization. `_SQLCONF_DEFAULTS` holds declared inference, session-timezone, and timestamp-type defaults. Runtime temp-directory `SET` stays refused (the Rust builder already applied it). DataFusion `SET` values route through the shared Spark string-literal helper. **DISPLAY-POLARS-1 step 1 (2026-09-09):** default flips to `polars` via new `default_display_style()` (env `REPARK_DISPLAY_STYLE` first, refuse-loud through `normalize_display_style`; builder config and `session.display_style` keep outranking both), re-exported through `_funcs.py` beside its two siblings. pins: display-polars-1/C-001 |
