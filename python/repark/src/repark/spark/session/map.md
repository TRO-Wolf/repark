...
| `reader_support.py` | Reader option sets and integer bounds, JDBC integer options, reader paths, JSON empty-input checks. **CSV-INFER-PERF-1:** inferSchema without `nullValue` keeps native types and CAST-only the columns DataFusion inferred as Timestamp (Utf8 re-read so offsets survive). `nullValue` still Utf8-forces and promotes bigint/double/boolean/timestamp/date via one `try_cast` failure-count aggregation (timestamp only when a cell has `:`, date only when it does not). JSON also runs the leftover ntz recast; DataFusion infers Utf8 so it is a no-op. pins: nullability-2/C-006; csv-infer-perf-1/C-002, C-005 |
