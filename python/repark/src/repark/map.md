...
The importable `repark` package — the near-drop-in PySpark facade
(`from repark import ReparkSession`; `SparkSession` is a drop-in alias). A thin, typed Python
...
- `dataframe.py` — **r24 DF1** (`# === r24 DF1: dynamicFlatten ===`): repark-extra
  ...
  (`drop_null_lists=True`), `max_depth=100`
...
- `functions.py` / `dataframe.py` / `column.py` — explode/explode_outer guarded unnest
  ...
  ColumnOrName not free SQL); element type only for explode_outer; no BIGINT fail-open;
