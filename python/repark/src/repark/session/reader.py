...
class DataFrameReader:
    ...
    def smartCsv(  # noqa: N802 — repark extension camelCase (Q5 contract)
        ...
        * Inference sampling (r26): full scan when ≤ 10_000 data rows, else first
        ...
        """
        from repark._csv_smart import load_smart_csv
    ...
    def _load_csv(self, path: str | Path | list[str]) -> DataFrame:
        ...
        return self._apply_reader_schema_semantics(
            frame, infer_schema=infer, header=self._option_bool("header", default=False)
        )
    ...
    def _apply_reader_schema_semantics(
        ...
        * No header and no user schema → rename DF ``column_N`` names to Spark ``_c0``, ``_c1``.
        """
        from repark import functions as F  # noqa: N812 — local import avoids cycle at module load
