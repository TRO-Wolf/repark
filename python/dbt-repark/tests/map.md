...
- `test_statement_surface.py` — 30 cases: every statement shape dbt emits, run through
  `repark.sql()` on a memory catalog. Eighteen served, ten refused with the exact message
  ...
  **WO-C3 C5 (2026-09-23):** `fetch_tbl_properties` moves to served for an Iceberg table;
  the core SHOW pin compares the complete `(key, value)` property rows and Arrow schema.
  pins: wo-c3/C-005
