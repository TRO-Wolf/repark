...
- [spill/](spill/map.md) — **NEVEROOM-1 step 1 (2026-09-10):** the spill-coverage matrix
  ...
  `completed` / `refused`, with `KILLED` failing the matrix), the worker subprocess
  entry, and the one CI-tier cell (`sort` at 2× the 64 MB limit, measured `spilled`).
...
- `test_cap_1_source_file_line_cap.py` — DISPLAY-LAZY-1 (2026-09-11): `dataframe/core.py` row 4486 → 4485 with the script baseline; the checkpoint-arm `_eager_shape` record is paid for by deleting the cache-pinned early-return rationale (fact restated on the dataframe map). pins: display-lazy-1/C-007
