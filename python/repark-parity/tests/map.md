...
- `test_dl_6_docs_links.py` — **DOCS-LINKS-1 (2026-09-09):** the markdown link gate on a
  ...
  closed (exit 2); the real tree runs green under the seeded allowlist. The tests carry no
  inline pins, so the unit's clauses are cited on this line.
  pins: docs-links-1/C-001, C-002, C-003
