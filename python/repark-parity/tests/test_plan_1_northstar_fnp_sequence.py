...
def test_live_slate_retires_v3e_5_and_queues_the_safe_work() -> None:
    """C-002: the rolling slate starts with RP-2 and uses FNP work only in the fork wait."""
    slate = _read("briefs/next-sequence.md")
    _assert_in_order(slate, ("| 1 | **RP-2**", "| 2 | **FNP-15/16**"))
    assert "<!-- unit id=v3e-5" not in slate
    assert "Fork F-17 runs in parallel" in slate
    assert "It does not gate v1.0" in slate
    assert "RP-3 opens after RP-2 and F-17" in slate
    status = _read("STATUS.md")
    assert "V3E-5 added the nightly v3 live-oracle leg" in status
    assert "**Next:** salvage RP-2" in status
...
def test_navigation_describes_the_revised_authoritative_documents() -> None:
    ...
    assertions = {
        "briefs/map.md": ("guarded **RP-2**", "per remaining unit or coupled pair"),
        ...
        "task/ledgers/staging/map.md": ("narrowed by the", "F-17 plus RP-3"),
    }
