...
def test_north_star_matrix_carries_the_three_engine_rulings() -> None:
    ...
    assert "V3-DV-1` FIXED (RP-7" in mor and "F-18" in mor
    ...
    assert "`V3-DV-1` **FIXED**" in status
    assert "[V3-DV-1](docs/spark-sql-iceberg-parity.md)" not in status
    assert "[V3-ROWID-3](docs/spark-sql-iceberg-parity.md)" not in status
