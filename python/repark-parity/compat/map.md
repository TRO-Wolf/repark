...
| [`smoke_suite.py`](smoke_suite.py) | Always-green Apache pin list + meta-pins (**215** PASS after FA-4/G15 demotion from 218; known-FAIL metas = `test_field_accessor` FAIL-VALUE nested dotted resolve, FA-4 `test_infer_map_pair_type_empty` + `test_infer_map_pair_type_with_nested_maps` FAIL-VALUE, G15 `test_udf_with_collated_string_types` FAIL-MISSING; each known-FAIL pins status **and** a cause fragment) |
...
<!-- 2026-08-22: pins 218 -> 215 — demote FA-4 map-infer pair + G15 collated UDF to known-FAIL
     meta pins (status + cause-string). Nightly had been red on stale PASS pins since 08-14. -->
