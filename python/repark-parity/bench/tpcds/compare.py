"""Sorted- or ordered-row TPC-DS result comparison.
...
- Row order: multiset (sort then compare) **unless** ``ordered=True`` (ORDER BY
  queries — compare in engine return order).
"""
...
def compare_result_sets(
    ...
    *,
    ...
    if ordered:
        ...
        left_seq = sorted(left, key=_sort_key)
        right_seq = sorted(right, key=_sort_key)
        position_label = "sorted row"

    for index, (left_row, right_row) in enumerate(zip(left_seq, right_seq, strict=True)):
        if len(left_row) != len(right_row):
            return CompareResult(
                equal=False,
                message=(
                    f"column count mismatch at {position_label} {index}: "
        ...
        for col_index, (left_cell, right_cell) in enumerate(zip(left_row, right_row, strict=True)):
            if not _cells_equal(left_cell, right_cell):
                ...
                return CompareResult(
                    equal=False,
                    message=(
                        f"value mismatch at {position_label} {index} col {col_label}: "
