...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        storage = str(config.get("storage", "parquet"))
        warehouse = config.get("warehouse")
        warehouse_path = Path(str(warehouse)) if warehouse else None

        engine = str(config.get("engine", "repark"))
        timeout_retry_s = float(config.get("timeout_retry_s", 300.0))
        duckdb_conn = runner._open_duckdb_over_parquet(data_dir)
        if engine == "sail":
            sail_engine = importlib.import_module(f"{package_name}.sail_engine")
            sail_session = sail_engine.open_sail_over_parquet(data_dir)
            spark = sail_session.spark
            subject_label = "sail"
        else:
            spark = runner._open_repark(
                data_dir,
                storage=storage,
        ...
        # Structured ERROR so the parent scoreboard never depends on stderr alone (C1-Q-002).
