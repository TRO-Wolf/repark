# coordinator lessons ledger — observed pattern → evidence → the instruction it becomes

Renamed 2026-09-22 from `xorch` (owner: the name did not roll off the tongue). Rows below keep the names they were written with: `xorch` = this directory, `xo-<engine>` = a lane unit, `xgate.sh`/`xreview.sh`/`xpr.sh` = today's `gate.sh`/`review.sh`/`pr.sh`, `xr-<lane>` = today's `rv-<lane>` critic clone, `XORCH_*` = `COORDINATOR_*`.
