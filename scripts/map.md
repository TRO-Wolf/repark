...
DISPLAY-LAZY-1 step 2 (2026-09-10): `check_lib_py.py` `dataframe/core.py` — the
checkpoint-arm `_eager_shape` record adds two lines (no new slot, so the CAP-1
`dir()` freeze is untouched). Close-out round: no increase — the two lines are funded
by deleting the three-line cache-pinned early-return rationale in the same seat, so
the row ratchets 4487→4486 with the debt and split-seam notes unchanged; the fact
lives on the dataframe map. Ratchets DOWN. pins: display-lazy-1/C-007
