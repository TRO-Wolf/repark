...
ICE-CATALOG-SESSION-1 (2026-09-20): `check_rust_file_size.py` ratchets `repark-spark/src/alter.rs` 1449 → 1444 (short-name completion and the source-anchored RENAME dest move to `use_ddl.rs`; the stale resolver doc and the premature token-gate go with them), shrink-only. pins: ice-catalog-session-1/C-022
