...
FNP-AGG-1 slice (d) (2026-09-21): `check_lib_py.py` sets `functions.py` 1983 → 1984 (the installer import) with the CAP-1 mirror; `check_example_coverage.py` registers `functions_agg_1.py` / `FNPAGG1_EXPORTS`, pays its two registry lines by compacting docstring prose, and `BACKLOG_BASELINE` holds at 108 (grouping_id was never backlogged). pins: fnp-agg-1/C-007
FNP-AGG-1 slice (d) (2026-09-21): `check_lib_rs.py` sets `repark-functions` 182 → 184 (the `mod grouping;` decl plus the re-export); `check_rust_file_size.py` ratchets `column/mod.rs` 1013 → 1012 (the n-ary widening is net-negative) with the CAP-1 mirror. pins: fnp-agg-1/C-007
