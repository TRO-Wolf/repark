...
| C-005 | Result values come from the current snapshot id and summary keys `total-records` / `total-data-files`; missing snapshot → nulls, never a fabricated walk | adopted engine table matches loaded summary; schema-only CREATE adopt is three nulls | PROVEN | `call_register_table_adopts_an_engine_written_table_and_returns_sparks_three_columns`; `call_register_table_of_a_table_with_no_snapshot_returns_three_nulls` |
