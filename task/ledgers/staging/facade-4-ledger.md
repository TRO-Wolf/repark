...
Q-R13-14 measured numbers (run-14b round-3 ABAB, 7 reps each side, medians; full tables in the step-1 perf doc):

| cell group | worst slower-than-base | verdict |
|---|---|---|
| end-to-end walls (cells a/d) | +1.8 % `collect` 1e5; +0.6 % csv infer; all others ≤0 | inside ±5 % |
| surface cells | `dtypes` −3.5 %/−4.3 % (flat7/wide50), −29.7 % (nested3); `df.schema`/`printSchema`/DESCRIBE all inside or faster | inside ±5 % |
| per-call conversions | +59.5 µs (`repark_type_to_arrow` wide50); +72.8 µs (`toDDL` wide50) | inside 1 ms |
| error/refusal paths | all verified correct answers, ≤215 µs | inside 1 ms |
...
- Commits: `f8e33cca` ledger skeleton; `585ef87a` pins + goldens + mutation
  ...
  `ace2e19b` R14b-D-2 dtypes literal-method restoration; this commit the
  round-3 measurement, perf doc and ledger evidence.
