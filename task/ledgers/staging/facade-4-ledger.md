...
|---|---|---|---|---|
| P1-DTYPES | P1 | per-column `simpleString` descriptor FFI regressed `dtypes` +106 % on wide50 | `24ae701d` | C-016 |
| P2-DICT | P2 | descriptor dict tree used un-interned keys/kind tags and owned `String` extraction per node | this section's P2/P3 commit | C-018 |
| P3-COLLATION | P3 | `SparkString.collation` allocated a `String` per node for the default collation | this section's P2/P3 commit | C-019 |
| P3-PARSE-DEPTH | P3 | `parse_ddl` recurses with no depth cap while the name surface bounds at 32 | not changed — owner question (base accepts DDL deeper than 32; a cap would change a refusal) | — |
