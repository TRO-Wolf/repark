...
| C-014 | S1d no regression: `run_baseline.py` re-run on this branch and on the base `/tmp/f-types4` release native, same box, back to back, idle builders before each cell, 8 GiB scope. Bar: no end-to-end cell slower by >5 %, no per-call conversion slower by >1 ms. Thinned line counts of `types.py`/`_csv_smart.py`/`timestamp_type.py` recorded; the two refreshed `test_production_file_size` body-hash baselines named (`_data_type_to_sql_type`, `_sql_type_to_arrow`). | `docs/perf/facade-4-types-step1-2026-09-14.md` before/after table; bars hold. | **PROVEN** | `docs/perf/facade-4-types-step1-2026-09-14.md` (run-14b round-3 re-measure): ABAB driver, one persistent worker per side, 7 alternating reps per cell, medians, loadavg recorded, cells behind a no-builder wait, drift-flagged cells re-run. Every end-to-end wall within ±5 % (worst +1.8 % `collect`, most +0.6 % csv infer); every surface cell inside ±5 % (`dtypes` −3.5 % to −4.3 % flat7/wide50, −29.7 % nested3); worst per-call conversion +59.5 µs (wide50 `repark_type_to_arrow`) vs the 1 ms bar; DDL parse faster than base on every schema; inbound `struct_type_from_arrow` faster than base everywhere after tagged tuples. Line counts: `types.py` 1834→1772, `_csv_smart.py` 899→875, `timestamp_type.py` 97→94, `create_dataframe_values.py` 569→518, `create_dataframe_inference.py` 737→713, `_type_table.py` new 475. Body-hash baselines refreshed for the two moved bodies only. Round-4 update (L-007..L-009): `types.py` 1772→1792, `_type_table.py` 475→582, `create_dataframe_values.py` 518→568; the round-4 surface/micro re-measure is in the doc (every surface cell inside ±5 % or faster). Caveat recorded: two passes this round ran against a debug native written over the release `.so` at 18:19 and inflated every native-touching cell 3–6×; the release `lib_native.so` was restored by copy before the recorded passes. |
| C-015 | Residue rows — every entry point left on its Python path, with the reason byte-identity cannot ride the table: (1) `json`/`jsonValue`/`fromJson`/`_parse_datatype_json_value` — JSON shape plus `__COLLATIONS` metadata walk is facade policy; (2) descriptor trees containing a foreign `DataType` subtype — `_ddl_token_python`/`_sql_type_token_python`/`_repark_type_to_arrow_python` and the `simpleString`/`_engine_type` MRO dispatch keep today's container spellings and leaf fallbacks; (3) decimals outside the Arrow FFI scale envelope (`pa.decimal128(10,300)` accepts where arrow-rs cannot represent) — `_arrow_type_to_repark_python`/`_struct_type_from_arrow_python`/`_repark_type_to_arrow_python` keep pyarrow's own acceptance/refusal bytes; (4) collation refusal — `refuse_evaluated_collation` stays Python policy in `_data_type_to_sql_type`; (5) `_sql_type_to_arrow` decimal and nested spellings — the `PySparkTypeError` wrap stays on the Python parse route. | Census pins cover each residue class; fallbacks named in the residue table below. | **PROVEN** | D16 (`list<item: int32 not null>` arrow-in), D6/D9 (decimal edges) and the unknown-subtype `toDDL`/`simpleString` paths are pinned and green; no refusal class or message moved. |

| C-016 | S1-remediation P1-DTYPES (Grok S2-21): `dtypes` regressed +106 % wide50 / +80 % flat7 / +16.7 % nested3 because every `simpleString()` paid a descriptor-dict build + PyO3 crossing + Rust re-parse. Fix: the atomic classes' `simpleString`/`_engine_type` answers are Python-side constants and parameter derivations in `_type_table.py::_ATOMIC_TYPE_ROWS`, byte-identical to the Rust table (MRO scan so foreign subclasses keep the inherited base answer); nested trees compose in Python over the same answers because one nested descriptor FFI (~15 µs on nested3 `mid`) alone breaks the +5 % bar. Agreement pin: `test_atomic_tokens_agree_with_rust_table` checks every atomic class against `simple_string_from_descriptor`/`engine_token_from_descriptor`. | `dtypes`/`df.schema`/`printSchema`/DESCRIBE within +5 % of base on flat7/wide50/nested3 (measured in the step-1 perf doc surface table); the agreement pin green. | **PROVEN** | Correction (run-14b round 3): the earlier "+2 %" wide50 dtypes evidence was un-reproducible — the dict-token leaf cost ~0.24 µs vs base's ~0.05 µs literal method, and the idle-gated probe plus three same-load A/B re-measures showed `dtypes` +13.0 % flat7 / +15.5 % wide50 (+16–19 %), outside the bar. Ruling R14b-D-2 restored base's dispatch shape: per-class literal `simpleString`/`_engine_type`/`typeName` on every constant-answer atomic class; the five dynamic-answer classes + `DataType` keep `type(self).typeName()` bodies; `_atomic_token` stays for nested composition, foreign subclasses and the SQL/DDL marker columns. Post-fix ABAB (7 reps, medians): `dtypes` flat7 10.92→10.45 µs (−4.3 %), wide50 62.30→60.14 µs (−3.5 %), nested3 47.26→33.24 µs (−29.7 %); leaf `simpleString` 0.052 µs vs base 0.053 µs; `df.schema`/`printSchema`/DESCRIBE all inside ±5 % or faster. The agreement pin and subclass golden still hold; the MRO-skip mutation still turns 12 pins red. `types.py` exact baseline 1610→1772, below main's 1834. Round-4 re-measure after the L-007..L-009 dispatch restoration (release native, idle_load 2.63): `dtypes` −14.9 % flat7 / −15.9 % wide50 / −44.9 % nested3, `df.schema` −5.1 %/−5.4 %/−48.4 %, `printSchema` −10.7 %/−2.8 %/−33.9 %, DESCRIBE +0.7 %/+0.5 %/+0.1 % — every surface cell inside ±5 % or faster; leaf `simpleString` −34.3 %/−32.1 %/−13.6 % vs base. `types.py` exact baseline 1772→1792. |
...
| C-018 | S1-remediation P2-DICT (Grok S2-21): the descriptor crossed PyO3 as a `PyDict` tree with un-interned keys/kind tags and owned `String` extraction per node. Fix: every key and kind tag interned via `pyo3::intern!` on both directions of `type_bridge.rs`; inbound `kind`/collation extract as `Cow<'_, str>` (borrowed compare); the missing-key refusal keeps its `type-table descriptor is missing key "…"` bytes. Companion fix found by the re-measurement: `_type_table.py`'s decode rebuilt its kind→class map and re-imported `types` per recursive node (~30 µs on nested3's `mid`, `df.schema` +50 %) — the maps and module handle now cache. | `dtypes`/`df.schema`/`printSchema` within +5 % of base on flat7/wide50/nested3; reviewer micro table and tracemalloc peaks re-measured back to back; census pins green. | **PROVEN** | Round-3 ABAB re-measure (7 reps, medians, doc's surface + micro tables): all surface cells inside ±5 % or faster — `dtypes` −4.3 %/−3.5 %/−29.7 %, `df.schema` −7.3 %/−4.9 %/−50.8 %, `printSchema` −3.6 %/−1.8 %/−35.0 % flat7/wide50/nested3. Micro (µs, base→branch): `repark_type_to_arrow` 9.6→19.6 / 58.7→118.2 / 22.6→30.5; `struct_type_from_arrow` 15.0→11.8 / 101.6→69.1 / 29.0→19.0 (faster than base everywhere after the C-026 tagged tuples); `fromDDL` 20.5→15.0 / 148.1→87.0 / 53.6→29.5 (faster everywhere); real `toDDL` 3.7→15.3 / 26.3→99.5 / 9.8→23.2; `simpleString` 3.0→3.9 / 19.4→20.3 / 4.6→9.7. Round-4 re-measure on the release native (idle_load 2.63; `_field_descriptor` + `None` propagation replaced the second tree walk): `repark_type_to_arrow` 11.8→15.2 / 60.4→58.5 / 22.9→19.1; `struct_type_from_arrow` 15.1→17.2 / 100.7→68.2 / 28.9→19.1; `fromDDL` 20.4→17.0 / 146.7→77.0 / 53.4→21.7; `toDDL` 3.8→6.9 / 26.2→40.1 / 10.0→12.1; `simpleString` 3.1→2.0 / 19.5→13.2 / 4.5→3.9; `jsonValue` ≈flat on all three. |
...
| C-029 | L-007 (critic re-check, `/tmp/oc-worker/f-crit4s1b/report.md`): `StructType.simpleString` ignored a `StructField` subclass `simpleString` override (and `ArrayType`/`MapType` dropped child overrides) because the container join read `field.name:child` tokens instead of calling `field.simpleString()`. Fix: base's container bodies restored — `ArrayType`/`MapType`/`StructField`/`StructType` `simpleString`/`_engine_type` compose through the child/field methods; `StructType.simpleString` joins `field.simpleString()` per member exactly as base did. | `test_facade_4_step1_remediation.py` L-007 pins green — field-override inside struct/array/map and nested trees. | **PROVEN** | Red-first: `StructType([OverField("a", IntegerType())]).simpleString()` answered `struct<a:int>` where base answers the override. Post-fix byte-identical: override survives inside `StructType`, `ArrayType`, `MapType` and deep nesting; scratch mutation reverting the `field.simpleString()` join turned the pin red, restore green. |
| C-030 | L-008 (critic re-check): an atomic subclass overriding `simpleString` (interval classes and every other tabled class) lost the override on the `toDDL` leaf — `_leaf_ddl` answered the engine token instead of `data_type.simpleString().upper()` as base did. Fix: `_leaf_ddl` answers the ordered DDL marker for tabled classes, then `simpleString().upper()`; `_ddl_token_python` walks containers with the ordered primary so the override survives nested in arrays/maps/structs. | L-008 pins green — `OverDT`/`OverYM` and generic `simpleString`-override leaves through `toDDL` and nested containers. | **PROVEN** | Red-first: `OverDT` leaf answered `INTERVAL DAY TO SECOND` where base answers `OVERRIDE_SIMPLE`. Post-fix override reaches `toDDL` top-level, inside `ARRAY<…>`/`MAP<…>`/`STRUCT<…>`; mutation back to `_engine_type().upper()` turned two pins red, restore green. |
| C-031 | L-009 (critic re-check): a class deriving from two public type classes collapsed to one resolution where base used different orders per surface. Fix: `_arrow_order`/`_sql_order`/`_ddl_order` cache base's three `isinstance` chains; `_primary_class` picks the first matching class per surface; `_atomic_token(order=…)` switches its subclass scan from MRO to the surface order; `DataType.simpleString` answers `_SIMPLE_STRING_FAST` for exact classes then `type(self).typeName()` (the five dynamic classes keep no literal method so MRO matches base); `DataType._engine_type` delegates to `self.simpleString()`; the parameterised `jsonValue`s dispatch `self.simpleString()`; collation refusal runs only after the `_sql_order` pick via `_sql_refuse_collation`. | L-009 pins green — `Dual`/`Dual2`/`Dual3`/`ArrInt` matrix across `simpleString`/`typeName`/`_engine_type`/`jsonValue`/`json`/`toDDL`/`_data_type_to_sql_type`/`repark_type_to_arrow`/`createDataFrame` schema, plus an override on each dynamic class; all recorded from base. | **PROVEN** | Comprehensive MI probe 0/191 diffs and extended edge probe 0/310 diffs vs `/tmp/f-types4`; `createDataFrame([(1,)], cls()).schema` matches on every MI class incl. `Dual3` (no stray `collation` access). Four scratch mutations each turned their pin set red: dynamic `simpleString` restored on `BinaryType` (1 red), ordered scans reverted to MRO (9 red), `jsonValue` un-delegated (1 red), collation refusal moved before the pick (2 red incl. the original createDataFrame crash); restores green. |
| C-032 | L-010 (critic re-check round 3, `/tmp/oc-worker/f-crit4s1c/report.md`): `repark_type_to_arrow` read `.precision`/`.scale` before base's Arrow `isinstance` order resolved, so `class P(Left, DecimalType); P()` constructed without decimal attrs crashed `AttributeError` where base returned the left parent's Arrow type. Fix: the descriptor is built first (the pick lives inside it); the C-028 wide-decimal FFI envelope guard then keys on `descriptor["kind"] == "decimal"` — the only pick that can produce it — so `.precision`/`.scale` are touched exactly when base's chain reaches the Decimal arm. | `test_facade_4_step1_remediation.py` L-010 pins green — all 15 `Left+DecimalType` rows, `DualDec`, and the `StringType+DecimalType+IntegerType` triple through `repark_type_to_arrow`. | **PROVEN** | Red-first: every L-010 row answered base (`int32`/`int64`/`int8`/`int16`/`bool`/`float`/`double`/`string`/`binary`/`date32[day]`/`timestamp[us, tz=UTC]`/`timestamp[us]`/`null`, triple `int32`); scratch mutation restoring the pre-pick `isinstance(DecimalType)` read turned all 17 pins red, restore green; C-028 wide-decimal answers unchanged (`decimal128(10, 300)`, `DecimalType(76,10)` `ValueError` bytes). |
| C-033 | L-011 (critic re-check round 3): `_datatype_to_descriptor` encoded any `isinstance(StructField)` with no surface-order hit as a field and read `.dataType`, so `class P(Left, StructField); P()` for a `Left` outside the orders crashed where base answered the non-field parent. Fix: the `isinstance(StructField)` arm deleted — a field is encoded only from `StructType.fields` through `_field_descriptor`; an unorderable `StructField`-derived object falls to the Python fallbacks like base (`pa.string()` / `simpleString().upper()` / `_engine_type()`). | L-011 pins green — all five `Left+StructField` rows through `repark_type_to_arrow`, wrap `toDDL`, nested Array/Map SQL, and direct SQL, plus `DualField`. | **PROVEN** | Red-first: Arrow `string`, wrap `toDDL` `f INTERVAL DAY TO SECOND`/`f INTERVAL YEAR TO MONTH`/`f INTERVAL`/`f TIME(6)`/`f VARIANT`, array `ARRAY<interval day to second>`/`…year to month`/`…interval`/`…time(6)`/`…variant`, map `MAP<STRING,…>` and direct SQL all match base; scratch mutation restoring the arm turned all 6 pins red, restore green. Critic `probe_r3.py`/`compare_r3.py` rerun unchanged on both trees: **DIFF_COUNT=0** (600 pairs, triples, overrides, FAST, collation — `/tmp/oc-worker/f-types4s1r/probe-r5/`). |

## Step-1 remediation findings — Grok S2-21 (`/tmp/oc-worker/f-rev4-rs/report.md`)

| Finding | Severity | Defect | Fix commit | Clause |
|---|---|---|---|---|
| P1-DTYPES | P1 | per-column `simpleString` descriptor FFI regressed `dtypes` +106 % on wide50; the dict-token fallback still cost 0.24 µs/leaf vs base's 0.05 µs (+13–15.5 % `dtypes`) | `f71b02fa`, `ace2e19b` | C-016 |
...
## Step-1 round-3 findings — critic logic re-check (Grok, `/tmp/oc-worker/f-crit4s1b/report.md`)

The re-check confirms L-001..L-006 equal base and files three new P1
byte-identity breaks; the orchestrator reproduced all three and ruled FIX.

| Finding | Severity | Defect | Fix commit | Clause |
|---|---|---|---|---|
| L-007 | P1 | `StructType.simpleString` and nested token arms bypassed `field.simpleString()`, dropping a `StructField` subclass override inside struct/array/map/nesting | this round | C-029 |
| L-008 | P1 | `toDDL` leaf answered `_engine_type` for a `simpleString`-overriding leaf (interval classes and every other tabled class) instead of `simpleString().upper()` | this round | C-030 |
| L-009 | P1 | multiple inheritance of two public type classes resolved one way on every surface where base used MRO for `simpleString`, an integer-first `isinstance` order for Arrow/SQL, and a string-first order for DDL — plus a `collation` attribute access base never made | this round | C-031 |

## Step-1 round-5 findings — critic logic re-check round 3 (`/tmp/oc-worker/f-crit4s1c/report.md`)

The re-check confirms L-007..L-009 equal base, 580 of 600 ordered
two-parent pairs and every override lattice byte-identical, and files
two P1 families; the orchestrator ruled both FIX.

| Finding | Severity | Defect | Fix commit | Clause |
|---|---|---|---|---|
| L-010 | P1 | `repark_type_to_arrow` read `DecimalType.precision`/`.scale` before the `_arrow_order` pick — `Left+DecimalType` without decimal attrs crashed where base returned the left parent's Arrow type (15 pairs + one triple) | this round | C-032 |
| L-011 | P1 | `_datatype_to_descriptor` treated any `isinstance(StructField)` with no order hit as a field and read `.dataType` — `Left+StructField` for a `Left` outside the orders crashed on Arrow / wrap `toDDL` / nested SQL (5 pairs) | this round | C-033 |

## Step-1 residue — ruled next-round under Q-R13-14 (R14b-D-5)

Both S2-21 perf re-checks PASS the bar (Rust:
`/tmp/oc-worker/f-rev4s1r-rs/report.md`; Python:
`/tmp/oc-worker/f-rev4s1r-py/report.md`). Their remaining P2s are residue —
recorded with the reviewers' numbers, not fixed this round.

| Item | Reviewer numbers | State |
|---|---|---|
| Outbound descriptor is still a `PyDict` tree (P2-DICT-OUT; write side of C-026) | wide50 `repark_type_to_arrow` 77→117 µs (+52 %), tracemalloc 1,456→20,912 B (14.4×); real `toDDL` 25.5→97.7 µs (+284 %), heap 3.9→20.9 KiB; py reviewer: to_arrow 104.5→202.0 µs (+93 %), 73.6 B/call retained, round-trip 236→327 µs — all ≪ 1 ms, 1-row collect/to_arrow ≤ +0.06 % | residue — tagged-tuple encode on the outbound wire is next-round work |
| Nested `simpleString` composition (PY-P2-001 residue) | nested3 `simpleString` 5.00→12.96 µs (+159 %) while container classes lacked Python overrides | **largely closed this round** — L-007 restored the container joins; round-4 re-measure: nested3 `simpleString` 4.49→3.88 µs (−13.6 % vs base), wide50 −32.1 %, flat7 −34.3 % |
| `toDDL` descriptor + native encode | wide50 `toDDL` 52.0→193.3 µs (+272 %) py / 25.5→97.7 µs (+284 %) rs; round-4 re-measure 26.2→40.1 µs (+53.2 %) — still the dict-tree cost above | residue — same outbound-wire item |
...
| R14b-D-2 dtypes surface fix (orchestrator, G-2) | Q1 → option (a): base's dispatch shape restored — per-class literal `simpleString`/`_engine_type`/`typeName` on the constant-answer atomic classes, dynamic classes keep `type(self).typeName()`; the `types.py` exact baseline moves from the branch's 1610 to 1772, still below main's 1834 |
| R14b-D-5 reviewer P2 residue (orchestrator) | the S2-21 re-checks' remaining P2s — outbound `PyDict` descriptor tree (P2-DICT-OUT), nested `simpleString` composition, `toDDL` descriptor+native encode — are ruled next-round residue under Q-R13-14; recorded with the reviewers' numbers in the residue table above, not fixed this round |
| R14b critic re-check L-007..L-009 (orchestrator) | all three P1, FIX byte-identical to base: L-007 field-override joins, L-008 `simpleString().upper()` DDL leaf fallback, L-009 per-surface resolution orders + post-pick collation refusal; the `dtypes` bar held via `_SIMPLE_STRING_FAST` inside `DataType.simpleString` instead of literal methods on the five dynamic classes |
| R14b critic re-check round 3 L-010..L-011 (orchestrator) | both P1, FIX byte-identical to base: L-010 no `.precision`/`.scale` read before the `_arrow_order` pick — the C-028 envelope guard keys on the descriptor's `decimal` kind; L-011 a field is encoded only from `StructType.fields` — the `isinstance(StructField)` descriptor arm deleted |

Q-R13-14 measured numbers (run-14b round-4 ABAB, 7 reps each side, medians, release native, idle_load 2.63; pass 2 at load ~7 reproduced every sign; full tables in the step-1 perf doc):

| cell group | worst slower-than-base | verdict |
|---|---|---|
| end-to-end walls (cells a/d) | +1.8 % `collect` 1e5; +0.6 % csv infer; all others ≤0 (round-3 set, unchanged this round) | inside ±5 % |
| surface cells | `dtypes` −14.9 %/−15.9 %/−44.9 %, `df.schema` −5.1 %/−5.4 %/−48.4 %, `printSchema` −10.7 %/−2.8 %/−33.9 %, DESCRIBE +0.7 %/+0.5 %/+0.1 % (flat7/wide50/nested3) | inside ±5 % |
| per-call conversions | +13.9 µs (`toDDL` wide50); `repark_type_to_arrow` −3.2 % wide50 / −16.5 % nested3 (round-4); round-5 post-L-010 re-measure flat7 +8.2 % (10.11 µs) / wide50 −3.5 % (57.82 µs) / nested3 −17.7 % (18.69 µs) — the reorder did not move the cell beyond the round-4 figure; `simpleString`/`jsonValue` at or under base | inside 1 ms |
| error/refusal paths | all verified correct answers, ≤215 µs (round-3 set, unchanged this round) | inside 1 ms |
...
- Commits: `f8e33cca` ledger skeleton; `585ef87a` pins + goldens + mutation
  ...
  (L-005..L-007, L-009..L-011); `afe8745c` spy remediation (L-008);
  `68fe2f05` step-1 consolidation; `f71b02fa`/`f0ada689`/`a86080b1` S2-21
  remediation; `4c8d25af` critic L-001..L-006 + PY-P3; `d82c5cc6` PY-P2
  tagged tuples + csv engine token; `fe9a5907` findings-table SHAs;
  `ace2e19b` R14b-D-2 dtypes literal-method restoration; `4d624a1d`
  round-3 measurement + perf doc + ledger; `a3424513`/`097306e9` round-4
  L-007..L-009 dispatch restoration, pins, re-measure and ledger; this
  commit the round-5 L-010/L-011 pick-order fixes, pins, micro
  re-measure and ledger.
...
COVERAGE_ATTESTATION:
  ...
  categories:
    ...
    - id: AT-7
      status: ATTACKED
      evidence: Step-1 before/after on the real base release native, same box, ABAB driver alternating sides per rep (7 reps, medians, 8 GiB scope): every end-to-end wall within ±5 % (worst +1.8 %), every surface cell inside ±5 % — round-4 re-measure on the release native after L-007..L-009: dtypes −14.9 %/−15.9 %/−44.9 %, df.schema −5.1 %/−5.4 %/−48.4 %, printSchema −10.7 %/−2.8 %/−33.9 %, DESCRIBE +0.1 % to +0.7 %, worst per-call conversion +13.9 us vs the 1 ms bar; the round-2 dtypes breach (+13–15.5 %) is fixed by the R14b-D-2 literal-method restoration and pinned; the L-007..L-009 dispatch restoration kept the bar via _SIMPLE_STRING_FAST and is pinned by the new MI/override pins; round-5 L-010/L-011 reorder re-measure of repark_type_to_arrow (micro cell, two passes): flat7 +8.2 % (10.11 us) / wide50 −3.5 % (57.82 us) / nested3 −17.7 % (18.69 us) vs round-4's +29.4 %/−3.2 %/−16.5 % — inside the round-4 envelope, ≪ 1 ms.
