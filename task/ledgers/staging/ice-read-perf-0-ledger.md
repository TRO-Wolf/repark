...
| C-019 | R-3 on every AWS step: each `setup` phase ends with the size check and each `run` checks before its first scan (C-007, C-017). The job runs them with no `continue-on-error`, under `set -euo pipefail` and with no `||`, `&&` or `;` recovery after any bench command, so exit 3 fails it at once. It runs cold, warm, concurrent and concurrent-cold on both catalogs at `--repeat 1` (`BENCH_REPEAT`, ruling Q-24a-1: at 3 a dispatch reads about 70 GB and six pass the $25 cap), writes one summary line per run with `run_io_total.bytes` and the dispatch total, and uploads the JSONs. | `test_every_mode_runs_on_both_catalogs_at_one_repeat_with_r3_failing_the_job`, `test_the_bench_job_runs_only_on_its_dispatch_behind_the_same_gate`, `test_no_bench_or_s3tables_failure_can_be_swallowed`; `both_setup_phases_end_with_the_r3_check`; `a_fake_size_above_the_limit_stops_every_mode_and_catalog_before_any_query`. | PROVEN | Green. Round 3 (F-MUT-4B): the only `||` in the job's `run:` scripts is `|| stop "…"` on the two `aws s3tables` calls, and no `cargo bench` command line carries `||`, `&&`, `;` or a pipe. `|| true` after either mode loop's bench, `|| true` after or in place of either compaction `stop`, `; true` after a setup write, or `&& true` after a setup create reds it (2026-09-19). F-MUT-4E: every mode on both AWS catalog paths takes the same R-3 gate (C-008). |
