...
| C-034 | The Spark door stamps the session conf on plain INSERT (positional `VALUES`), COW DELETE overwrite, refuses a bogus session codec, and unstamps after unset. | Spark-door battery. | PROVEN | `crates/repark-spark/src/tests/session_write_conf.rs` — 4 tests green (`session_team_stamps_plain_insert`, `session_team_stamps_cow_delete_overwrite`, `bogus_session_codec_refuses_naming_the_codec`, `unset_session_conf_restores_unstamped_writes`). Round 6 (2026-09-20): the DELETE pin seeds two rows and deletes one, because on one row the statement covered the whole data file and #739 routes that metadata-only, where Spark stamps nothing; the partial delete keeps the pin on the CoW overwrite arm it names. |
...
| C-063 | Round-8 CI regression, `build + import smoke (debug, host)` red on ICE-MIXED-CASE-1 `MC-UPD-01` / `MC-UPD-02`: the owned plain UPDATE resolves an unquoted or wrong-case-quoted SET target case-insensitively and answers, as Spark does under `spark.sql.caseSensitive=false`. | Two Rust pins on the owned route, both red on the pre-fix tree, plus the two facade cells going green. | PROVEN | `canonicalize_identity_selection` canonicalised the SET target through `rewrite_fragment_case`, whose `FragmentRepair::rewrite_ident` sets `quote_style = Some('`')` when the stored spelling differs from the requested one; that is right for the selection and for each SET value (both are SQL fragments) and wrong for a target, which is a bare column name. `eventName` arrived at `validate_update_assignments` as the string `` `eventName` ``, matched no field and refused `UPDATE SET column … does not exist in the target table`; had it passed, `update_projection_sql` matches assignments by `eq_ignore_ascii_case` and would have dropped the SET silently. The rewrite predates this unit (`f43d522d`) but only became reachable for a plain `UPDATE … WHERE <scalar>` when `c1b11764` / `552e5ae6` / `ca5abfb8` moved that statement onto the owned identity route, so the regression is this unit's. `spark_ast::canonical_assignment_target` now resolves the target name-to-name against the table's fields and leaves a non-unique match alone, so the ambiguity and missing-column refusals still fire with their Spark-shaped messages. Pins `an_owned_plain_update_resolves_a_mixed_case_set_target` and `an_owned_plain_update_resolves_a_wrong_case_quoted_set_target`. |
| C-064 | Round-8 CI regression, `test_mw7_scale_smoke` / `test_mw8_runbook` bin-pack bands: RePark's own Parquet writer takes Java's `parquet.enable.dictionary` default — ON when the table names no such property — and only an explicit `false` turns dictionary pages off; the owned plain INSERT, and only it, takes the fork insert exec's opposite rule so no session conf decides the INSERT layout. | Measure the Java default two ways, then pin both arms and the INSERT equality. | PROVEN | `ca5abfb8` copied the fork insert exec's rule (`is_some_and("true")`, absent = OFF) into the SHARED `writer_properties_for`, so every RePark-written data file — CTAS, overwrite, COW rewrite, MERGE lineage, fanout — silently lost dictionary encoding and changed size; the MW-7 C-011 seed fell to 40,242 B and the MW-8 seed to 39,941 B against a band floor of 49,152 B. Measured: (1) `ParquetProperties.DEFAULT_IS_DICTIONARY_ENABLED = true` in the shaded parquet-mr of `iceberg-spark-runtime-4.1_2.13-1.11.0` (javap `-constants`); (2) Spark-written fixture files on tables that name no such property carry dictionary pages — `ice_spark_table_1`'s `ds` and `v3_dv`'s `part` column chunks are `PLAIN_DICTIONARY` with a dictionary page offset, the high-cardinality chunks beside them having fallen back to `PLAIN` under parquet-mr's own size heuristic, which is only reachable with dictionary encoding ON. `writer_properties_with` takes a `fork_insert_dictionary_rule` flag; it is `false` everywhere (absent = ON, Java) and `true` only on the owned append that stands in for the fork's insert exec, set at the two INSERT entry points (`append_with_options.rs` when the statement carries no write options, `repark-sql`'s `session_insert.rs`), so C-045's `a_session_conf_keeps_a_plain_insert_layout` still holds byte-for-byte. Pins `a_table_without_the_dictionary_property_writes_dictionary_pages`, `parquet_enable_dictionary_false_turns_the_dictionary_pages_off`, `parquet_enable_dictionary_true_keeps_the_dictionary_pages_on`, `the_owned_plain_insert_takes_the_fork_insert_exec_dictionary_rule`. OPEN beyond this unit: the fork's insert exec still diverges from Java on the unowned INSERT route, which needs a fork change and a pin bump. |
  ...
  round_5_reattested:
    ...
    - id: AT-10
      ...
      evidence: Every round-5 pin was run against the mutation that names it and
        ...
        before this round, which is the finding the critic filed. Round 8 (the
        four CI regressions this unit introduced) adds six pins: two on the
        owned plain UPDATE's mixed-case SET resolution, four on the
        `parquet.enable.dictionary` default read from the written footers. All
        six red on the pre-fix tree; the whole repark-spark battery (1307),
        repark-iceberg (630) and repark-sql (383) are green with them in, and
        the four regressed facade cells pass.
      artifacts: [crates/repark-spark/src/tests/session_write_conf_removals.rs, crates/repark-iceberg/src/write/summary_collision.rs, crates/repark-iceberg/src/write/static_value.rs, crates/repark-spark/src/tests/parquet_dictionary.rs]
  complete: true
```
