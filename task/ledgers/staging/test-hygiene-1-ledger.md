...
|---|---|---|---|---|
| C-001 | `call_register.rs` excludes cross-process while keeping the baked-in path: a lock directory beside the path, held for the fixture lifetime, wipe-and-copy under the lock. | The diff; `cargo test -p repark-spark --lib call_register` green twice concurrently (same target dir serializes the build, binaries run concurrently). | **PROVEN** | `SparkV3DirLock::acquire` spins on atomic `create_dir` of `/tmp/repark-v3-1-spark-mor.lock` (2 min cap, 25 ms poll), `Drop` removes the dir, the in-process `Mutex` stays the fast path, the fixture holds both. No new crate dependency (`libc`/`fs4`/`fs2` are not direct deps of `repark-spark`; pure-`std` keeps `Cargo.toml`/`Cargo.lock` untouched). Single: 12 passed in 0.19 s. Concurrent double: `a_exit=0 b_exit=0`, both `12 passed; 0 failed`. pins: test-hygiene-1/C-001 |
