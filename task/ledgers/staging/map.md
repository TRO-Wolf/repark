...
- [door-converge-2-ledger.md](door-converge-2-ledger.md) —
  **DOOR-CONVERGE-2 round 1 (2026-09-15), in flight:** one-kernel convergence for the four
  P1 SQL-door wrong answers — `concat` over arrays (element widening, OR `containsNull`,
  `DATA_DIFF_TYPES` on mixed shapes, `Binary` stays `Binary`), `reverse` over arrays,
  `sequence` (int widths, dates, timestamps, step errors) and `split` (Java regex, limit
  semantics) — each pinned on both doors for value, Arrow type and nullability.
  `risk_tier: standard`. Branch `feat/door-kernel-converge-2`.
  pins: door-converge-2/C-001, C-002, C-003, C-004, C-005, C-006
