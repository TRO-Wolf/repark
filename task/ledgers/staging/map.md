...
- [fnp-9-collections-json-ledger.md](fnp-9-collections-json-ledger.md) — **FNP-9/10
  ...
  `array_insert`, `arrays_zip`); nine refused loudly with a §7 row and a pin that reds when the
  name is fixed. Six of the nine share one seam — Spark's multi-column generators
  (`posexplode`, `posexplode_outer`, `inline`, `inline_outer`, `stack`, facade `json_tuple`)
  need a plan shape the facade select path does not have, so the seam is filed once instead of
  six one-column impostors. Branch `feat/fnp-9-collections-json`.
