...
- [eager-own-1-ledger.md](eager-own-1-ledger.md) —
  **EAGER-OWN-1 step 0 (2026-09-13), in flight:** the measured reproduction of
  the bare-`eager()` retention defect — a subprocess pin over the deterministic
  1e6-row × 25-column TA `withColumns` fixture asserting ten orphan
  `__repark_cache_*` registrations after the loop and after `gc.collect()` on
  the base tree, with the per-iteration wall / VmRSS / VmHWM table committed at
  [docs/perf/eager-own-1-2026-09-13/base.json](../../../docs/perf/eager-own-1-2026-09-13/base.json).
  Steps 1–2 (the shared-ownership handle, the orphan-assertion flip, docs) carry
  clauses C-002…C-012 OPEN. No product code. `risk_tier: standard`. Branch
  `fix/eager-own-1`.
