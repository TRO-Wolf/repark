...
**Unit:** EX-12 · **Date:** 2026-09-03 · **Model:** muse-spark-1.2-contributor (continuation of glm-5.3-flash) · **Branch:** `feat/ex-12-functions-aggregates-a` · **Base:** `84c1801`
...
| `first_last.py` | `F.first`, `F.last`, `F.first_value`, `F.last_value` | Group endpoints and their `first_value`/`last_value` aliases answering identically; `ignorenulls` skips the trailing NULL. |
...
| `strings_agg.py` | `F.listagg`, `F.string_agg` | String join of a group's values into one delimited string; the two names are aliases. |
...
| `F.first` (grouped) | `[('a', 1), ('b', 4)]` | `[('a', 1), ('b', 4)]` | kept | `first_last.py` |
| `F.last` (grouped) | `[('a', None), ('b', 6)]` | `[('a', None), ('b', 6)]` | kept | `first_last.py` |
| `F.first_value` (grouped) | `[('a', 1), ('b', 4)]` | `[('a', 1), ('b', 4)]` | kept | `first_last.py` |
| `F.last_value` (grouped) | `[('a', None), ('b', 6)]` | `[('a', None), ('b', 6)]` | kept | `first_last.py` |
...
| `F.listagg` (global `","`) | `'x,y,x,z,z'` | `'x,y,x,z,z'` | kept | `strings_agg.py` |
| `F.string_agg` (global `","`) | `'x,y,x,z,z'` | `'x,y,x,z,z'` | kept | `strings_agg.py` |
...
Dropped names stay on the backlog with both values recorded above; no file asserts them. `approx_percentile`/`percentile_approx` are approximate sketches: Spark 4.1.2 and repark diverge on the grouped 0.5 quantile (`'b'` group 4 vs 5.0) and on type (`3` vs `3.0` globally). `mode` is not yet implemented in repark (engine gap `R-FN-BATCH4`, disclosed).
...
COVERAGE_ATTESTATION:
  ...
  categories:
    ...
    - id: AT-8
      status: ATTACKED
      evidence: make ci stays native-build-free with the new inventory; no facade import in the walk.
