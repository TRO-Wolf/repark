...
| C-025 | `CAT-TYPE-HADOOP` replays EQUAL (`hadoop` aliased to `Memory`; UUID metadata names, not `vN`). | `test_hadoop_type_cell` + `test_hadoop_type_registers_memory_catalog` + `hadoop_type_aliases_to_memory`. | OPEN | Mechanism pinned S7; cell pins S8. |
| C-026 | `CAT-CATALOG-IMPL-INMEMORY` replays EQUAL. | `test_catalog_impl_inmemory_cell` + `test_inmemory_catalog_impl_registers_memory_catalog` + `inmemory_catalog_impl_aliases_to_memory`. | OPEN | Mechanism pinned S7; cell pins S8. |
