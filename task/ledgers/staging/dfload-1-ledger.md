...
| C-001 | A load argument containing `/` that is a table location reads that table's current snapshot, with or without one trailing `/`. | `python/repark/tests/test_iceberg_load_path.py::test_load_table_location_reads_current_rows`, `test_load_table_location_trailing_slash`. | **PROVEN** | Both shapes answer `[[2,"b","y"],[3,"c","x"]]` — the current snapshot after the delete — with BIGINT/STRING Arrow types. On the pre-change branch both pins fail with the measured invalid-identifier refusal (r1 red run). |
...
| C-007 | The path read is static and catalog-free: both scoreboard cells answer Spark's recorded rows on the lane build. | `python/repark/tests/test_iceberg_load_path.py` end-to-end rows, `test_load_path_registers_no_catalog_table`; `nc-inventory` replay of `R-DF-LOAD-PATH` and `R-DF-LOAD-METADATA-JSON`. | **PROVEN** | `cells_read.py` replay on the lane build answers `[[2,"b","y"],[3,"c","x"]]` for both cells, equal to the recorded live-Spark answer; the provider is `IcebergStaticTableProvider` over `StaticTable::from_metadata_file`, registered nowhere (`spark.catalog.table_exists("path.events")` stays false after a path load). |
...
- The fork's local `FileIO::list` is recursive (it walks subdirectories), so `resolve_metadata_location` keeps only entries whose parent is the listed `metadata/` directory — `is_direct_child`, comparing `file://`-stripped spellings because local `FileInfo.location` comes back without the scheme and slash-normalized (measured for `/abs`, `/abs/` and `file:///abs` roots; `file:/abs` is rejected by `file_io_for_location`).
