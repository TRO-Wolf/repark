...
The pins below were proven red by the mutations their rows name, then green on the restored tree.
The mutations M-a to M-open and L-wire, L-ns and L-off (at d1f8fa9b), M-root, M-share, M-whole,
M-over and M-reason (layout-r7), and M-anc, M-stop, M-whole-anc, M-ns and M-cat (layout-r8) ran in
logged runs: a harness applies a mutation, runs the named pins, and restores the file byte for byte,
and each run keeps its diff and cargo log. M1 to M9 (C-001 to C-008) are reported from the
layout-r1 worker's hand-back, with no kept log. C-028 has no mutation run behind it (see its row).
Where a row names two mutations together (M-root with M-share, M-whole-anc with M-stop), both were applied
...
| C-007 | Path-escape identifiers (`..`, `/`) are refused before any path is built, even with a recorded layout root: a table name, a namespace level (top level `..` and `x/y`, and nested `ns.(..)` and `ns.(x/y)`, each created in the registry), and a catalog name (a registry entry named `..` or `x/y`). Each refusal is the complete Plan message, which names the identifier kind (`table`, `namespace`, `catalog`) and the segment. | `mem_layout.rs::mem_layout_refuses_path_escape_identifiers` | **PROVEN** | M7 (move the layout-root branch above the `reject_path_escape_ident` calls): FAILED. Layout-r8: M-ns (drop the namespace loop in `resolve_table_create_location`): FAILED on `mem.[".."].t: expected a Plan refusal, got Ok("<wh>/../t")`. M-cat (drop the catalog check there): FAILED on `got Ok("<wh>/ns/t")`. Under a recorded layout root the catalog name is not a path segment, so the catalog check guards the `repark_ctas` arm, and the pin still reds without it. |
...
| C-009 | The Q-55-6 guard pins stay green. Under `crates/repark-spark/src/call`, the diff against `origin/main` changes only `call/remove_orphan_files.rs` (the C-011 co-tenancy guard: `refuse_scan_over_other_tables` becomes `pub(crate)` and takes the swept `Table`; `refuse_scan_over_foreign_metadata`, `metadata_probe_ancestors`, `foreign_metadata_file` and `foreign_metadata_refusal` are added; and `execute_remove_orphan_files` calls both refusals before a listing or a `file_list_view`), `call/orphan_file_list.rs` (layout-r8: `location_path_start` becomes `pub(super)` so the ancestor walk keeps the scan's spelling; its body is unchanged) and `call/map.md`. `call.rs`, every other `call/` file, `tests/call_orphan.rs`, `view_ddl/` and the ANSI `create_table.rs` are untouched. | `cargo test -p repark-spark --lib call_orphan` | **PROVEN** | `64 passed; 0 failed` at the layout-r8 head. `git diff origin/main...HEAD --stat -- crates/repark-spark/src/call.rs crates/repark-spark/src/call crates/repark-spark/src/tests/call_orphan.rs crates/repark-spark/src/view_ddl crates/repark-sql/src/create_table.rs` lists `call/map.md`, `call/orphan_file_list.rs` (1 line) and `call/remove_orphan_files.rs` only. |
...
| C-011 | Two memory catalogs registered on the SAME warehouse, both `CREATE TABLE ns.t`, share `<wh>/ns/t`. Ruled 2026-09-23 (orchestrator, `u1-cotenancy-guard.md` with the tick-8 addendum, and layout-r7 for V-001): the layout stays, and under `TempFallbackAllowed` `remove_orphan_files` refuses a scan path that holds another table's files. It has two rules. (a) Before listing and before a `file_list_view`, the `*.metadata.json` files under the scan path at any depth, other than the swept table's current metadata file and its `metadata_log` files, are read in sorted path order until the first that refuses. A different `table-uuid` refuses, and so does an unreadable file. The same uuid does not refuse. (b) In the catalog walk, a scan path that equals or contains another registered table's location refuses, a scan path inside another registered table's location and outside the swept table's own refuses, and so does a scan path inside the swept table's own location when another table's location equals it. (c) Layout-r8 (V-001 r2): after (a), the same filter reads `<A>/metadata/` for each proper ancestor `A` of the normalised scan path in order, from its parent upward, stopping at the first refusal, spelled the way the scan path is spelled (`metadata_probe_ancestors`). A scan path strictly inside the swept table's own location stops at that location, inclusive; that last step is the layout-r7 own-metadata probe, and its message is unchanged. A scan path outside the own location walks to the storage root (`/`, or `s3://bkt/`). A scan path equal to the own location walks nothing. A hit at any other ancestor names that ancestor. In this step only `metadata/` directories are read, never a whole ancestor. `file_io.list` on a missing directory returns an empty list (measured on local storage in the bare, `file:///` and `file:/` spellings, on `/metadata`, and on memory storage), so a missing directory is empty; an unreadable one returns `Err` (measured: `Permission denied`), which fails the call. Rules (a) and (c) need no catalog, so they cover another catalog, another session, or no catalog at all. All three rules compare through `normalize_orphan_scan_path` and component-wise `Path::starts_with`. | `crates/repark-spark/src/call/remove_orphan_files.rs::refuse_scan_over_foreign_metadata`, `::metadata_probe_ancestors`, `::refuse_scan_over_other_tables`; C-012 to C-020, C-023 to C-032, C-034 | **PROVEN** (ruled 2026-09-23; the guard is proven by C-012 to C-020, C-023 to C-032 and C-034) | The pins below. See "Two catalogs, one warehouse". |
...
| C-028 | Class sweep: `refuse_shared_temp_fallback_location` refuses a scan path that equals or contains `<root>/repark_ctas` or `<root>/repark_ansi_ctas`. A path strictly inside the fallback root, such as one table's own `data/`, passes this guard by design and is left to the two co-tenancy refusals. | `call_orphan_cotenancy.rs::call_orphan_cotenancy_fallback_root_guard_passes_a_table_dir_inside_the_root` | **PROVEN** | The pin passes at the current head (`cargo test -p repark-spark --lib call_orphan`). No logged mutation run covers this clause: the layout-r7 worker reported M-fallback-inside (the guard also refuses a path inside the fallback root) as failing this pin, and the orchestrator's logged layout-r7 mutation run did not repeat it. |
| C-029 | V-001 r2 (layout-r8). Two memory catalogs on one warehouse: `m1.ns.a` at `<wh>/ns/a` and `m2.ns.b` at `<wh>/ns/b`, one row each, both trees aged 10 days, and an aged orphan in `<wh>/ns/b/data` and another in `<wh>/ns/b/data/sub`. `CALL m1.system.remove_orphan_files(table => 'ns.a', location => …)` with `<wh>/ns/b/data` and with `<wh>/ns/b/data/sub`, each in the bare, `file:///` and `file:/` spellings, and the `<wh>/ns/b/data` call with `file_list_view => 'v'`, refuse. The complete Plan message says the path lies inside `<wh>/ns/b` (in the scan's spelling), whose metadata directory holds `m2`'s first metadata file in sorted order, and names both uuids. Every file under `<wh>/ns/b` survives, and the ids stay `[1]` and `[2]`. | `crates/repark-spark/src/tests/call_orphan_ancestor.rs::call_orphan_ancestor_two_catalogs_sibling_data_dir_scan_refuses` | **PROVEN** | M-anc (the walk keeps only the own-location step, the layout-r7 behaviour): FAILED on `the CALL must refuse: ()`, so the unguarded sweep ran. That is the P1. |
| C-030 | Two sessions each register catalog `ice` on one warehouse, the first creates `ns.a` and the second `ns.b`. The same outcome as C-029. | `call_orphan_ancestor.rs::call_orphan_ancestor_two_sessions_sibling_data_dir_scan_refuses` | **PROVEN** | M-anc: FAILED on `the CALL must refuse: ()`. |
| C-031 | `metadata_probe_ancestors(scan, own)` gives `/w/ns/b/data`, `/w/ns/b`, `/w/ns`, `/w`, `/` for `/w/ns/b/data/x` outside own `/w/ns/a`; `s3://bkt/w/ns/b`, `s3://bkt/w/ns`, `s3://bkt/w`, `s3://bkt/` for `s3://bkt/w/ns/b/data`; keeps the `file:///` and `file:/` spellings up to `file:///` and `file:/`; resolves `/w/ns/a/../b/data/` lexically; stops at the own location, inclusive, for `/w/ns/a/data/x`, `file:///w/ns/a/data` and `s3://bkt/w/a/data`; and gives nothing when the scan equals the own location in another spelling. | `call_orphan_ancestor.rs::call_orphan_ancestor_enumeration_walks_to_the_storage_root_or_the_own_location` | **PROVEN** | M-stop (the walk does not stop at the own location): FAILED, along with C-018's two pins. |
| C-032 | Near misses. With `m1.ns.a` and `m2.ns.b` as in C-029, a scan of `m1.ns.a`'s own `<wh>/ns/a/data`, and a scan of `<wh>/scratch/x` that no table lies above, pass `refuse_scan_over_foreign_metadata` in all three spellings. A bare-path `CALL m1.system.remove_orphan_files(table => 'ns.a', location => …)` then lists and deletes the aged file. Every file under `<wh>/ns/b` survives, and the ids stay `[1]` and `[2]`. C-018 (a table located inside another table sweeps its own `data/`), C-026 and C-027 stay green. | `call_orphan_ancestor.rs::call_orphan_ancestor_own_data_dir_scan_beside_another_catalog_deletes_the_orphan`, `::call_orphan_ancestor_scan_with_no_table_above_it_is_swept` | **PROVEN** | M-whole-anc (the walk lists each whole ancestor, not its `metadata/`): the scratch pin FAILED, and so did C-027. M-whole-anc with M-stop: the own-`data/` pin FAILED, with 8 other `call_orphan` pins. A `file://` location lists nothing on the fork's local listing (ORPHAN-3 row, unchanged), so the `CALL` runs on the bare path only. |
| C-033 | V-002 (layout-r8). `python/repark/tests/test_profiles1_table_properties.py::_data_files` reads `<wh>/ns/<table>`, asserts that it is the only directory of that name anywhere under the warehouse, and asserts that `<wh>/repark_ctas` does not exist. That is at least as strong as the old `assert len(roots) == 1` over `repark_ctas/mem/ns`. | `test_profiles1_table_properties.py::test_bogus_distribution_mode_refuses_at_write`, `::test_target_file_size_applies_at_table` | **PROVEN** | `2 passed`. A one-off call of `_data_files` on a warehouse with a second directory named `t` at `x/t`, at `repark_ctas/mem/ns/t`, or at `ns/t/data/t` raised `AssertionError` each time; the clean layout passed. |
| C-034 | Class A sweep (layout-r8): a scan path inside a table nested in the swept table's own location. Host `a.t` at `<wh>/a/t` and `o.x` created with `LOCATION '<wh>/a/t/x'`, one row each, all files aged 10 days, an aged orphan in `<wh>/a/t/x/data`. Sweeping `a.t` with `<wh>/a/t/x/data` in all three spellings refuses with the complete ancestor message naming `<wh>/a/t/x` (in the scan's spelling) and `o.x`'s first metadata file. Sweeping `a.t` with `<wh>/a/t/x` refuses too: in one catalog with the catalog walk's "holds table `ice.o.x`" message, and with `o.x` in a second catalog `m2` with rule (a)'s "holds `<file>`" message. Every file under `<wh>/a/t/x` survives, and the ids stay `[1]` and `[2]`. Before layout-r8 the `data/` scan passed every guard even in one catalog: the catalog walk skips a scan inside the own location unless the other table's location equals the own, and neither the scan path nor `<own>/metadata/` holds `o.x`'s metadata. | `call_orphan_ancestor.rs::call_orphan_ancestor_same_catalog_scan_of_a_table_nested_in_the_own_location_refuses`, `::call_orphan_ancestor_other_catalog_scan_of_a_table_nested_in_the_own_location_refuses` | **PROVEN** | M-anc: both FAILED on `the CALL must refuse: ()`, so the unguarded sweep ran. |
...
COVERAGE_ATTESTATION:
  ...
  categories:
    ...
    - id: AT-3
      status: ATTACKED
      evidence: C-020 refuses unreadable metadata, and each refusal row (C-012 to C-014, C-020, C-023 to C-025, C-029, C-030, C-034) states which files and rows survive the refused call.
      artifacts: [C-012..C-014, C-020]
    - id: AT-4
      status: ATTACKED
      evidence: C-012, C-013, C-023, C-024, C-029 and C-030 cover two catalogs and two sessions on one warehouse, sharing one table directory or holding sibling ones.
      artifacts: [C-012, C-013, C-023, C-024, C-029, C-030]
    - id: AT-5
      status: ATTACKED
    ...
    - id: AT-7
      status: N/A
      justification: No unbounded resource claim. Per CALL the probe lists the scan path at most once, then the `metadata/` directory of the ancestors `metadata_probe_ancestors` returns (at most the path depth) up to the first refusal, filters every listed entry by name, data files included, and reads each unlogged *.metadata.json it keeps until the first foreign one; a file reached both from the scan path and from an ancestor probe can be read twice.
    - id: AT-8
      status: ATTACKED
    ...
    - id: AT-10
      status: ATTACKED
      evidence: At d1f8fa9b, M-a 3, M-b 1, M-uuid 1, M-filter 35, M-naive 33, M-unless 2, M-norm 2, M-open 1, L-wire 3, L-ns 5, and L-off 5 pins failed. At layout-r7, M-root 2, M-share 1, M-root with M-share 3, M-whole 1, M-over 8, and M-reason 1 pins failed; the layout-r7 worker also reported M-fallback-inside, which the logged run did not repeat (C-028). At layout-r8, M-anc 4, M-stop 3, M-whole-anc 2, M-whole-anc with M-stop 9, M-ns 1, and M-cat 1 pins failed.
...
- Inferred, not measured (the 24-hour floor stops a fresh run): `remove_orphan_files` on `m1.ns.t`
  ...
  that sweep listed `repark_ctas/m1/ns/t` only, and `m2`'s table sat in `repark_ctas/m2/ns/t`.
...
Layout-r8 (V-001 to V-005 of critic r2), 2026-09-23:

- `cargo test -p repark-spark --lib call_orphan` → `64 passed; 0 failed`
- `cargo test -p repark-spark --lib mem_layout` → `9 passed; 0 failed`
- `pytest python/repark/tests/test_profiles1_table_properties.py python/repark-parity/tests/test_cap_1_source_file_line_cap.py` → green (tails in the round's hand-back)
- Mutation table: `/tmp/xo-xo-opus60/wo/mutr8/results.md`, with each mutation's diff and log beside it.
