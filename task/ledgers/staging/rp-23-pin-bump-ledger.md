...
| C-003 | Family C is fallout, not regression: s2/s7 MoR MERGEs still write ≥ Spark's floor of 2 delete files with the recorded kinds, the concurrency cap still moves the count down, s8/s9 COW MERGEs still write zero delete files; the only golden movement is the C-001 codec row. A measured count below the floor or a capped count above default is a STOP with evidence, never a masked edit. | `/tmp/probe_del.py` raw DEL counts; green runs of the six MERGE tests. | **OPEN** | Measured on the new pin: s2 raw 3 PARQUET / capped 2; s7 raw 3 PUFFIN / capped 2; s8/s9 raw 0 / capped 0. The `-vv` diff on s2 shows the codec row as the sole probe movement (`DEL [[1, 'PARQUET']]` identical both sides). |
