...
| C-011 | `degrees` / `radians` compute on a Rust kernel with thin facade binds (owner Q-15a-1 via G-2 R-7): new `call_scalar` dispatch arms reuse the engine's `degrees` / `radians` scalar UDFs; the facade wrappers are `_scalar` binds over a `double` cast keeping the `DEGREES(x)` / `RADIANS(x)` display names, the `toDegrees` / `toRadians` warnings, and every existing `fnp_alias_1_spark_oracle.json` pin green; `_rescaled` and the factor constants are deleted. | `test_fnp_alias_1.py` (55 pins, values/names/warnings/joins). | **PROVEN** | Superseded in round 3 (run 16a): the arms rebind to the Spark-exact `repark_functions::spark_degrees` UDFs and the Python cast is dropped (PYPERF-001); the DataFusion-UDF reuse described here no longer holds. The evidence below (55 alias pins, 4015-value sweep at 0 mismatches) was re-verified on the Spark-exact kernel. pins: fnp-bitmap-facade-1/C-011 |
...
| C-017 | Every `DEGI-degrees-*` / `DEGI-radians-*` answer shape matches on its door and ANSI setting: infinities and NaN by class, hex refusal/NULL, neg-zero, numeric spellings; values, Arrow types and default names. The L-010 `'Inf'`/`'inf'` shapes already matched and are pinned against regression. | `test_fnp_degrees_rust_1.py::test_degi_matches_oracle` + `::test_degi_nonansi_matches_oracle`. | **PROVEN** | Green at first run except the 16 suffix shapes (C-018). `DEGI-cast-*` cells are not pinned here: Repark `CAST('1d'/'1f' AS DOUBLE)` refuses where Spark answers 1.0 — run-16c divergence, handed off below, no code. pins: fnp-bitmap-facade-1/C-017 |
| C-018 | The degrees/radians kernel accepts one trailing Java float suffix after trimming when the remainder is a decimal floating literal that already parses (`'1d'`, `'1.5F'`, `'1e2d'` answer on both doors, both ANSI); suffixes after `Infinity`/lowercase names/hex keep today's refuse/NULL. The shared SQL `CAST` kernel is untouched. | `test_fnp_degrees_rust_1.py` suffix pins (16 shapes) + `spark_degrees.rs::degrees_accepts_java_float_suffixes_and_keeps_hex_refused`. | **PROVEN** | Red first: exactly the 16 suffix shapes failed pre-change with `CAST_INVALID_INPUT` on `'1d'` (0 non-suffix failures); 160/160 function cells green after. Rust unit tests cover `'1d'`, `'1.5F'`, `'1e2d'`, `'Infinityd'`/`'infd'`/`'0x10'`/`'1dd'`/`'1_0f'` refusal and `' 1f '` trim-then-suffix. pins: fnp-bitmap-facade-1/C-018 |

VERDICT: 18 clauses, 18 PROVEN, 0 OPEN, 0 REJECTED.

## Round 4 rulings and findings — dispositions (run 16a, 14:00)

| Ruling | Content | Disposition |
|---|---|---|
| L-010 REFUTED (G-2) | The critic claimed `'Inf'`/`'inf'` must refuse per Java `parseDouble`. Live Spark 4.1.2 answers ±Infinity for `'Inf'`, `'inf'`, `'-Inf'`, `'+inf'`, `' Inf '`, `'Infinity'`, `'-infinity'`, `'INFINITY'` and NaN for `'NaN'`/`'nan'`/`'NAN'`, both doors, both ANSI, in CAST too. | No code change: RePark already matches. Pinned against regression by the DEGI answer shapes, C-017. |
| ORACLE-FOUND P2 (G-2) | Spark accepts a Java floating-literal type suffix (`'1d'`, `'1f'` answer 57.29577951308232); RePark raised on 4 of 40 ANSI-on shapes. | Fixed in the degrees/radians kernel only, C-018. |
| L-011 P3 (G-2) | Stray `FU-and-float*` rows in the DEG oracle; 52 unused DEG cells. | The strays are moved out (`FU-and-float` already lives in the bitmap facade fixture). Nonansi answers (36 cases), SQL-nonansi refusals (8) and the 8 alias malformed cells are parametrized. Coverage now: 144/144 DEG cells and 160/160 DEGI function cells pinned; the 40 `DEGI-cast-*` cells are excluded by design (run-16c hand-off). |
| CAST hand-off (G-2) | Repark `CAST('1d' AS DOUBLE)` raises `spark_float_stringify` `CAST_INVALID_INPUT` where Spark answers 1.0 (`CAST('Inf')` already answers `inf`). | Run 16c's divergence. Noted here, no code: the degrees kernel does not route through CAST. |
| Q-15c-3 (owner) | Builder-config ANSI-off divergence shape if needed. | Not needed again: the DEGI `-nonansi` pins assert real kernel behavior on builder-ansi-off sessions. |
...
| `python/repark/tests/fnp_degrees_rust_1_spark_oracle.json` (round 4) | 200 `DEGI-*` cells appended verbatim; the 2 stray `FU-and-float*` rows moved out. |
| `python/repark/tests/test_fnp_degrees_rust_1.py` (round 4) | DEGI pins on door and ANSI; nonansi answers, SQL-nonansi refusals, alias malformed splits. |
| `crates/repark-functions/src/spark_degrees.rs` + `src/map.md` (round 4) | The Java float-suffix rule with unit tests; the suffix rule stated in the map. |
