...
| C-005 | V3-05: multi-argument `bucket(4, id, name)` refusal measured on merged main and matched against the registry row `V3-MULTIARG-1` (PR #700, merged 2026-09-18 14:17 EDT as `04f669da`). | Refusal class + message at CREATE and ALTER; single-arg control succeeds; the merged row quotes the same text. | **PROVEN** | Probe `sweep1b_a.py item5`: CTAS `PARTITIONED BY (bucket(4, id, name))` → `ERR AnalysisException: … CTAS PARTITIONED BY \`bucket(…)\` expects (numBuckets, column), got 3 argument(s): [4, id, name]`; `ADD PARTITION FIELD bucket(4, id, name)` refuses the same way; single-arg `bucket(4, name)` control commits. At write time #700 was unmerged and this ledger held the state; the orchestrator merged main after #700 landed (run 22b) and checked the row: `### V3-MULTIARG-1` present once, and it quotes the measured `got 3 argument(s): [4, id, name]` text. No second row written. |
