...
| C-009 | No regression: the session test files stay green and the API inventory count is updated for the 19 names. | `test_session*.py`; `test_ex_0_example_coverage.py`; `check_lib_py.py`. | **PROVEN** | `test_session.py` + `test_session_range.py` + `test_session_surface_1.py` 91 passed; `test_session_config_knobs.py` + `test_session_sources.py` 82 passed; `len(rows)` moved 930 → 949 (948 → 967 after the types-bases rebase). `session_core.py` holds its exact 2304 baseline (five one-line signature joins funded the import plus 19 bindings). Three `test_session_timezone_parity` cells fail on `repark._native` lacking `default_timestamp_descriptor` — a stale vendored native module in this Python-only clone, reproduced on paths this diff never touches; recorded as out-of-scope. pins: session-surface-1/C-009 |
| C-010 | Critic round 1 (L-001..L-009) is remediated with rulings R-4/R-5/R-6 applied: the artifact dir dies with the session on every stop path; `interruptOperation` validates as `Try(toLong)`; the no-path pyfile call raises Spark's own `TypeError`; `DUPLICATED_ARTIFACT` and `render`'s `type` check order are pinned; `tvf.json_tuple` converts string-literal Columns to `str` fields; the P3s are documented, not changed. | The new pins in `test_session_surface_1.py`. | **PROVEN** | Red-first on the base: `test_artifact_dir_dies_with_the_session` (`assert dir_a not in sys.path` — the dir and entry leaked), `test_interrupt_operation_java_long_validation` three cells (`"١٢٣"`/`"２"`/`"9223372036854775808"` returned `[]` under the Unicode `\d` regex), `test_interrupt_operation_non_str` (raised `IllegalArgumentException`, not `NOT_STR`), `test_add_artifact_pyfile_without_path_raises_type_error` (returned `None` and prepended `sys.path`), `test_profile_render_type_check_precedes_the_refusal` (`NOT_IMPLEMENTED` before `VALUE_NOT_ALLOWED`), `test_tvf_json_tuple_field_columns` (a non-literal Column field rode the kernel refusal instead of `NOT_STR`). All green after; the `hasattr` and `DUPLICATED_ARTIFACT` pins arrived green (no behaviour change / already implemented). L-008 (explode nullability invisible to `simpleString` — pre-existing `F.explode` nullable=True vs cell's `nullable=false`) and L-009 (the oracle's `F.array().cast("array<int>")` cannot parse; the pin uses a `slice` stand-in with identical columns/simpleString/rows) are documented here, unchanged. `sys.modules` note (L-001): `stop()` deliberately does not evict modules imported from an artifact dir — Spark keeps loaded modules too, and the pin proves it. `session_core.py` stays exactly 2304 (the `create_dataframe` signature join funds the four-line `stop()` cleanup block). pins: session-surface-1/C-010 |
