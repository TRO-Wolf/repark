...
**What Spark does (measured 2026-09-24; every measurement is committed as `python/repark/tests/u8_write_sql_spark_oracle.json` (sha256 4ad643bb…, generator `target/probe-u8-r2fix/build_oracle.py`), keys `pr1/…`; originally `target/probe-u8-pr1/spark.json`, `spark2.json`,
...
VERDICT (2026-09-25, critic r2 remediation): 21 clauses, 21 PROVEN, 0 OPEN, 0 REJECTED.
...
(`target/probe-u8-pr1/mut/M*.txt`; round 2 in `target/probe-u8-r1fix/mut/`, round 3 in `target/probe-u8-r2fix/mut/`).
...
| R-2 | The untranslatable-predicate refusal quotes the statement's spelling: `(id + 1) = 3` reads `id + 1 = 3`, `CAST(id AS string)` reads `CAST(id AS STRING)`, `UPPER(cat)` reads as written, and Spark's `LIKE '%x' ESCAPE '\'` reads `LIKE '%x'` (the six oracle keys `r2fix/Q-…upper…`, `r2fix/Q-and-or-arith`, `r2fix/Q-like-suffix-and` compare class, condition and SQLSTATE, not text). A `TIMESTAMP` literal in the predicate refuses as untranslatable (probe RW2-ts); Spark converts it in the session zone. A `DATE` literal or date string converts. |
