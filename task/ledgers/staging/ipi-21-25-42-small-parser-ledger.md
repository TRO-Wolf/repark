...
| C-010 | `gc.enabled=false` refuses the purge with Spark's text, sweeps nothing and drops nothing; per-file delete failures are collected by the sweep and logged at the door — one `tracing::warn` naming the table and the count — never surfaced in the SQL or DataFrame result (Java's log-only suppression). | The property pin on both values, and a Rust pin that fails one path's delete and still sees the table gone. | PROVEN (door warn implemented, not test-observed) | `crates/repark-spark/src/tests/purge.rs::{purge_refuses_when_gc_is_disabled_and_sweeps_nothing,purge_collects_delete_failures_and_the_drop_still_runs}`; `python/repark/tests/test_ice_small_parser_1.py::{test_drop_table_purge_gc_disabled_refuses,test_drop_table_purge_gc_enabled_true_still_purges}` — these pin the gc-refusal on both values and the sweep's failure-collection with the drop still running; the fail-path pin drives `DeleteReachableFiles::delete_with`, not `execute_drop_table`, and installs no tracing subscriber, so the door warn (implemented at `crates/repark-spark/src/namespace_ddl.rs:47-53`) is observed by no test — the door-level deleter-Err mutant is residue 5. |
...
COVERAGE_ATTESTATION:
  ...
  categories:
    ...
    - id: AT-3
      status: ATTACKED
      evidence: Per-file delete failures are collected by the sweep and the drop still completes — pinned by driving one path's delete to fail at the DeleteReachableFiles level; the door's single tracing::warn naming the table and the count is implemented at namespace_ddl.rs:47-53 but no test observes the log line (the door-level deleter-Err mutant is accepted residue 5), matching Java's log-only posture; gc.disabled refuses before any sweep and drops nothing; missing-target refusals carry Spark's TABLE_OR_VIEW_NOT_FOUND with SQLSTATE 42P01.
    ...
    - id: AT-9
      status: ATTACKED
      evidence: The one diagnosable failure path — a per-file delete failure during the sweep — surfaces as exactly one tracing::warn naming the table and the failure count while the SQL result stays clean, matching Java's log-only suppression: the warn is implemented at namespace_ddl.rs:47-53 and the collection-and-clean-result half is pinned at the DeleteReachableFiles level, but no test observes the log line itself (the door-level deleter-Err mutant is accepted residue 5); the refusal paths carry error class and SQLSTATE a caller can alarm on.
