...
| C-006 | `dataframe_writers_fill`: `writeTo(t).append()`, `saveAsTable(append)`, and `insertInto` with `c` missing fill from `write_default` instead of refusing. | Offline + live pins green. | **PROVEN** | Offline + live green (same runs as C-004); `check_lib_py.py` ratchets `writer_readwriter.py` 1101 → 1095. |
...
| C-011 | `v2_unchanged`: a v2 table behaves exactly as before. | Pin green. | **OPEN** | Offline green (same run as C-004); PROVEN recorded after the step-7 suite rerun confirms no drift. |
| C-012 | `write_default_differs`: write-default 7 with initial-default 5 (Java API `updateColumnDefault` / `UpdateSchema`) — old rows read 5, new omitted writes fill 7. | Pin green. | **OPEN** | Offline green (same run as C-004); PROVEN recorded after the step-7 suite rerun confirms no drift. |
...
| C-014 | `gates_green`: the new test file offline and live, `cargo test -p repark-iceberg --lib`, `cargo test -p repark-spark --lib`, `make verify`, the whole facade suite, and the whole parity suite are green on the release native. | Counts in this ledger. | **OPEN** | Pending step 7. |

VERDICT: 14 clauses, 11 PROVEN, 3 OPEN, 0 REJECTED.
