...
`execution.enable_ansi_mode` — repark never sets it, so the door *wraps* on integer-min
(`spark.sql abs(-2147483648)` → `-2147483648`) while the facade's core `checked_abs`
raises (Spark ANSI-on answer, same as the old `0 - c` path). The divergence is
pre-existing — the facade previously composed a CASE, so the ratchet could not see it —
and `("abs", Kernel(1), …)` is added to `EXPECTED_DIVERGENCES` with the reason
(table 21 → 22). Wiring `spark.sql.ansi.enabled` to `execution.enable_ansi_mode` is a
door-side fix for its own card. `cbrt` and `nullif` resolve identically on both doors
(datafusion-spark ships neither kernel, so the door falls back to the same core UDFs).
...
| `DataFrame.replace` dict loop | `dataframe/core.py:2752` | 2× per mapping entry | 28 MB @12-entry dict, ~×3.3/entry (297 MB @16) | unchanged | audited only — the file is fenced to another lane; a dict of N entries nests `expression` 2^N → needs its own card |
