...
| C-006 | **F-16 measured** (transferred from #254 C-009). MW-7's 1e7×50 MERGE-then-maintain sequence on a merge-on-read table ends at zero delete files and zero delete records with the default `delete-ratio-threshold` (0.3); the MW-7 pin flips from "documents the gap" to "asserts zero"; the maintenance runbook drops its residual-delete caveat. | The 2,500-row reproduction first, the 1e7 run once; the pin; the runbook diff. | **PROVEN** | Measured red, not a false zero. 2,500-row pin still holds (`test_delete_laden_in_band_file_survives_the_runbook`). 1e7×50 MOR (34.6 min): after maintenance **8 delete files / 10,000,000 delete records** (400 → 8 at rewrite_position_delete_files; rewrite_data_files leaves 8). Pin retains the gap. Runbook RDF-1 caveat stays. Citation: `python/repark/tests/test_mw7_scale_smoke.py`. |
...
```yaml
SELF_LOGIC_REVIEW:
  id: SLR-C-004-CLASSIFY
  agent: orchestrator
  action: classify C-004 C7 as a measured red cell that stays refused and is filed as a fork finding
  charter_trace: C-004
  preconditions:
    - matrix pins: SATISFIED (cells 1-6 green three doors; cell 7 first DELETE Spark-equal; cell 7 second DELETE refused; cell 8 live-DV UPDATE refused)
    - Spark transcript: SATISFIED (ledger §10)
    - charter or-path: SATISFIED (a red cell stays refused and is filed as a fork or engine finding)
    - ownership: SATISFIED (plain WHERE DELETE is fork iceberg-datafusion FirstRowIdPolicy::Suppress)
  success_condition: C-004 is PROVEN; F-rp3-c7 names the fork owner; V3-3 is not assigned the C7 defect
  step_risks:
    - classifying a green lineage implementation: HANDLED(second DELETE remains V3-COW-1)
    - routing C7 to V3-3: HANDLED(finding names the fork StreamingDataFileWriter path)
  contingencies:
    - grammar lacks a C-004 pin: EXECUTABLE(citations already in v3_cow.rs, v3e3.rs, test_v3_cow_dml.py)
  tripwire_scan: CLEAN
  uncertainty: NONE
  verdict: PROCEED
  escalation: —
...
FINDING:
  ...
  category: AT-6
  clause: [C-004]
  claim: A second copy-on-write DELETE after an overwrite snapshot reassigns `_row_id` and `next-row-id` in fork iceberg-datafusion (`FirstRowIdPolicy::Suppress`); Spark preserves lineage (next-row-id stays 5, added-rows 0).
  evidence: ledger §10 PySpark 4.1.2 transcript vs unguarded RePark next-row-id 6 / (5, 1)
  disposition: REMEDIATED (refuse pins in v3_cow.rs / test_v3_cow_dml.py; fork owner filed F-rp3-c7)
```
