...
| C-015 | String leaves trim exactly Spark's set: code points <= U+0020 and U+007F (DEL); U+0085 is not trimmed (ANSI `CAST_INVALID_INPUT`, legacy NULL). | Round-4 cells `del_prefix_leaf_*`, `tab_prefix_leaf_*`, `del_bool_leaf_*`, `c1_prefix_leaf_*` on the facade; Rust `ascii_controls_and_del_trim_but_c1_controls_do_not`. | PROVEN | DEL cells red on the round-3 tree (`bf8d3353`), green on the round-4 fix; tab and U+0085 cells green both ways, pinned against regression. |
| C-016 | Residue L-008 (ruling Q-23b-9): `try_cast(map(128, 'a') AS MAP<TINYINT, STRING>)` — Spark stores a map with one NULL key (size 1, keys [NULL], values ['a']) that PySpark cannot collect; an Arrow map cannot hold a NULL key, so RePark refuses loud (`a map key cast produced NULL`) in both modes. | Round-4 cells `try_cast_overflow_key_*` on both doors; Rust `try_cast_key_overflow_refuses_loudly`. | PROVEN | Refusal pinned on both doors and in Rust; a residue, not parity. |
