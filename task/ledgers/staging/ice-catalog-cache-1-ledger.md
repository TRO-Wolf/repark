...
| C-009 | The bench exposes evictions: the per-query JSON's `metadata_cache` object carries `evictions` and the markdown table's cache cell reads `hits/misses/evictions`. | `benches/ice_read_perf/pins.rs`. | **OPEN** | Next commit of this round. |
