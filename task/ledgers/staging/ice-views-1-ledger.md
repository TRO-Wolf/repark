...
| C-005 | On a viewless catalog, CREATE/CREATE OR REPLACE refuse with Spark's `Creating/Replacing a view is not supported by catalog: <name>`, SHOW VIEWS returns empty, DROP VIEW reports VIEW_NOT_FOUND — all driven off fork `FeatureUnsupported`, never a `view_exists` capability probe. | `ViewlessCatalog` Rust tests (Python cannot construct a viewless catalog offline: registration eagerly lists namespaces). | PROVEN | `create_view_on_a_viewless_catalog_reports_creating_unsupported`, `replace_view_on_a_viewless_catalog_reports_replacing_unsupported`, `list_views_on_a_viewless_catalog_returns_empty`, `drop_view_on_a_viewless_catalog_reports_view_not_found`. |
...
| C-007 | Every reachable D-9 row carries Spark's message, SQLSTATE and condition: duplicate CREATE (VIEW_ALREADY_EXISTS/42P07), DROP VIEW missing (VIEW_NOT_FOUND/42P01), DROP TABLE over a view (TABLE_OR_VIEW_NOT_FOUND/42P01, `drop_table` never called), DROP VIEW over a table (VIEW_NOT_FOUND/42P01), INSERT INTO a view (TABLE_OR_VIEW_NOT_FOUND/42P01, never writes). | Battery contract test plus catalogue unit tests. | PROVEN | `test_view_error_contract` asserts all three fields per row; `spark_error.rs` condition tests; `execute_drop_table` checks `is_view` before `drop_table`; INSERT/DELETE/UPDATE arms call `refuse_view_write_target`; TRUNCATE keeps its EXPECT_TABLE_NOT_VIEW loud refusal. |
...
| C-012 | A view over a view reads through; 100 nested views read and the 101st raises typed `[VIEW_NESTED_DEPTH_LIMIT]` on both the read door and creation validation. | Battery nesting pins. | PROVEN | `test_view_over_view`; `test_nested_view_depth_guard` (w50/w99 read, SELECT w100 and CREATE w101 raise). |
...
| C-017 | PR2: DESCRIBE / DESCRIBE EXTENDED / SHOW CREATE TABLE / SHOW TBLPROPERTIES on views and ALTER VIEW SET/UNSET/RENAME answer packets M-3..M-6. | Packet §7 V-* pins. | OPEN | Out of WO-1; `view_ddl/describe.rs` is a stub. What exact row shapes land per statement? |
