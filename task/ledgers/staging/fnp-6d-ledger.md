...
|---|---|---|---|---|
| C-001 | `bitmap_construct_agg(bitmap_bit_position(x))` over `VALUES (1),(2),(3),(32767),(NULL)` answers the F6D-construct bitmap: 4096-byte BINARY, first byte `0x07`, last byte `0x40`, count 4, NULL ignored. | Rust `construct_agg_sets_bits_zero_one_two_and_last_and_ignores_null`; Python `test_construct_agg_sets_bits_and_ignores_null`. | OPEN | Red-first on the base tree: planning `Invalid function 'bitmap_construct_agg'`. |
| C-002 | Grouped construct then `bitmap_or_agg` / `bitmap_and_agg` answers F6D-or-and: OR popcount 3, AND popcount 1, both non-null bigint. | Rust `or_and_agg_fold_grouped_bitmaps`; Python `test_or_and_agg_fold_grouped_bitmaps`. | OPEN | Same red: `Invalid function 'bitmap_construct_agg'`. |
| C-003 | Empty input (F6D-empty): construct and or answer 4096 zero bytes; and answers 4096 `0xFF` bytes; all non-null BINARY. | Rust `empty_input_construct_and_or_are_zeros_and_is_ones`; Python `test_empty_input_identities_are_non_null_binary`. | OPEN | Same red. |
| C-004 | `length(bitmap_and_agg(b))` of one constructed bitmap is 4096, integer, non-null (F6D-and-empty-type). | Rust `and_agg_of_one_bitmap_has_length_4096`; Python `test_and_agg_length_is_4096_int`. | OPEN | Same red. |
| C-005 | Result type of each of the three aggregates is BINARY non-null. | The binary-cell assertions in the C-001 and C-003 pins. | OPEN | Same red. |
| C-006 | A sliding frame over each of the three names refuses loudly (WIN-SLIDE: message contains `retract_batch` or `sliding`). | Rust `sliding_frame_refuses_loudly`; Python `test_sliding_frame_refuses_loudly`. | OPEN | Red-first: base tree raises `Invalid function`, not the sliding refusal. |
| C-007 | The three UDAFs register through `aggregate::functions()` so the SQL door resolves them; `lib.rs` registration is untouched beyond `mod bitmap_agg`. | `aggregate.rs` extend + `bitmap_agg::functions()` returning the three names. | OPEN | Step 1 wires an empty `functions()` so the pin is red. |
| C-008 | Registry §7 row `FNP-6D` is FIXED 2026-09-15; `docs/design/spark-function-parity.md` FNP-6d line is delivered. Sliding-window shape is DECLARED under that row. | The two doc edits. | OPEN | Step 3. |
| C-009 | Every new or moved file is listed in its directory `map.md` in the same commit. | `src/map.md`, crate `map.md`, `python/repark/tests/map.md`, `task/ledgers/staging/map.md`. | OPEN | Step 1 lists the pin files; step 2/3 keep lockstep. |
| C-010 | No new dependency; accumulator state is `[u8; 4096]`. | Diff of `Cargo.toml` / `Cargo.lock` empty of this unit; the state field in `bitmap_agg.rs`. | OPEN | Step 1 adds no deps; step 2 lands the array. |
