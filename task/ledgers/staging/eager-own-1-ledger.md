...
| C-003 | Reuse one eager frame across actions: no new materialization; values and Arrow types remain unchanged (review validation row 2). | `test_cache_view_reuse_across_actions` — `collect`, `count`, `filter().collect`, `to_arrow` on one eager frame. | **PROVEN** | Guard pin — green on base AND on the fix (one registration, stable values and Arrow schema `id: int64 / v: int64`); it pins the invariant that must not change, so no red exists for this row. |
...
the five guard pins (`test_cache_view_reuse_across_actions`,
`test_unpersist_twice_and_clear_cache_twice`, the wrapper-hold arm of C-006 on
