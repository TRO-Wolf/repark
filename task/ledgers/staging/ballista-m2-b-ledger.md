...
| C-002 | The spec carries the predicate as a `datafusion-proto` `Expr`. Conversion covers And/Or/Not, unary IsNull/NotNull/IsNan/NotNan, binary comparisons including StartsWith/NotStartsWith, In/NotIn, and Datum → ScalarValue for bool/int/long/float/double/date/time/timestamp/timestamptz/string/uuid/binary/fixed/decimal. An inexpressible shape refuses loud at encode naming the shape. Wire `MAGIC`/version bumped so an old-format payload refuses on decode. Each wire filter item is explicitly tagged (0 = SQL, 1 = Expr); encode writes both lists; scan applies both; unknown tag refuses loud. Timestamptz uses `UTC_TIME_ZONE`. StartsWith prefixes carrying `%`, `_` or `\` refuse. | `predicate_expr::tests::always_true_predicate_shape_refuses_loud`, `always_false_predicate_shape_refuses_loud`, `timestamptz_datum_converts_to_utc_timestamp_microsecond`, `starts_with_percent_prefix_refuses_loud`, `starts_with_underscore_prefix_refuses_loud`, `starts_with_backslash_prefix_refuses_loud`; `iceberg_scan_spec_old_format_payload_refuses_loud`, `iceberg_scan_spec_sql_and_expr_filters_round_trip_together`, `iceberg_scan_spec_sql_and_expr_filters_both_apply_on_scan`, `iceberg_scan_spec_unknown_filter_tag_refuses_loud` in `tests/iceberg_scan.rs`. | **PROVEN** | `src/predicate_expr.rs` converts the listed shapes; AlwaysTrue/AlwaysFalse refuse naming the shape. `RPIC` version 2 (not bumped again: v2 has not shipped). Filter items are `[tag][payload]`; SQL+Expr round-trip and both apply on scan (`id >= 1` SQL ∧ `id <= 3` Expr → rows 1,2,3); tag 99 refuses naming the tag. Timestamptz → `TimestampMicrosecond(_, Some("UTC"))`. StartsWith `%`/`_`/`\` refuse. pins: ballista-m2-b/C-002 |
...
- Decode still rebuilds through `IcebergScanSpec::scan` on the codec-carried
  ...
  spec's frozen value.
- `RPIC` version is 2 (unchanged in the audit round: v2 has not shipped). Each
  filter item on the wire is `[tag][payload]` with tag 0 = SQL text and tag 1 =
  datafusion-proto `Expr`. Encode writes both lists; decode refuses an unknown
  tag by name; scan applies SQL then Expr.

## Audit round (2026-09-11)

F-1 wire ambiguity. Red on the step-1 commit before the tagged encode:
`iceberg_scan_spec_sql_and_expr_filters_round_trip_together` — `SQL filters dropped
on the wire: [] != ["id >= 1"]`;
`iceberg_scan_spec_sql_and_expr_filters_both_apply_on_scan` — `got ["0", "1", "2", "3"]`
(Expr only); `iceberg_scan_spec_unknown_filter_tag_refuses_loud` — untagged payload
made the patch hit a length prefix (`bytes length 1660944391 exceeds 1048576`).
Green after explicit tag bytes: SQL+Expr round-trip, scan rows `["1", "2", "3"]`,
tag 99 refuses `filter tag 99 is unknown`.

F-2 type fidelity. Red: `timestamptz zone must be UTC, got +00:00`. Green after
`iceberg::arrow::UTC_TIME_ZONE`.

F-3 LIKE escape. Red: `StartsWith prefix "a\\b" must refuse, got name LIKE Utf8("a\b%")`.
`%` and `_` already refused on that tree. Green after `\` joins the refusal.
...
| `cargo test -p repark-distributed --features cluster` | 37 passed; 0 failed (lib 6, cluster_two_executors 4, codec 11, iceberg_scan 8, local_executor 3, multi_stage 5) |
...
COVERAGE_ATTESTATION:
  ...
  categories:
    ...
    - id: AT-3
      status: ATTACKED
      evidence: Failure paths pinned loud — vanilla decode refuses naming the session catalog, vanilla encode refuses, unowned-node encode refuses with Unsupported plan node, AlwaysTrue/AlwaysFalse refuse naming the shape, v1 payload refuses naming the version, unknown filter tag refuses naming the tag, StartsWith LIKE-wildcard/escape prefixes refuse, decode refuses when the rebuilt resolved_snapshot_id differs from the spec's frozen value.
    ...
    - id: AT-6
      status: ATTACKED
      evidence: Wire format is RPIC v2 (not bumped again — v2 has not shipped). Each filter item is an explicit tag byte plus payload (0 = SQL, 1 = Expr); encode writes both lists; scan applies both; unknown tag refuses. A v1 version byte still refuses on decode. The scan's frozen resolved_snapshot_id is what travels and is re-verified on the rebuilt node via the typed accessor.
