...
- A column list that mixes `COMMENT` with another change (`data COMMENT 'a', cat TYPE STRING`) is accepted by Spark's `alterColumnSpecList` and refused here as not implemented (UnsupportedOperationException, named text). Loud.
