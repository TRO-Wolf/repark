...
- R-U5-SPEC-LIST-NOT-COMMENT-FIRST: a list whose first spec is not `COMMENT` (`cat TYPE STRING, data COMMENT 'm8'`, `id TYPE BIGINT, cat TYPE STRING`) returns OK in Spark (`mixcomment_type_first`, `typelist`). RePark leaks `SQL error: ParserError("Expected: ADD, RENAME, PARTITION, SWAP, DROP, REPLICA IDENTITY, SET, or SET TBLPROPERTIES after ALTER TABLE, found: data …")`, because the top-level TYPE route reads one spec. This is pre-existing on that route.
