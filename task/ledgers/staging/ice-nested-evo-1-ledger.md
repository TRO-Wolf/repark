...
- Non-Spark aliases that the shared `sql_type_to_iceberg_with_timestamp_type` lowering accepts (`TEXT`, `INT2`/`INT4`/`INT8`, `FLOAT4`/`FLOAT8`, `BOOL`, `NVARCHAR`, `CHARACTER VARYING`, `DOUBLE PRECISION`, `FLOAT(p,s)`, `TIMESTAMP(p) WITH TIME ZONE`) are unmeasured on the nested TYPE route. They lower to a Spark type and follow C-027. The same lowering serves CREATE TABLE and top-level ALTER, so this is pre-existing.
