...
| C-058 | A hive-style typed column in a column-def `CREATE TABLE … PARTITIONED BY (<name> <type> [NOT NULL] [COMMENT '<doc>'], …)` is appended to the table schema after the declared columns, in clause order, with the next field ids, and gets an identity partition field; `NOT NULL` makes it required and `COMMENT` lands the field doc. A table with no column list takes only the typed columns. `CREATE OR REPLACE` / `REPLACE TABLE` re-key it like a declared column (Spark's id 4 and spec field 1001). A typed column beside an untyped column or a transform raises ParseException `Operation not allowed: PARTITION BY: Cannot mix partition expressions and partition columns:\nExpressions: <e, …>\nColumns: <name type, …>.`; a typed name that repeats a declared or earlier typed column raises (compared case-insensitively under `spark.sql.caseSensitive=false`, the default, and exactly under `true`, where a case-only pair reaches the fork's lower-case-index refusal, R-U5-PR3-CASE-SENSITIVE) AnalysisException `Error during planning: [COLUMN_ALREADY_EXISTS] The column `<lower-cased name>` already exists. …SQLSTATE: 42711` (Spark's text after RePark's `Error during planning: ` prefix, R-U5-PR2B-PLANNING-PREFIX; pinned at `test_ice_ddl_alter_2.py::test_typed_partition_columns_become_identity_columns_like_spark`); neither creates a table. The options take `NOT NULL` then `COMMENT '<doc>'`, in that order, each at most once, as Spark's colDefinition grammar does: `COMMENT 'c' NOT NULL`, `NOT NULL NOT NULL` (also with a following `, q INT`) raise ParseException `[PARSE_SYNTAX_ERROR] Syntax error at or near 'NOT'. SQLSTATE: 42601`, and `COMMENT 'a' COMMENT 'b'`, `NOT NULL COMMENT 'a' COMMENT 'b'` raise `… near 'COMMENT'. SQLSTATE: 42601`, creating nothing (verifier r2 V-001; Spark 4.1.2 + Iceberg 1.11.0 measured 2026-09-25, `scratchpad/fx/sp.out`; red first: head 48c6b5c9 committed all five). The old `PARTITIONED BY typed column … is not supported` refusal is gone. `PARTITIONED BY (cat)` and `PARTITIONED BY (days(ts))` are unchanged. | Spark probe plus Rust and facade pins, cell replay. | PROVEN | `target/probe-u5-pr3/spark.out` (`c_*`), `spark2.out`; replay `target/probe-u5-pr3/replay/after.json` D-X-PARTITIONED-COLDEF EQUAL. Pins: `create_typed_partition.rs` (four tests), `test_ice_ddl_alter_2.py::test_typed_partition_columns_become_identity_columns_like_spark`, `…::test_typed_partition_column_refusals_match_spark`. Option order: `create_typed_partition.rs::typed_partition_column_options_beyond_not_null_and_comment_refuse_like_spark`, `test_ice_typed_partition_refusals.py::test_typed_partition_column_shapes_refuse_like_spark` (`comment-not-null` … `not-null-comment-twice`). |
| C-059 | On the Spark door, `ALTER TABLE <src> RENAME TO <target>` reads the whole target inside the source catalog, as Spark 4.1.2 does on InMemoryCatalog: one part keeps the source namespace, and two or more parts are `namespace…name`, so a catalog name in the target is a namespace level. When the source exists and the target namespace does not, the statement raises `Cannot rename <src ns>.<src> to <target>. Namespace does not exist: <target ns>` (Spark's NoSuchNamespaceException text; RePark's class is PySparkException `datafusion engine error: Execution error: …`, the Py4JJavaError precedent), e.g. `sc.ns.u` → `sc.ns`, `hc.ns.u` → `hc.ns`, `x.y.z.w` → `x.y.z`, and the source keeps its rows. When the nested namespace exists, the table moves into it. Two-part, one-part and other-namespace targets rename. An existing target raises AnalysisException `[TABLE_OR_VIEW_ALREADY_EXISTS] Cannot create table or view <ns>.<name> because it already exists.\nChoose a different name, … SQLSTATE: 42P07`. A `type=hadoop` catalog keeps `Cannot rename Hadoop tables` for every target. The cross-catalog refusal is gone; ice-catalog-session-1/C-022's cross-catalog half and dbt-1-adapter/C-001's `S-RENAME` three-part row are superseded. The ANSI door is unchanged. | Spark probe plus Rust and facade pins, cell replay. | PROVEN | `target/probe-u5-pr3/spark.out` (`sc_*`, `hc_*`); replay `target/probe-u5-pr3/replay-r2.json` against `/tmp/oc-worker/scoreboard/2026-09-25/out/spark-core.json`: D-RENAME-TABLE, D-RENAME-TABLE-SHORT and D-X-PARTITIONED-COLDEF EQUAL. Pins: `rename_target.rs` (five tests), `hadoop_rename.rs::hadoop_type_rename_refuses_every_target_shape`, `…::memory_type_rename_still_succeeds`, `use_ddl.rs::rename_three_part_dest_reads_the_catalog_as_a_namespace`, `test_ice_ddl_alter_2.py::test_rename_to_reads_the_target_inside_the_source_catalog`, `…::test_rename_to_an_existing_table_answers_spark_text`, `test_ice_catalog_session_1.py::test_rename_across_catalogs_reads_a_namespace_like_spark`, `test_statement_surface.py` `S-RENAME` / `R-RENAME-THREE-PART`, `repark-sql/tests/cross_door.rs::cross_door_alter_lands_the_same_evolved_schema` (per-door target). |
| C-060 | Since fork F-V1-REFS-1 (repin repark#836) the fork keeps `refs` on format v1 tables, so RePark commits every v1 ref write like Spark and the `refuse_ref_write_on_format_v1` kernel is removed with every call: CREATE BRANCH/TAG (with IF NOT EXISTS, OR REPLACE, REPLACE, AS OF VERSION, `main` with retention) on a seeded v1 table; `INSERT` into `t.branch_b1`; a session WAP branch write on a seeded and on an empty v1 table; `CALL fast_forward` (including to a new branch); `rewrite_data_files` with and without a branch; a branch on an empty v1 table commits with `refs` keys `[b1]` and no `main`. A tag on an empty v1 table refuses IllegalArgumentException `Cannot complete create or replace tag operation on <ns>.<table>, main has no snapshot`, the same text as v2. A write into a missing `t.branch_x` still answers `Cannot use branch (does not exist): x` first. | Spark probe plus Rust and facade pins, red-first; scoreboard replay. | PROVEN | Spark 4.1.2 + Iceberg 1.11.0 measured 2026-09-26 (`v1refs-spark-2026-09-26.json`, `wapv1-spark-2026-09-26.json`): seeded v1 branch/tag DDL ok with refs `[b1, main, t1]`; the branch write reads `[1, 2]` on the branch, `[1]` on main and on the tag; refs rows `[[b1, BRANCH], [main, BRANCH], [t1, TAG]]`; fast_forward returns `[main, previous, new]` with the metadata's ids and main then reads `[1, 2]`; b2 retention lands `refs.b2` `{type branch, min-snapshots-to-keep 2, max-ref-age-ms 604800000}`; an empty v1 branch commits with refs `[b1]` while the tag refuses the v2 text; the WAP write reads `[1, 9]` under the conf, `[1]` after unset, `[1, 9]` on `branch_wap1`, refs `[main, wap1]`, fast_forward to main then `[1, 9]`, and `rewrite_data_files` `[2, 1, …]`. Pins: `crates/repark-iceberg/src/tests/v1_ref_writes.rs::every_ref_write_on_a_v1_table_commits_and_survives_a_reload`, `crates/repark-spark/src/tests/v1_ref_writes.rs` (8 tests), `ref_branch_on_empty.rs` (3 v1 tests), `v3/create.rs::ref_ddl_on_a_v1_table_commits_like_spark`, `test_ice_ddl_alter_2.py::test_a_wap_branch_write_on_a_v1_table_commits_on_the_branch`. Red-first: each flipped pin failed on the unfixed tree with the removed guard's text. The WAP-gating control (no `write.wap.enabled`, the write lands on main) keeps its pin. Supersedes C-053; retires R-U5-PR2B-V1-REFS. |

### PR2b residues (dated 2026-09-25)

- R-U5-PR2B-V1-REFS: the fork writes format v1 metadata without `refs` (Java writes them), so `D-CREATE-V1` shows `md.refs` `[]` against Spark's `[[main, branch, S0]]`. C-053 refuses every non-main ref write until fork unit F-V1-REFS-1 lands. A RePark commit to a Java-written v1 table that carries branches or tags drops them (fork-level, pre-existing). RETIRED 2026-09-26 (WO RP50-A): fork F-V1-REFS-1 keeps v1 refs (repin repark#836), the C-053 guard is removed, and C-060 commits what C-053 refused.
...
- R-U5-PR3-NON-PRIMITIVE (narrowed 2026-09-25, verifier V-002): a STRUCT, ARRAY or MAP typed partition column raises PySparkException with the fork's text, and Spark raises Py4JJavaError ValidationException with its own rendering. Measured pairs: `STRUCT<a: INT>` → `DataInvalid => Cannot partition by non-primitive source field: 'struct<int>'.` vs `…: struct<2: a: optional int>`; `STRUCT<a: INT, b: STRING>` → `…: 'struct<intstring>'.` vs `…: struct<2: a: optional int, 3: b: optional string>`; `MAP<STRING,INT>` → `…: 'map'.` vs `…: map<string, int>`; `ARRAY<MAP<STRING,INT>>` → `…: 'list'.` vs `…: list<map<string, int>>`. The element splitter keeps commas inside `<…>`, so each whole type reaches this path. Neither engine creates a table.
- R-U5-PR3-CTAS-TYPED (pre-existing; narrowed 2026-09-25, verifier V-004): CTAS whose PARTITIONED BY holds only typed columns keeps RePark's AnalysisException `Partition column types may not be specified in Create Table As Select (CTAS): partition column `p` carries a data type. …`; Spark raises ParseException `Operation not allowed: Partition column types may not be specified in Create Table As Select (CTAS).`
- R-U5-PR3-RENAME-TABLE (HALT, 2026-09-25; **corrected 2026-09-25 by the orchestrator ruling**): the brief's near-miss line, "RENAME TO on the default (uuid-naming) memory catalog unchanged", and its premise that only hadoop-type catalogs refuse a three-part target are withdrawn. The Spark door now reads the target inside the source catalog on every non-hadoop catalog (C-059), and D-RENAME-TABLE replays EQUAL.
- R-U5-PR3-RENAME-CLASS: Spark raises Py4JJavaError wrapping NoSuchNamespaceException; RePark raises PySparkException `datafusion engine error: Execution error: Cannot rename …` with Spark's text after the prefix.
- R-U5-PR3-NESTED-NS-SQL (pre-existing, owner item D-NS-NESTED): Spark SQL `CREATE NAMESPACE sc.sc.ns`, `SHOW TABLES IN sc.sc.ns` and a four-part `SELECT` work; RePark refuses all three. The move into an existing nested namespace is pinned through the catalog API only.
- R-U5-PR3-RENAME-MISSING-SOURCE (pre-existing): a missing source keeps the catalog's `NamespaceNotFound => No such namespace` or `No such table` text; Spark raises TABLE_OR_VIEW_NOT_FOUND at analysis.
- R-U5-PR3-DBT-RENAME: dbt-repark relations render the catalog, so `spark__rename_relation` emits a three-part target, which now refuses as on Spark (`R-RENAME-THREE-PART`). dbt-spark renders a two-part target. The fix is a `repark__rename_relation` that renders the target without the catalog. No dbt materialization calls rename (the table materialization uses `create or replace`).
- R-U5-PR3-CTAS-TYPED addendum (2026-09-25, V-004): a CTAS mixing an untyped and a typed element (`PARTITIONED BY (id, p STRING) AS SELECT …`) runs the mix check first and raises ParseException `Operation not allowed: PARTITION BY: Cannot mix partition expressions and partition columns:\nExpressions: id\nColumns: p string.`, Spark's text (`target/probe-u5-pr3` critic `xr-spark.out` line `ctasmix`). Pins: `create_typed_partition.rs::ctas_mixing_untyped_and_typed_partition_elements_answers_the_mix_text`, `test_ice_typed_partition_refusals.py::test_ctas_mixing_untyped_and_typed_partition_elements_answers_the_mix_text`.
- R-U5-PR3-CASE-SENSITIVE (2026-09-25, verifier V-001): under `spark.sql.caseSensitive=true`, Spark 4.1.2 commits `(id BIGINT, data STRING) PARTITIONED BY (DATA STRING)` (schema `data`#2 + `DATA`#3, spec `DATA`/1000) and `(id BIGINT) PARTITIONED BY (p STRING, P INT)`. RePark's duplicate check reads the conf and compares exactly, so both reach the fork, whose schema builder cannot hold two names that differ only by case: PySparkException `DataInvalid => Cannot build lower case index: data and DATA collide` (`p and P`), the same refusal as the declared form `(data STRING, DATA STRING)`. Nothing is created. The fix is in the fork's lower-case index. Pins: `create_typed_partition.rs::typed_partition_columns_differing_by_case_reach_the_fork_under_case_sensitive`, `test_ice_typed_partition_refusals.py::test_typed_partition_columns_differing_by_case_reach_the_fork_under_case_sensitive`.
- R-U5-PR3-SYNTAX-CONTEXT (2026-09-25, verifier V-003): a typed column option other than `NOT NULL` or `COMMENT` (`DEFAULT 'x'`, `NOT NULL DEFAULT 'a' COMMENT 'c'`, `NULL`) raises ParseException `[PARSE_SYNTAX_ERROR] Syntax error at or near '<token>'. SQLSTATE: 42601`, Spark's text; Spark adds ` (line 1, pos N)` and the `== SQL ==` block, which RePark does not render (the nested-column DDL precedent). Measured: `default`, `nn_default` in the critic's `xr-spark.out`; `NULL` in `scratchpad/xr/rep_spark.out`. Nothing is created.
- R-U5-PR3-TEMP-VIEW-RENAME (pre-existing, outside the PR3 diff; 2026-09-25, verifier V-006): `CREATE TEMPORARY VIEW tv …; ALTER TABLE tv RENAME TO tw` renames on Spark (SHOW VIEWS lists `tw`); RePark raises `Error during planning: unknown catalog `spark_catalog`` from the source resolution in `catalog_ops.rs`. `ALTER VIEW tv RENAME TO tw2` answers the same on RePark, where Spark raises TABLE_OR_VIEW_NOT_FOUND after the first rename. No code in this PR.
- R-U5-PR3-INSERT-NOT-NULL (pre-existing, outside the PR3 diff; 2026-09-25, verifier V-004): after `CREATE … PARTITIONED BY (p STRING, q INT NOT NULL)` (or a declared `q INT NOT NULL`), `INSERT INTO t (id, p) VALUES (1, 'x')` raises Spark's AnalysisException `[INCOMPATIBLE_DATA_FOR_TABLE.CANNOT_FIND_DATA] … Cannot find data for the output column `q``, and `INSERT … VALUES (1, 'x', NULL)` raises SparkRuntimeException `[NOT_NULL_ASSERT_VIOLATION]`; RePark answers both with AnalysisException `Error during planning: Input schema does not match Iceberg table schema…`. No code in this PR.
- R-U5-PR3-OPTION-TAIL-TEXT (2026-09-25, verifier r3 V-001; text only, both engines refuse and create no table): three option tails answer a different PARSE_SYNTAX_ERROR text: `(p STRING NOT)` — Spark `Syntax error at or near ')': missing 'NULL'`, RePark `near 'NOT'`; `(p STRING COMMENT)` — Spark `[PARSE_SYNTAX_ERROR] … near ')'. SQLSTATE: 42601`, RePark ParseException without condition or SQLSTATE `SQL error: ParserError("Expected: literal string, found: EOF")` (`parse_literal_string`); `(p STRING NOT NULL NULL)` — Spark `near 'NULL': extra input 'NULL'`, RePark drops the `: extra input 'NULL'` tail (`xr3-spark.out` vs `xr3-repark.out`).
- R-U5-PR3-EMPTY-ELEMENT (pre-existing, outside the PR3 diff; 2026-09-25, verifier r3 V-002): `(p STRING,)` and `(p STRING NOT NULL,)` raise RePark's `Error during planning: malformed PARTITIONED BY clause: empty partition field (write PARTITIONED BY (col[, col…]))` where Spark raises ParseException `[PARSE_SYNTAX_ERROR] Syntax error at or near ')'. SQLSTATE: 42601`; a bare `(p)` on a table without `p` raises UNRESOLVED_COLUMN.WITH_SUGGESTION 42703 where Spark raises AnalysisException `_LEGACY_ERROR_TEMP_3060` `Couldn't find column p in:\nroot\n |-- id: long (nullable = true)`. Both engines refuse and create no table.
- Follow-up (2026-09-25, verifier V-008): the dbt consequence of C-059 is a regression for dbt-repark callers. `spark__rename_relation` renders the three-part relation (`ReparkIncludePolicy` database=True), so any `adapter.rename_relation` now refuses `Namespace does not exist: <catalog>.<schema>`. The fix is a `repark__rename_relation` macro that renders the target without the catalog. No roadmap card for dbt rename exists; this line tracks it.
- Verifier V-005 (2026-09-25): `CREATE OR REPLACE TABLE` and `REPLACE TABLE` with a typed column kept (`b`) and one added (`c`) are pinned: `b` keeps id 3 and spec field 1001, `c` gets id 4 and 1002, last-partition-id 1002, equal to Spark's `ror0`/`ror1` and the `REPLACE TABLE` measurement in `scratchpad/xr/rep_spark.out`. Pin: `create_typed_partition.rs::typed_partition_columns_append_identity_fields_like_spark` (`replace_rekeys_typed_columns`).
