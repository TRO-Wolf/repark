...
| C-022 | Run 22b: planning through the column repair is stack-bounded — a 1,000-branch `UNION ALL` (plain and wrong-case fold) plans on a 2 MiB thread in debug frames, and a 5,000-branch one plans on the Spark door (debug wheel) and on a 2 MiB thread. No walk the unit added recurses per set-operation level. | Rust `s22b_*` (tests.rs); Python `test_five_thousand_branch_union_all_plans_on_the_spark_door`; `test_perf_approxpct_1.py` on the debug wheel. | PROVEN | Run 22b below: red on `759ec9bd` (SIGABRT stack overflow, gdb frame `SetExpr::clone` from `column_resolution.rs:28`), green after the grown-stack fix; 8 KiB-per-level mutation crashes. |
