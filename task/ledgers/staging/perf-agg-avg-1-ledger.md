...
| C-003 | Grouped `avg` / `try_avg` answers are Spark-equal on int, float and decimal inputs with NULLs, on empty input, on 2e5 groups, and on decimal result precision/scale — every expectation recorded from live PySpark 4.1.2, value AND Arrow-path type. | `python/repark/tests/test_perf_agg_avg_1.py` (always-run pins + `REPARK_PARITY_LIVE=1` legs); the recorded Spark outputs in §8. | **PROVEN** | 28 passed, 7 skipped routine; 154 passed in the `test_parity_live.py` co-collection with `REPARK_PARITY_LIVE=1` (all 7 live legs green beside `test_live_disclosure_still_diverges`). Every literal in the file was printed by the throwaway recorder from live Spark and is quoted verbatim in §8. Three pre-existing divergences disclosed, not absorbed: group keys nullable here vs not-null on Spark (live legs project to the avg column), multi-column distinct refuses here vs answers on Spark, and the decimal sum-wrap fixtures answering the wrapped quotient (`0.0000`, `100000.0000`) where Spark NULLs or raises (BACKLOG AVG-DEC-SUMWRAP-1). A fourth disclosure is unit-caused, not pre-existing: grouped float avgs change bit-for-bit vs the base (repark `153846153846153.84` vs Spark `153846153846154.34` on the 1e16-plus-ones fixture, 3.3e-15 relative, inside 1e-12; FLOAT-AGG-3). |
...
sum-floor unreachability proof, and the pin citations. Round 2 adds BACKLOG rows
AVG-DEC-SUMWRAP-1 (S2-4) and FLOAT-AGG-3 (S2-2). `docs/perf/aggregate-baseline.md`
...
Grouped-float fixture: Spark grouped and global `153846153846154.34`; repark
grouped `153846153846153.84`, global `153846153846154.6` (the global path still
uses the Arrow kernel, so the unit leaves it unchanged). Round-3 oracle additions
(Spark 4.1.2, UTC, ANSI on, `local[2]`, no sibling JVM): non-zero-wrap fixture
Spark grouped `try_avg` `[None]` and grouped `avg` raising `[ARITHMETIC_OVERFLOW]
Overflow in sum of decimals`, SQL and DataFrame alike; repark grouped `try_avg`
and `avg` answering `Decimal('100000.0000')` at `decimal128(38, 4)` on facade
SQL, native SQL and DataFrame. Zero-wrap window re-verified: Spark `try_avg`
four `None`s, Spark `avg` raising `[NUMERIC_VALUE_OUT_OF_RANGE...] cannot be
represented as Decimal(38, 4)`; repark `[None, None, None, 0.0000]` and
...
COVERAGE_ATTESTATION:
  ...
  categories:
    ...
    - id: AT-2
      status: ATTACKED
      evidence: NULLs in values, all-NULL groups, fully-filtered groups, never-seen group indices, empty input, 2e5 groups, all four decimal widths, Decimal(38,0) 2x-MAX overflow for both avg (raises) and try_avg (NULL) plus the sum-wrap divergence pin, unrepresentable count, mismatched type pairs refused, grouped refusal shapes pinned, grouped-float drift owned at exact bits, EmitTo::First partial emission, merge of two partials, convert_to_state with filter.
    ...
    - id: AT-6
      status: ATTACKED
      evidence: Every Python expectation was printed by a throwaway recorder from live PySpark 4.1.2 running the test module's own SQL constants, and is quoted verbatim in section 8; the 7 live legs re-derive the answers from the pinned oracle on every live run beside the disclosure control. Value AND Arrow type asserted on the collect path, never show-only.
    ...
    - id: AT-9
      status: ATTACKED
      evidence: A silently skipped groups path cannot pass as success: groups_supported is unit-pinned true per type, and the committed cost probe is red (4.06x) unless the groups path actually runs. A silently wrong answer cannot pass: 24 Spark-recorded pins plus 3 round-2 behavior pins and bit-exact live frame compares, with the three pre-existing divergences plus the unit-caused grouped-float drift disclosed rather than absorbed.
      artifacts: [python/repark/tests/test_perf_agg_avg_1.py]
    - id: AT-10
      status: ATTACKED
