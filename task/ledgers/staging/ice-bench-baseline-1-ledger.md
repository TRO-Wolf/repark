...
| C-003 | The report JSON header carries `"baseline": <bool>` and, when true, `"baseline_switches": {"row_selection_enabled": false, "metadata_cache": false, "manifest_cache_bytes": 0, "footer_cache_bytes": 0}` (null when false); the markdown header line names `mode=` and `baseline=`. | The header asserts inside `a_warm_baseline_run_reads_every_footer_on_its_second_sample`; `the_human_table_names_the_baseline_in_its_header_line`. | PROVEN | Green on `001ad6d9`. Mutations " `baseline_switches` always null" and "drop the header `writeln!`" each red their pin (2026-09-19). |
