...
| `python/repark-parity/tests` (whole suite) | first run 755 passed, 2 failed — both from a stale staging-map row for the completed REGISTRY-16B-1 unit (ledger already in `completed/`, row still pointed at `staging/`); repointed to `../completed/` per the file's own idiom, after which `ledger-check` (1008 links) and `map-sync` (276 maps) pass. Pre-existing on `origin/main` (verified via `git show`/`ls-tree`, not by a main-tree test run). |
