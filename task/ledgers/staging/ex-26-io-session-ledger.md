...
repark-only names (`smartCsv` and `dynamicFlatten`/`dynamic_flatten`)
...
oracle values exist). Eleven new §7 rows (EX-IO-1..10, EX-SES-6) pin the diverged arms of covered
names, with thirteen tests in the new `test_examples_io_session.py`; EX-SES-1's Spark
...
| C-015 | Every diverged arm of a covered name that the oracle table measures as a value, shape, or listing divergence carries a §7 row naming its measured cell (EX-IO-1..10, EX-SES-6; EX-IO-8 gains the insertInto arity arm, EX-IO-10 rows the writer-output listing that round 1 left disclosed-not-rowed), the four excel names stay with EX-IO-7, and every new row carries a pin in `python/repark/tests/test_examples_io_session.py` (13 tests, all green); the three programming-misuse/eagerness arms stay observed-not-rowed in review notes, and EX-SES-1's Spark half-sentence carries a dated correction that leaves its pin untouched. | The eleven registry rows plus the dated correction, the pin file's green run, and the oracle table's stayed column. | **PROVEN** |
...
the file bytes, the hasattr batch, and the repark-only cells, plus a third round
(`spark_probe_round3.py`, `repark_probe_round3.py`) for the writer-output listings and the
insertInto arity texts. The Spark halves below are verbatim
...
`maturin develop --release` in-lane). Four JVM runs (three in round 1, one in round 2), each
stopped by its script (`spark.stop()`); no lane JVM remained afterwards. The seventeen
pre-disposed stays were not re-measured: their rows and pins stand, and the pin suites re-run
green in the gates table.
...
| `DataFrameWriter.csv` | header-false default; explicit arms | explicit arms byte-identical; default headers | covered + BACKLOG ARM | `io/writer_csv.py` + EX-IO-4, EX-IO-10 |
| `DataFrameWriter.format` | csv/json + save spellings | identical | covered | `io/writer_csv.py` |
| `DataFrameWriter.insertInto` | positional `[(1,a),(2,b),(9,z)]`; arity `[INSERT_COLUMN_ARITY_MISMATCH.…]` | identical; missing and arity texts differ | covered + BACKLOG TEXT | `io/writer_tables.py` + EX-IO-8 |
...
| `DataFrameWriter.json` | `{"id":1,"name":"a"}` lines | byte-identical; listing differs | covered + BACKLOG SHAPE | `io/writer_json.py` + EX-IO-10 |
...
| `DataFrameWriter.partitionBy` | `name=` dirs, `1`/`2` bytes | identical dirs + bytes; listing differs | covered + BACKLOG SHAPE | `io/writer_partition.py` + EX-IO-10 |
...
| `DataFrameWriter.save` | explicit formats; parquet default | explicit identical; default raises; listing differs | covered + BACKLOG ARM | `io/writer_csv.py` + EX-IO-5, EX-IO-10 |
...
## Gates (round 2, 2026-09-06, on this tree)
...
| `make py-lint` | **0** |
| `make py-format-check` | **0** (751 files formatted) |
| `.venv/bin/python -m pytest python/repark/tests/test_examples_*.py -q` | **0** (68 passed: 55 existing + 13 new) |
| the twelve scripts, each run twice from the repo root | **0** (all 24 runs green; `git status` shows no new artifact) |
| `make check-map-sync` | **0** |
| `make check-ledger-grammar` | **0** |
| `make check-ledgers` | **0** |
| `make check-docs-compaction` | **0** |
...
## Review notes (round 2, critic remediation)

| Finding | Disposition |
|---|---|
| F1 (S2): no §7/§8 row for the writer-output listing divergence, so round-1 C-015 was false for `DataFrameWriter.csv/json/save/partitionBy` | rowed as EX-IO-10 (both engines' listings measured in round 3, Spark's quoted) with pin `test_writer_output_listing`; C-015 re-worded with the honest bound. The `test_writer_v2.py:362` parquet shape disclosure was judged not-exact (parquet only, silent on `part-*` naming), so the row was added instead of cited |
| F2 (S2): `session/udtf.py` imported `udtf` from `repark.spark.udtf`, not the drop-in `repark.spark.functions` | import moved (verified the same object); the session map row names the drop-in path |
| F3 (S3): the scope sentence claimed the writer snake spellings say repark-only in their module docstrings; they do not | sentence narrowed to `smartCsv` and `dynamicFlatten`/`dynamic_flatten`; the family map rows carry the snake-spelling disclosure |
| F4 (S3): the twelve scripts lacked the `main()` one-liner the io map's house form names | one-liners added to all twelve, plus EX-25's five under `docs/examples/functions/` in the same commit; all four directories now carry the one-liner on every example (verified by scan) |
| F5 (S3): root runs left 16 `ex26_*` artifacts | the six io scripts write under `tempfile.TemporaryDirectory()` (the `describe_ingest.py` form); all twelve run green twice from the root with no new artifact |
| F6 (S3): the `insertInto` arity-mismatch texts were unrowed | EX-IO-8 extended with the arity arm (both widths measured on both engines) and pin `test_insert_arity_text` |
...
Round 2 (same leg, 2026-09-06) remediated the critic's two S2 and four S3 findings: measured
the writer-output listings and the insertInto arity texts on both engines (one more JVM run,
stopped by its script), rowed EX-IO-10, extended EX-IO-8, added the two pins, moved the io
scripts under `TemporaryDirectory`, added the seventeen `main()` one-liners, and re-worded
C-015 with the honest bound.
...
156 KB after round 3's two probes plus two cell logs, all removable at close). Lane-local weight added by this unit: `.ivy2/` (182 MB, copied
...
- Pins: [../../../python/repark/tests/test_examples_io_session.py](../../../python/repark/tests/test_examples_io_session.py) (13 tests for EX-IO-1..10, the EX-IO-8 arity arm, and EX-SES-6)
- Registry: [../../../docs/spark-sql-iceberg-parity.md](../../../docs/spark-sql-iceberg-parity.md) §7 (EX-IO-1..10 and EX-SES-6 new; EX-IO-8 extended; EX-SES-1 corrected)
...
DELIVERY_SIGNOFF:
  ...
  status_update: v1.1 example backfill, reader/writer/session long tail — 29 covered, 21 stayed with EX-IO-1..10 and EX-SES-6
  verdict: PENDING
  rejection_route: N/A
```
