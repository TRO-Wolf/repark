...
| File | Change |
|---|---|
| `python/repark/src/repark/spark/dataframe/streaming_batch.py` | New, ~190 lines. The interval parser, the `_raise_analysis` structured-error helper (reusing `_integral.py`'s attach functions), the three refusal properties + `pandas_api` in `DECLARED_MEMBERS`, `with_watermark`, and `drop_duplicates_within_watermark`. |
...
| `python/repark/tests/test_df_stream_batch_1.py` | New. 12 cell-driven pins. |
