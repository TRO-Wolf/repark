...
| C-005 | `start-timestamp` / `end-timestamp` on a plain table load refuse with Java's ``Only changelog scans support `start-timestamp` and `end-timestamp`. Use `start-snapshot-id` and `end-snapshot-id` for incremental scans.`` — they are changelog-only, and they stay loud on a non-Iceberg format. | `append_boundaries_refuse_a_timestamp_window`; `test_denylist_semantic_keys_fail_loud`; `test_incremental_start_timestamp_refuses_on_plain_iceberg_load` | PROVEN | Read off `SparkReadConf.incrementalAppendScanBoundaries`'s first `checkArgument`. Round 2 (V-004): the denylist parametrize covers all four keys and the new pin asserts the `IllegalArgumentException` on a plain table load. pins: ice-changelog-1/C-005 |
...
| C-009 | `t.changes` is a RELATION, not a metadata table: the table's columns plus `_change_type` (string), `_change_ordinal` (int) and `_commit_snapshot_id` (bigint), raw INSERT/DELETE, whole history when no window is given — including a copy-on-write UPDATE's carryover rows, which the relation keeps. | `test_changes_relation_whole_history`, `test_changes_relation_schema`, `test_changes_relation_over_a_copy_on_write_update`, `test_changes_relation_over_a_whole_file_merge_on_read_delete`, `test_changes_relation_v3_deletion_vector` | PROVEN | Cells QI-CHANGES-DEFAULT, QI-CHANGES-COLS, QI-CHANGES-COW-UPDATE, QI-CHANGES-MOR-DELETE, QI-CHANGES-V3-DV. Round 2 (V-002, A-8): `test_changes_relation_whole_history` asserts `_commit_snapshot_id == snaps[_change_ordinal]` per row plus a full-row value assertion, and the Rust `rendered()` helper carries the column. pins: ice-changelog-1/C-009 |
| C-010 | The changelog window takes snapshot ids or TIMESTAMPS, resolved as `SparkChangelogScanBuilder` resolves them: the start timestamp maps through `SnapshotUtil.oldestAncestorAfter` (its own id when the timestamp matches exactly, else its PARENT), the end timestamp to the newest ancestor at or before it, with Java's two empty-scan short-circuits. | `test_changes_relation_reader_window`, `test_create_changelog_view_snapshot_window`, `test_create_changelog_view_start_timestamp_zero`, `test_changes_relation_reader_timestamp_window`, `test_changes_relation_reader_start_timestamp_between_snapshots` | PROVEN | Cells QI-CHANGES-READER-OPEN, QC-OPT-RANGE, QC-OPT-TS. Round 2 (V-003, A-3): the reader timestamp window (R-CHANGES-TS) and the strict-between start pin row values, not just success. pins: ice-changelog-1/C-010 |
...
4. **Timestamps resolve through Java's own helpers (A-3), not through `resolve_snapshot_id`.**
   ...
   implementation follows the bytecode. Round 2 pins it with rows:
   `test_changes_relation_reader_timestamp_window` and
   `test_changes_relation_reader_start_timestamp_between_snapshots`.
...
## Remediation round 2 (2026-09-20)

The critic returned the RePark half NEEDS_REMEDIATION on V-001..V-005, and CI was red on
ruff. Round 2 closes V-001..V-004 plus the ruff red; V-005 is residue 4 below. No product
code changed — the classifier design, the plan-time `plan_files` probe, the site-local
`read_table_at` Incremental catch and the `classify_datafusion_error` mapping are untouched.

- V-001 → `test_incremental_snapshot_bounds_reach_the_incremental_scan` (C-004, C-007).
- V-002 → the `_commit_snapshot_id` value pins (C-009, A-8).
- V-003 → the reader timestamp-window pins (C-010, A-3, R-CHANGES-TS).
- V-004 → the timestamp denylist pins (C-005).
- CI ruff → the eight unused `B017` directives are deleted; `B017` stays silent.
...
3. **`Identifier field is required as table contains unorderable columns: %s`** (Java's third
   ...
   refuses.
4. **The MoR `DeletedRows` refusal crosses the Arrow C stream as `PySparkException` (critic
   V-005, recorded 2026-09-20, not fixed).** The plan-time probe in
   `ChangelogTableProvider::scan` gates the MoR refusal at `plan_files`, but
   `ChangelogReader::read_one_task`'s `DeletedRows` error is a stream-item error that still
   crosses the Arrow C stream and surfaces as `PySparkException`. RePark never sets
   `with_row_level_deletes(true)`, so Spark-matching MoR is covered and
   `F-CHANGELOG-READER-1` stays unpinned on RePark.
