...
| C-003 | A `location` that equals or contains another table of the same catalog refuses, names that table as `<catalog>.<ns>.<table>`, and deletes nothing from either table. | Rust `call_remove_orphan_files_refuses_a_location_holding_another_table`. | **PROVEN** | The namespace directory, the other table's own directory and a `file://` spelling of the namespace directory all refuse naming `ice.ns.b`; both planted orphans and the other table's live data file survive. |
