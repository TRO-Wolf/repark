...
| C-007 | `PARTITION … BY NAME` matches Spark: static overwrite keeps other partitions, dynamic overwrite replaces all, static append injects the clause value, a source naming the static column refuses 42713. | `partition_by_name` oracle section + 6 pins + positional controls. | PROVEN | Spark cells re-measured 2026-09-17 into the generator; pins green on the release native. Dynamic overwrite is whole-table replace-all (Spark default mode; `partitionOverwriteMode` unread). pins: ice-rtas-byname-1/C-007 |
| C-008 | Empty `INSERT OVERWRITE … BY NAME` wipes; empty static-partition overwrite drops only the named partition. | 2 pins. | PROVEN | Pins green on the release native. pins: ice-rtas-byname-1/C-008 |
| C-009 | A missing required target refuses `CANNOT_FIND_DATA` (KD000) before any write on INSERT and OVERWRITE; a missing nullable still NULL-fills. | `not_null_by_name` oracle section + 3 pins. | PROVEN | Spark error text re-measured 2026-09-17; pins green on the release native. pins: ice-rtas-byname-1/C-009 |
| C-010 | `spark.sql.caseSensitive=true` matches exact and answers `EXTRA_COLUMNS` on mismatch; default folds. | `case_sensitive_by_name` oracle section + 3 pins + carrier. | PROVEN | Spark cells re-measured 2026-09-17; new `SparkCaseSensitiveConfig` carrier served through `conf.set`; pins green on the release native. pins: ice-rtas-byname-1/C-010 |
