...
| R2-C004 (gates) | Round-2 gates green with counts. | PROVEN | Unit file live (NaN-lane command shape) `21 passed, 2 xfailed` in 121.48s; `cargo test -p repark-iceberg --lib hadoop_stale_commit` 2 passed; `uvx ruff@0.15.22 check .` clean; `make verify` rc 0; whole facade `9340 passed, 374 skipped, 28 xfailed` (`/tmp/oc-worker/jb-vn/facade-r2.log`); whole parity `757 passed, 2 skipped, 12 xfailed` (`/tmp/oc-worker/jb-vn/parity-r2.log`); added-comment count 0. pins: ice-hadoop-vn-1/C-009 |
