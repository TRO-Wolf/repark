...
**Amended by U10-MC-DELETED-1:** branch `fix/u10-mc-deleted`, commit "mcdel-r4: parity row and predecessor ledger say _deleted is served", Claude Opus (`claude-opus-5-5`), 2026-09-23. That unit added supersession notes to C-007, C-008 and C-018 and moved C-023 to REJECTED (SUPERSEDED by u10-mc-deleted-1/C-001). It also updated row `ICE-MC-FILEPOS-1`. The header fields above describe this unit's own branch and base.
...
| C-023 | `_deleted` alone refuses typed `[ICE-MC-1]` advertising the served four; the former `_partition`-beside-`_spec_id` pin now succeeds. | Originally `unserved_metadata_columns_refuse_with_a_typed_error`, `test_unserved_metadata_columns_refuse_typed`; both removed by U10-MC-DELETED-1. | **REJECTED** (SUPERSEDED by u10-mc-deleted-1/C-001) | Held until 2026-09-23. U10-MC-DELETED-1 serves `_deleted`: `SELECT id, _deleted` on the merge-on-read seed answers `[(1,true),(2,false),(3,false),(4,false)]` (`deleted_column_marks_merge_on_read_deleted_row`, `test_deleted_marks_merge_on_read_deleted_row`), and backtick `` `_deleted` `` answers its rows (`served_names_fold_and_composed_shapes_refuse`). The `_partition`-beside-`_spec_id` half still holds under ice-metadata-cols-1/C-021. |
