...
| `scripts/check_lib_py.py` | `test_dynamic_flatten.py` ceiling 1618 → 1659 |
...
| `test_three_level_struct_with_keep_column_hits_qualified_name_clash` | `DYNFLATTEN-QUALNAME-1` error text |
...
| `check_lib_py.py` | `test_dynamic_flatten.py` ceiling raised 1618 → 1659, exactly the 41 lines of the `DYNFLATTEN-QUALNAME-1` pin. No other file, threshold or rule in that gate was touched; the ceiling only ratchets for the one pin the review asked us to file. |
...
| `.venv/bin/python -m pytest python/repark/tests/test_dynamic_flatten.py python/repark/tests/test_dynflatten_bed_gate.py -q` | 0 (47 passed) |
