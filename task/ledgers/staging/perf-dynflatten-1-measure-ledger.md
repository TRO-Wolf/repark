...
| Round 6 — live module split | Merging main pushed `test_parity_live.py` to 1038 lines against its 1000 ceiling (main added printSchema and regexp_extract legs; this unit added ~95). Ceilings only move down, so no EXCEPTIONS row: the unit's live legs moved to `python/repark/tests/test_parity_live_dynflatten.py` (934 + 101 lines). The session-scoped `spark_engine` moved to `conftest.py` — importing a fixture into a second module registers a SECOND definition and would build a second JVM; one conftest definition keeps one. `spark_iceberg_engine` stayed put (single consumer). Registry pin paths, both map rows and the gate command follow the move; co-collection with the disclosure legs is unchanged (17 passed). |
...
| Live leg | `REPARK_PARITY_LIVE=1 … -k "dynflatten or disclosure"` exits 0 (17 passed, 105 deselected) with ivy redirected into the clone. The ivy directory is untracked and is in no commit. |
