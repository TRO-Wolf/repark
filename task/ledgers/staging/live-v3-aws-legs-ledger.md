...
| C-004 | Documents say exactly what is true: registry `S3T-V3-1` records wired-and-unmeasured with the pending question; the north-star row stays ❌ and names what the first run answers; `docs/tier2-aws.md` §6 carries one row per leg and states that nothing widens; every touched `map.md` in lockstep; this ledger `move`d to `completed/` last. | A tree pin over the three documents; `make check-map-sync`, `check-ledger-grammar`, `check-ledgers`, `check-docs-compaction`. | **PROVEN** | `python/repark-parity/tests/test_live_v3_docs.py` (four tests, whitespace-normalized). Maps: `python/repark/tests/map.md` (four entries), `python/repark-parity/tests/map.md`, `docs/map.md`, `task/roadmap/epic-term/map.md`, `task/ledgers/staging/map.md`. Citation: `python/repark-parity/tests/map.md`. |
...
COVERAGE_ATTESTATION:
  ...
  categories:
    ...
    - id: AT-9
      status: ATTACKED
      evidence: Registry S3T-V3-1 states the pending measurement and the decision table, and the tree pin refuses a green claim before a run. The measured _row_id nondeterminism is recorded as F-LIVEV3-1 rather than pinned to a flapping value.
      artifacts: [docs/spark-sql-iceberg-parity.md, python/repark-parity/tests/test_live_v3_docs.py]
    - id: AT-10
      status: ATTACKED
      evidence: Four clauses pinned; five map.md files in lockstep; mutation 18 red of 18, restored and re-run green.
...
| 2 | `INSERT INTO … VALUES (n, 'nn', n % 2)` × 9 (ids 2–10, one row each) | 5 data files per identity partition — one over the `min-input-files` floor of 5 for `rewrite_data_files` |
...
**Finding F-LIVEV3-1 (S2, AT-9).** The `_row_id` a merge-on-read `MERGE … WHEN NOT MATCHED THEN
INSERT` assigns to the new row is **not deterministic**: over five identical local runs of this
exact sequence it was `11` three times and `10` twice, with `_last_updated_sequence_number` `12`
every time and every survivor id stable. Two earlier seed shapes (a two-row CTAS spanning both
partitions, and two-row appends) also shuffled survivor `_row_id` between runs, which is why the
seed is one CTAS row plus nine single-row appends. Disposition: RECORDED. The asserter pins the
invariant the spec actually requires — the inserted row takes an id no survivor holds, at or above
the seed count — rather than a value that would flap. Repair belongs to a v3 lineage unit, not to
a live-leg unit, and no Spark oracle was run for this shape, so no parity claim is made.
