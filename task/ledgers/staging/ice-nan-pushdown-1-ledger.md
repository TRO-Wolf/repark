...
| C-003 | `d != NaN` answers the non-NaN non-null ids and pushes nothing since RP-27 (fork #294; `Unary NotNan(d)` before); double and float, both doors | `test_ice_nan_pushdown_1.py` neq legs + Rust `nan_inequality_pushes_nothing` | PROVEN |
...
| C-006 | `NOT IN (NaN)` answers the non-NaN non-null ids; single-element reaches `!=` and pushes nothing since RP-27 (`NotNan` before); double and float, both doors | `test_ice_nan_pushdown_1.py` not-in legs + Rust `nan_single_not_in_pushes_nothing` | PROVEN |
