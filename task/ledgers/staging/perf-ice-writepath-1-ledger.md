...
| C-007 | The writers' work overlaps instead of serializing. | A Rust differential pin: an injected blocking delay costs one writer four times what it costs four writers; a mutation forcing one writer reds it. Plus the measured CTAS walls. | **PROVEN** | `every_input_partition_gets_its_own_writer_and_data_file` — four partitions, four data files holding exactly 8/16/24/32 rows in writer-index order, and a one-task drive of the same partitions answering identically; the executor spawns one task per partition (`RecordBatchReceiverStream::run_input`), which is where the encode leaves one task. Mutation M1 (one writer over a coalesce) reds it. A wall assertion was tried and REMOVED: under `cargo test` the fixed cost of four small parquet writes swamped a 120 ms injected delay (6.41 s floor against a 6.00 s delayed run), so the timing evidence is §8's release measurement instead. |
