...
| C-005 | At every partition count the CTAS commit is an ORDERING: the manifest ascends by content, `_row_id` tiles it contiguously from zero, the committed row set is the expected digest of ids, and two runs with the same file grouping commit the same id-to-`_row_id` map. The committed LAYOUT is NOT reproducible, because the scan's grouping is not. | Five v3 CTAS over eight UNEQUAL source files at 3, 4, 8 and 16 partitions; ten consecutive runs of the pin on four cores and ten on all cores; a mutation restoring round 1's ordering must red it at every count. | **PROVEN** | Refuted twice before it was true. Round 1 ordered by writer index — the index is not stable. Round 2 ordered by content and asserted the record-count sequence — refuted by CI's 4-core runner: `target_partitions` 3 gives 3 distinct groupings in 5 runs, 4 gives 4-6 in 10, and 8 and 16 give 1, because at 8 the eight source files meet the partitions 1:1, which is the only reason round 2's pin looked green. Round 3 asserts what survives any grouping; round 4 (critic G3) made the fourth conjunct falsifiable — it was keyed by the record-count sequence, which `_row_id` is already asserted to be the cumulative sum of, so it could not fail; it is now keyed by a hash of the id-to-`_row_id` map, and two groupings can share a count sequence while differing in which rows each file holds. 20 of 20 consecutive runs green; mutation M5 reds it at all four counts. Residual `WRITE-GROUPING-CTAS-1`. |
...
| C-007 | The writers' work overlaps instead of serializing, and the shipped tree beats the base on both CTAS cells by a factor that reproduces across boxes. | The structural Rust pin, plus a before/after pair on the SHIPPED tree measured back to back with a same-run `df.write.parquet(zstd)` control, read as a ratio. | **PROVEN** | `every_input_partition_gets_its_own_writer_and_data_file`; mutation M1 reds it. B0 `6eaccd5e` against the branch, both on the pinned fork, load 8-14, control inside 5 % on both sides: `ctas` 1,384.80 → 135.48 ms (10.2×) and `ctas_partitioned8` 4,901.75 → 293.19 ms (16.7×). The ratio to control is what reproduces — 1.28× / 2.78× here, 1.38× / 3.00× on the round-2 critic's box at load 11.8-12.3, 1.39× / 2.69× on the round-3 critic's at load 6.4-8.5, i.e. inside 8 %. **The absolute targets are NOT claimed**: `ctas` read 135.48 ms here and 152.24 ms on the third box, so ≤ 150 ms is a property of the box. Round 1 called the targets missed and round 2 called them met; both were reading the load, and §8 carries the qualification. |
| C-008 | A failed write commits nothing, and when the table has no current snapshot — the CTAS case, which is the only caller — it leaves NO data file behind at all: the files completed writers returned and the files the failing writer had already rolled. Against a table that DOES have a snapshot the sweep is narrowed to the attempt's own completed files, so nothing another writer owns is touched. | A Rust pin with `write.target-file-size-bytes = 65536` so the failing writer rolls before the injected failure; a second pin over a seeded commit with a concurrent interloper file; mutations for both arms. | **PROVEN** | Round 2 widened the clause (round 1's sweep deleted only the completed files; the critic measured 9 orphans, 398 KB); round 4 (critic G7) qualifies it to the fresh arm, which is what the code checks. `a_failed_partition_deletes_every_completed_data_file` (fresh table, nothing survives) and `a_failed_write_into_a_committed_table_sweeps_only_its_own_files` (seeded commit plus a file another writer drops in DURING the attempt, both survive). Mutations M4 (completed files only) and M6 (sweep regardless of freshness) each red their arm. |
...
| `pytest python/repark/tests -q` | round 4 final: **4,897 passed, 204 skipped**. Round 2 read 4,805 / 200 on this lane (round-2 re-run on the shipped tree). The round-2 critic measured 4,803 / 202 on its own module: two live-oracle tests skip without `pyspark` in the environment, and this lane has it installed. The count is environment-dependent, not tree-dependent. |
| `pytest python/repark-parity/tests -q` | 574 passed |
| the C-005 pin at 3/4/8/16, lineage-keyed, 10 consecutive runs under `taskset -c 0-3` and 10 on all cores | 20 of 20 green |
| fork lane: `cargo test -p iceberg --lib arrow::record_batch_partition_splitter` over all 14 allow-listed RESULT types | 4 passed |
| `cargo test -p repark-spark --lib v3_row_order` | 6 passed (the V3-11 order pins) |
| `REPARK_PARITY_LIVE=1 pytest test_parity_live.py test_perf_ice_writepath_1.py test_sql_harden_cutover.py -q` | round 4 final: **178 passed** (the ordering pin is parametrized over 3, 4, 8, 16) |
...
## 15. Round 4 — the round-3 critic's seven findings

RePark FAIL (3 S2), fork PASS (1 S3). CI green on both PRs; M5, M6, the 10/10 ordering runs at
3 and 4, the tracked probes and the ratio-to-control all reproduced independently.

| # | sev | finding | disposition |
|---|---|---|---|
| G1 | S2 | `python/repark/tests/map.md` still carried round 2's retracted claim verbatim. | **REMEDIATED**: the sentence is deleted and the row states the round-3 claim. |
| G2 | S2 | The registry row and C-007 said "both targets are met" unqualified. | **REMEDIATED**: both now carry the gain, the three boxes' ratios (1.28 / 1.38 / 1.39 and 2.78 / 3.00 / 2.69) and the load qualification. `ctas` read 152.24 ms on the third box, so ≤ 150 ms is not claimed. |
| G3 | S2 | C-005's fourth conjunct could not fail: it was keyed by the record-count sequence, which `_row_id` is already asserted to equal the cumulative sum of. | **REMEDIATED**: keyed by a hash of the id-to-`_row_id` map, with the row set asserted as a digest of ids. Two groupings can share a count sequence and differ in membership, which is what it now catches. |
| G4 | S3 | Five places said "at 4 AND 16" while the pin runs 3/4/8/16. | **REMEDIATED**: one count list everywhere. |
| G5 | S3 | `WRITE-GROUPING-CTAS-1` cited a pin that is green with or without the defect. | **REMEDIATED**: the row now says plainly that no red-on-fix pin is possible and why, and points at the flip rates as the standing measurement. |
| G6 | S3 | The fork matrix covered 13 of 14 allow-listed RESULT types — Timestamp never appeared as one, Date and Timestamp had no identity spec. | **REMEDIATED** on the fork lane: `d_identity` and `t_identity` added, so all 14 result types are compared. |
| G7 | S3 | C-008 stated unqualified that a failed write leaves no file behind. | **REMEDIATED**: qualified to the fresh arm, with the committed-table arm and its own pin stated beside it. |
