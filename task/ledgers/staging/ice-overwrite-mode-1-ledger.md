...
| C-018 | (verification critic round 2, P2-OWP-TRANSFORM) `writeTo(t).overwritePartitions()` replaces the partitions the frame's rows land in on every spec, transforms and unpartitioned included, in both session modes, as Spark does; EX-W2-4 is FIXED on the way. | `test_ice_overwrite_mode_1_transform.py` (16 recorded Spark cells, `_record_ice_overwrite_mode_1_transform.py`); `binding_dynamic_intent_without_a_clause_replaces_the_staged_partitions_of_a_transform_spec`; the 16 `test_ice_evo_dml_1.py` DataFrame-door cells and `test_writerv2_overwrite_partitions_unpartitioned_replaces_the_table` run plainly. | PROVEN | Round 2's identity-only binding made the `PARTITION (<spec field names>)` clause the writer sent refuse `[NON_PARTITION_COLUMN]` on transform fields. The writer now sends no clause with the dynamic intent (`_dynamic_partition_sql` deleted; `writer_readwriter.py` 1093 → 1091). Orchestrator fix-up, measured on a release native. |
| C-019 | (verification critic round 2, P2-CAST-SPARK) A string static value on a TIMESTAMP / TIMESTAMPTZ identity partition refuses loud instead of casting in UTC (the session zone and Spark's grammar come with CAST-TS-STRING-1); DATE, numeric and boolean casts stay. | `string_static_value_on_a_timestamp_partition_refuses_until_the_cast_follows_spark` in `crates/repark-spark/src/tests/overwrite_mode.rs`; the guard in `write/static_value.rs`. | PROVEN | No recorded Spark cell has a timestamp identity static value; the refusal keeps a silent wrong partition from reachable input until the cast unit lands. P3-NOOP-UNIT and P3-RENAME-IDENT are accepted as P3: the door pins catch an always-true no-op, and a renamed identity field binds the right source column. |
