...
pins), the module entries in `crates/repark-spark/src/tests/mod.rs`, and
...
`[_deleted]` EQUAL; B10 `count(*) … WHERE id > 0 AND _spec_id = 0` = `[[2]]` EQUAL.
...
_spec_id FROM D` refuses `[_deleted]` on both (EQUAL — routed, and the expanded star
reads the user column); R4 the copy-on-write `DELETE FROM uc WHERE id = 1` — Spark
...
all refusing `[_deleted]` on both engines (EQUAL): N1 `GROUP BY _deleted`, N2
...
as Spark does, M-20). N14 (`WHERE`), N15 (`SELECT *`) and N16
...
`mcdqualfix-spark.log` (Spark 4.1.2), `mcdqualfix-repark-before.log` (RePark at
`ee655264`) and `mcdqualfix-repark.log` (RePark after the fix, wheel rebuilt), all rc 0.
Before the fix, X1–X3, K1, P1 and P2 widened the wildcard to every provider field
(ids `[1,2,3,4]` with the deleted row). B1/B2 (bare `*` under an alias) failed with
`UNRESOLVED_COLUMN` because the expansion qualified by the table name. Q1/Q2/N13
failed on the duplicate `_spec_id`. After the fix, every row is EQUAL with Spark:
X1–X3, B1, B2, K1, W1–W5, D1, C1, P1–P3, Q1, Q2, and N13 (the reserved-name refusal on
both). M1 and M2 error on both engines with different text (pre-existing). The lineage
rewriter's `x.*` shapes over a v3 table (L2–L4) are EQUAL before and after — no alias
gap there. Mutation: the FROM-alias lookup in `qualified_rewrite` disabled (the
qualifier resolved by `by_alias` alone) reds
`qualified_wildcard_under_another_alias_serves_user_columns` (at X3),
`qualified_wildcard_near_misses_keep_their_answers` (at P1),
`reserved_name_query_positions` (at N13) and the core leg
`a_qualified_wildcard_under_a_from_alias_expands_to_user_columns`; 33 of 36 pass. A
per-row report under the mutation shows X1, X2, X3, K1, Q1, Q2 and P2 wrong and B1, B2
and W2 unaffected: the bare-`*` fix is a separate `sole_relation_alias` change. The
red commit `8573a684` also removes a duplicate N11/N13 loop left by mcdel-r7 and
moves N12 into `reserved_name_query_positions_that_answer`, where C-015 already
placed it.
...
Logs: `mcdscope-spark.log` (Spark 4.1.2), `mcdscope-repark-before.log` (RePark at
`66f7b647`) and `mcdscope-repark.log` (after the fix, wheel rebuilt), all rc 0. Before the
...
wheel (`mcdqualfix-repark-r9.log`) keeps every r8 row as it was, except M2's RePark
...
statement-global CTE-name check does not mis-scope that shape. Mutations: the
`by_alias` fallback restored in `sole_rewritten_relation` alone reds
`wildcard_over_a_plain_relation_keeps_its_own_columns` (S1) and the core leg; in
`qualified_rewrite` alone it reds `wildcard_over_a_plain_relation_keeps_its_own_columns`
(S2), `qualified_wildcard_near_misses_keep_their_answers` (M2) and the core leg; in
`select_touches_rewrite` alone it reds both scope tests (S1, and S5's subquery `*`, with
`[ICE-MC-1]`). All restored.

**M-22 — mcdel-r10 unreachable-arm sweep (Sol critic r8 V-001, class T).** Step 1
of the tick-57 ruling: with `|| &entry.replacement == name` dropped from
`rewrite_for_relation`, the repark-core lib (752 passed), the repark-spark lib (1891
passed), the facade pytest (23 passed) and both probes on a rebuilt wheel
(`/tmp/xo58-mcd-r10probe/mcdscope-repark-r10.log`, `mcdqualfix-repark-r10.log`, all 18
and 35 rows identical to the r9 logs) stay green. The arm was dead:
`pre_visit_select` makes every wildcard decision before `pre_visit_table_factor`
renames that SELECT's relations, and a later pass over rewritten SQL collects no
Iceberg names. It is removed and `rewrite_for_relation` is `find(name)` over a
`TableFactor::Table`. Class-T mutations, one per branch of the r8/r9 functions
(`/tmp/xo58-mcd-r10probe/mutate.py`, `mutate_post.py`; each run over repark-core
`metadata_columns` plus repark-spark `metadata_columns` and `input_file_name`), each
red at least one test, except two. Those two are removed: `written_qualifier`'s
Derived-alias arm, which only decided between duplicate qualifiers in one SELECT, and
`qualified_rewrite`'s unaliased `entry.alias` branch. That branch equals the qualifier
as written for every routable table: a mixed-case Iceberg table `Tm` is served
metadata columns only through the fully quoted form (`/tmp/xo58-mcd-r10probe/mcdcase-{spark,repark}.log`
T2, EQUAL), where both are the same quoted ident, while the unquoted forms T1/T3
never route (a pre-existing divergence, not changed here). Both lib suites stay green
after the removal (752, 1891). The `QualifiedWildcard` `Expr(_)` arm stays, because
the match must be exhaustive: an expression-qualified wildcard is a parse error on
both engines (`mcdexprstar-{spark,repark}.log` E1, E2).
...
| `build-slot.sh cargo test -p repark-core --lib metadata_columns` | exit 0 — 11 passed |
...
| `build-slot.sh make check-rust-file-size` | exit 0 — 665 + 677 + 929 + 181 lines (`metadata_columns.rs`, `metadata_columns_deleted.rs`, `metadata_columns_reserved.rs`, `metadata_columns_scope.rs`), all under the 1000 ceiling |
