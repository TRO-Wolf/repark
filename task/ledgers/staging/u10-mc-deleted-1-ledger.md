...
expression names …` — DIVERGENT (see M-18). N14 (`WHERE`), N15 (`SELECT *`) and N16
...
| `build-slot.sh cargo test -p repark-spark --lib metadata_columns` | exit 0 — 35 passed |
...
| `build-slot.sh make check-rust-file-size` | exit 0 — 665 + 677 + 803 lines (`metadata_columns.rs`, `metadata_columns_deleted.rs`, `metadata_columns_reserved.rs`), all under the 1000 ceiling |
