...
in `crates/repark-core/src/metadata_columns.rs`). Tests in
...
`pub(super)` helpers of `metadata_columns_deleted.rs`), the one-line module entry in
`crates/repark-spark/src/tests/mod.rs`, and
...
| `build-slot.sh cargo test -p repark-spark --lib metadata_columns` | exit 0 — 36 passed |
| `build-slot.sh cargo test -p repark-core --lib metadata_columns` | exit 0 — 10 passed |
...
| `build-slot.sh make check-rust-file-size` | exit 0 — 665 + 677 + 932 lines (`metadata_columns.rs`, `metadata_columns_deleted.rs`, `metadata_columns_reserved.rs`), all under the 1000 ceiling |
