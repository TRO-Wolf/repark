...
| C-010 | No forbidden surface: diff touches only `python/repark/tests/` (new live file), the one `parity-live.yml` workflow, the northstar, the ledger, maps, plus `Cargo.lock` (`chacha20` yanked → `cargo update`) and `deny.toml` (`thiserror` duplicate `skip`); no `Cargo.toml [patch]`, no AWS/IAM, no secrets, no other workflow | `git diff --name-only origin/main...HEAD` | **PROVEN** | diff allowlist (now includes `Cargo.lock`/`deny.toml`); `check_ledger_grammar` green |
...
| cargo-deny `yanked chacha20` + `duplicate thiserror` | S1 | REMEDIATED — `cargo update -p chacha20` 0.10.1→0.10.2 (yanked), `deny.toml` `skip = [{name="thiserror"}, {name="thiserror-impl"}]` for `multiple-versions = "warn"` |
| wheel `git diff origin/main...HEAD` 128 in `test_v3_live_oracle_pins_cover_all_clauses` | S1 | REMEDIATED — fallback to `git diff HEAD` + skip when not available, allowlist now includes `Cargo.lock`/`deny.toml` for cargo-deny fix |
