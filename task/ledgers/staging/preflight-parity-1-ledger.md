...
| Item | Note |
|---|---|
| No `COVERAGE_ATTESTATION` block | C-005 stays `OPEN` (the PR-body owner question is the orchestrator's departure action), so the unit is in flight and files no attestation yet; the orchestrator writes it at the departure edit. |
