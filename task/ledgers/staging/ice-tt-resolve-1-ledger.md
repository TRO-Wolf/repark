...
- Q-24c-1 — (verification critic L-002) far-future and no-seconds reader `timestampAsOf`
  strings refuse through the engine cast where main read the current snapshot by accident
  (the option was ignored). Accepted as a registered, dated interim: the refusal is loud,
  the cause is the scalar cast (the same gap returns NULL on `CAST` with ANSI off), and
  `CAST-TS-STRING-1` is the fix. The resolver is not special-cased.
...
| C-012 | The size-baseline amendments and the lib-rs row are visible diffs for review (round 2 close ratchets each below main; merged with main at RP-34, `repark-python/src/session.rs` sits at 1122). | Amended rows in `scripts/check_rust_file_size.py`, `scripts/check_lib_py.py`, `scripts/check_lib_rs.py`. | **PROVEN** | `repark-python/src/session.rs` 1127 → 1135 (mandated PyO3 params), `repark-sql/src/guards/tests.rs` 1207 → 1213 and `repark-sql/src/tests.rs` 1520 → 1530 (4-arg `EngineContext::new` plumbing), `session_core.py` 2290 → 2297 (raw-string pass-through), new `repark-core` 154 lib-rs row (shared-resolver re-exports). Flagged in the hand-back for review. |
| C-013 | `TIMESTAMP AS OF` is inclusive at a snapshot's commit time, and the cast-gap residue is registered. | `timestamp_as_of_is_inclusive_at_a_snapshot_commit_time` in `crates/repark-spark/src/tests/time_travel.rs`; registry row `ICE-TT-RESOLVE-1` residue paragraph (Q-24c-1). | **PROVEN** | Verification critic L-001: flipping `<=` to `<` in `snapshot_id_as_of_time` left every Rust test green. The new test resolves the exact commit ms of s1 and s2 to themselves, `s2 - 1 ms` to s1, and `s1 - 1 ms` to the `Cannot find a snapshot older than` refusal; the `<` mutation turns it red. L-002 is ruled Q-24c-1. |
