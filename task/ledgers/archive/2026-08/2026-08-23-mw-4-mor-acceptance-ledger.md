...
**Charter:** [mw-0-charter-ledger.md](../../staging/mw-0-charter-ledger.md) OD-3
...
| C-004 | Expire mutation-proof: CTAS snapshot is unreadable via `VERSION AS OF` after `retain_last=1`. | PROVEN | Dual probe: `require_snapshot_readable` before expire, `require_snapshot_expired` after (`unknown Iceberg snapshot id` + the id, `AnalysisException` only). AST-pinned on both helpers. |
