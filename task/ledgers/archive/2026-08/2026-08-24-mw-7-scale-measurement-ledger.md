...
**Branch:** `feat/mw6-wave` · **Base:** `15fe4e4` (MW-6's departure
...
Four of the five census rates are exact per merge: **+32 data files, +8 delete files
...
at a near-constant rate rather than an exactly constant one. `COUNT(*)` holds 10,000,000 at
every row. Live data bytes 260 MB -> 560 MB; live delete bytes 0 -> 28.6 MB.
...
## Coverage attestation (Critic, reissued at `2aed8dc`)
...
COVERAGE_ATTESTATION:
  ...
  categories:
    - id: AT-1
      status: ATTACKED
      evidence: >
        First pass: recomputed every committed number independently from the run JSONs rather
        than reading the ledger's tables — both census tables cell by cell, the five per-merge
        rates, all 21 MOR/COW ratios, the maintenance walls and result rows, peak RSS against
        getrusage and /usr/bin/time, and the substitution arithmetic term by term. Reissue at
        2aed8dc: re-derived every corrected figure. 3.72 + 0.509 = 4.23 h and 1.91 + 0.509 =
        2.42 h both check; the +13.0 % reconciliation is not just stated but correctly
        attributed — the COW plateau read at 91 s against an actual ~113 s under-projects by
        909.9 s where the total deviation is 892.9 s, so it accounts for the whole gap; the
        five manifest-list deltas quoted match my recomputation exactly; 142.34 s, 4,032 MiB,
        19.6 merges and 39.2 % all check. The MW-9 verdict is rebuilt on the one probe that
        carries it and explicitly declines the one that does not.
      artifacts: [mw7/full.json, mw7/calib.json, mw7/probe1e7.json, mw7/growth.json, mw7/growthmor.json, "ledger sections 1, 4.1-4.4, 5, 6.1-6.8 at 2aed8dc"]
    - id: AT-2
      status: ATTACKED
      evidence: >
        First pass: ran the driver at three shapes the Actor had not, including the tiling
        boundary (merges x rows_per_merge == rows) that exposed F-C7-13. Reissue: the Actor's
        C-011 fixture takes that boundary to its limit — a single data file, 100 % of its rows
        deleted by one MERGE — and adds the size boundary that turns out to be the decisive
        variable, asserted as an explicit precondition against Java's [0.75x, 1.8x] band. I
        drove that boundary from both sides myself (P7a in-band precondition, P7b out-of-band
        behaviour).
      artifacts: [mw7-critic/engine/tile.json, mw7-critic/engine/demo.json, "test_mw7_scale_smoke.py:375-386 band precondition"]
    - id: AT-3
      status: N/A
      justification: >
        Measure-only unit; no crates/ change in either commit. The new pin adds no retry,
        timeout or partial-failure path. The one failure surface touched (remove_orphan_files
        refusing an older_than under the 24-hour floor) was checked against call.rs:978 and
        registry ORPHAN-1 in the first pass and is unchanged by the remediation; section 6.4
        now also states its operational consequence, which is a documentation gain not a new
        failure path.
    - id: AT-4
      status: N/A
      justification: >
        Single process, one leg at a time, no shared mutable state, no concurrency. The one
        load-bearing ordering assumption — the maintenance sequence — is pinned by C-006 and
        is exercised again in full by the new C-011 pin, which drives all five procedures in
        order through measure.maintenance_sequence rather than open-coding them.
    - id: AT-5
      status: N/A
      justification: >
        No privileged action, credential, deserialization of untrusted input or network. Local
        filesystem and a memory catalog. Re-scanned all 13 files the remediation touched for
        absolute home paths, scratchpad paths and session ids: none.
    - id: AT-6
      status: ATTACKED
      evidence: >
        First pass established the mechanism by measurement on both engines and refuted the
        filed one. Reissue: verified the remediation states what was measured and no more.
        F-MW7-1's three refutations each match a run I made. RDF-1's Spark cells match
        oracle.json and granularity.json; its jar constants match my javap reads; its fork
        cites at :66-67, :138-140 and :177 verify verbatim in the checkout at 5e7b2e4. The
        format-v3-track 3b errata is independently checkable from line 33 of the same file —
        the six-file fixture deletes id % 11 = 0, 55 of 600 rows = 9.17 %, below Java's 0.3
        ratio — so the scoping is arithmetic, not assertion. One residual causal slip inside
        RDF-1 filed as F-C7-16.
      artifacts: [mw7-critic/oracle/oracle.json, mw7-critic/oracle/granularity.json, "docs/spark-sql-iceberg-parity.md RDF-1", "docs/design/format-v3-track.md:33,104-116", "fork 5e7b2e4 rewrite_data_files.rs:48,66-67,138-140,177"]
    - id: AT-7
      status: ATTACKED
      evidence: >
        The unbounded-growth finding I filed (F-C7-13) is now carried in four durable places
        rather than one ledger paragraph: F-MW7-1's mechanism and consequence_for_mw8 fields,
        registry RDF-1, fork ask F-16 with a named acceptance flip, and ledger section 6.8,
        which tells an MW-8 author that cadence bounds scan degradation and does NOT bound
        retained dead rows. I re-verified the growth is real at gate scale by running the pin
        and its two provocations: in band the 100 %-dead file is kept, out of band the very
        same file is rewritten. F-MW7-2 (delete bytes +31 %) still reproduces from full.json
        and stays correctly filed as S3.
      artifacts: [mw7-critic/smoke2.log, "P7a/P7b outputs above", "ledger section 6.8", "handoff F-16 Acceptance"]
    - id: AT-8
      status: ATTACKED
      evidence: >
        First pass read every upstream constant out of the 1.10.0 jar's bytecode and the fork
        source rather than presuming it. Reissue: checked that the remediation's upstream
        claims are the ones I measured and that the tree's prior statements were reconciled
        rather than left to contradict them — MW-2's "the zero is a real count" is preserved
        but re-scoped in the tests map so it no longer reads as "delete files therefore survive
        compaction"; format-v3-track 3b takes a dated errata instead of a silent edit; the
        handoff's "Not fork work" MOR-2 bullet is disclaimed so F-16 is not re-proposed as
        MOR-2; and F-16 states that landing F-3 alone does not close RDF-1, which is the
        distinction the oracle actually forces.
      artifacts: ["python/repark/tests/map.md MW-2 row", "docs/design/format-v3-track.md errata", "handoff section 4 MOR-2 bullet", "handoff F-16 Relationship to F-3"]
    - id: AT-9
      status: ATTACKED
      evidence: >
        Both observability gaps I filed are closed: the "sixteen data files" figure the driver
        could not produce is gone, and the delete-file reference question now has a reader —
        _delete_file_references reads file_path out of the delete Parquet itself, with a
        docstring stating why a count cannot answer it. Ran the lockstep gates at 2aed8dc:
        grammar reds on exactly the expected missing attestation and nothing else, so the
        reshaped FINDING block and the C-004, C-011 clause form are accepted; map, lifecycle,
        conventions and docstring presence all clean. Two stale-claim residues found by
        grepping the tree rather than the diff — STATUS.md keeps three superseded S3 figures
        and the C-003 test docstring keeps the deleted claim (F-C7-14, F-C7-15).
      artifacts: [mw7-critic/smoke2.log, "scripts/check_ledger_grammar.py output", "scripts/check_map_md.sh rc=0", "scripts/ledger_lifecycle.py check", "STATUS.md:406,409,411,420", "test_mw7_scale_smoke.py:221"]
    - id: AT-10
      status: ATTACKED
      evidence: >
        First pass: 10/10 green, three mutations run and restored, including the Critic's own
        on C-003 — the control-validity clause the Actor had declined to mutate. Reissue:
        11/11 green at 2aed8dc, and I re-ran the finding's discharge pair myself rather than
        reading the Actor's transcript. P7a reproduces verbatim (seeded file 68523 B is
        outside the bin-pack band for 8192; assert 68523 <= (1.8 * 8192)). P7b reproduces
        verbatim (the 100 %-dead seeded file is still live: it was never a rewrite candidate)
        with the seeded path absent from the live set, i.e. the out-of-band file WAS rewritten
        — size the only variable, which is what makes the pair identify the mechanism instead
        of asserting the outcome. P7b also passes the removed_delete_files_count == 0 assert
        on its way to failing, which independently confirms that counter is not the mechanism.
        No weak pin: each mutation reds its named clause and only that clause. Both restored;
        clone byte-identical, HEAD 2aed8dc, no stash, origin intact.
      artifacts: [mw7-critic/smoke2.log, mw7-critic/test_smoke.2aed8dc.bak, "P7a/P7b outputs above", "git status empty at 2aed8dc"]
  reattested: [F-C7-1, F-C7-2, F-C7-3, F-C7-13, F-C7-4, F-C7-5, F-C7-6, F-C7-7, F-C7-8, F-C7-9, F-C7-10, F-C7-11, F-C7-12, AT-1, AT-2, AT-6, AT-7, AT-8, AT-9, AT-10]
  complete: true
```
