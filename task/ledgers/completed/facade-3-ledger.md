...
**Date:** 2026-09-13 · **Branches:** `feat/facade-3-s1` (C-001..C-008), `feat/facade-3-s2` (C-009..C-013), `perf/facade-3-s3` (C-018..C-027) · **Base:** `23bd047b`
...
**Critic-logic review findings** (`/tmp/oc-worker/f-crit3/report.md`, remediated as
C-027 on `a246df41`; the `Row` index route, the dict key-union fast path and the
named-decline routing came back clean):

| Finding | Class | Status |
|---|---|---|
| L-001 — a `date` subclass overriding `__sub__` reaches the epoch-subtraction fast path and yields `1970-01-01` instead of `2024-06-15` on the tuple, dict and `Row` doors | P1 | Fixed (`0f45f1dc`): the date arm is armed on `type(obj) is datetime.date` (pointer compare vs `Ctx.date_type`); subclasses take the step-2 `timetuple()` route natively |
| L-002 — a `datetime` subclass with a `year` property override reaches the interned-attribute fast path and yields 1999 instead of 2024 | P1 | Fixed (`0f45f1dc`): the interned-getattr wall-clock route is armed on `type(obj) is datetime.datetime`; subclasses take `timetuple()` + `microsecond` natively |
| L-003 — the `utcoffset` cache keyed `tzinfo.as_ptr() as usize` with no reference held, so a recycled address reads a stale offset (`tzinfo` property returning a fresh `timezone(...)` per access → allocator-dependent series) | P1 | Fixed (`0f45f1dc`): cache entries store the owned `Py<PyAny>` of the tzinfo beside the offset for the life of the export call; armed only for exact `datetime.timezone` reached from an exact `datetime` |
| L-004 — the temporal subclass corpus used empty subclasses, so it stayed green on all three P1s | P2 | Fixed (`0f45f1dc`): `SubDateSub`, `SubDtYear`, a `toordinal` override and `FreshTZDatetime` (six rows) added to the parity pin across all three doors; `fresh_tz` asserts the literal base-native series because the forced fallback reads the C-level tzinfo field, not the property |
...
| C-027 | Critic-logic review (`/tmp/oc-worker/f-crit3/report.md`) remediated: L-001/L-002/L-003 (P1, wrong values on temporal subclasses) and L-004 (P2, empty-subclass pin gap) fixed. Fast temporal routes armed on EXACT types only (`type(obj) is datetime.date` / `datetime.datetime` pointer-compared against `Ctx` types); every subclass — pandas `Timestamp` included — extracts natively through the step-2 `timetuple()` + `microsecond` + per-cell `utcoffset` path, never `Cdf::Fallback`. The `utcoffset` cache stores the owned `Py<PyAny>` of the tzinfo beside the offset for the export call's life, so a freed-tzinfo address cannot be recycled into a stale entry; armed only for exact `datetime.timezone` reached from an exact `datetime`. Expanded pin red-first on `a246df41`; `repro_p1.py` outputs match between this venv and `/tmp/f-rev3/base`; re-measure within the +5 % bar (tuples resolved by same-box A/B — box drift, +0.1 %). | `cells.rs` exact-type gates + owned-ref cache; `test_facade_3_cdf_step3.py` corpus; red output, repro diff, and re-measure tables in the step-3 doc + evidence block. | **PROVEN** | One commit `0f45f1dc`. Red-first on `a246df41`: `subclass_date_sub` mismatched on all three doors (1970-01-01 vs 2024-06-15), `subclass_dt_year` on all three (1999 vs 2024), `fresh_tz` on all three (stale uniform offset vs per-access offsets); `subclass_date_toordinal` green (guard — `toordinal` was never on a fast path). Green after `0f45f1dc`: 17 step-3 pins pass. `repro_p1.py` on `.venv` vs `/tmp/f-rev3/base/.venv` — every `CASE` section byte-matches; only the exe/debug/named header lines legitimately differ. Re-measure: tuples 245.24 vs ledger 230.00 is +6.6 % literal but the pre-gate binary measures 245.00 on today's box → +0.1 % (C-022 drift-resolution precedent); rows 240.0 vs 248.54 → −3.4 %; micro cells same-probe A/B +3.1 %/+3.7 %/+0.9 % — all inside +5 %. Version-skew and named-decline behavior untouched. |
...
**C-027 remediation evidence (critic-logic review, branch `a246df41`).**

Red-first on `a246df41` (pre-fix native; the expanded corpus lives inside
`test_temporal_cells_keep_fallback_values`'s case loop — `name door N`
assertion messages — plus the new
`test_fresh_tzinfo_property_applies_per_access_offset`):

```
FAILED test_temporal_cells_keep_fallback_values -
  subclass_date_sub door 0/1/2: date(1970, 1, 1) != date(2024, 6, 15)
FAILED test_temporal_cells_keep_fallback_values -
  subclass_dt_year door 0/1/2: 1999 wall-clock fields vs 2024
FAILED test_fresh_tzinfo_property_applies_per_access_offset -
  door 0/1/2: stale uniform offset, not the per-access series
(subclass_date_toordinal green on all three doors — toordinal was never on a
fast path; the case is a guard.)
```

Green after `0f45f1dc` on a debug native: **17 passed** in the step-3 file
(`subclass_date_sub`/`subclass_dt_year`/`subclass_date_toordinal`/`fresh_tz`
all A/B or literal-match across the tuple, dict and `Row` doors). The
`fresh_tz` case asserts the literal base-native series `[22,22,22,22,3,3]`
(walls minus per-access offsets 2,3,4,5,1,2 h) because the forced Python
fallback's `astimezone`/`utcoffset` are C methods reading the C-level tzinfo
field — they never invoke the overridden property, so the fallback is not a
valid oracle for that case. Verified on `/tmp/f-rev3/base` itself: base native
produces the per-access-offset series, base forced-fallback produces
system-local walls.

`repro_p1.py` A/B (`.venv/bin/python` vs
`/tmp/f-rev3/base/.venv/bin/python`): every `CASE` output section matches
byte-for-byte; only the three header lines differ legitimately (`exe` path,
`debug` flag, `named` symbol presence — the base predates the named export).

Re-measure (release native, `systemd-run --user --scope -p MemoryMax=8G
-p MemorySwapMax=0`, idle builders): tuples @1e5 245.24 ms median of 5
(240.33/246.33/245.24/247.29/236.85) vs ledger 230.00 → +6.6 % literal, but the
pre-gate binary re-measures 245.00 on today's box → **+0.1 %** — the ledger
number is unreproducible on this box state for either binary (same drift class
as C-022's nested artefact, resolved the same way). Rows @1e5 240.0 vs ledger
248.54 → −3.4 %. Micro cells via a rebuilt same-shape `extract_cell` scratch
probe (never committed), same-box A/B pre-gate → post-gate: `date` 16.11 →
16.61 (+3.1 %), naive `datetime` 39.64 → 41.12 (+3.7 %), aware `datetime`
43.08 → 43.48 (+0.9 %). Tables in the step-3 doc's C-027 section.
