...
| C-003 | An unclosed comment that hides TABLE answers Spark's `UNCLOSED_BRACKETED_COMMENT` (WO-C10/C-001 superseded the original tokenizer fall-through claim), and SHOW CREATE TABLE followed by another statement keeps Spark's invalid-statement answer. | Pin both exact outcomes and their parser error class. | PROVEN | `show_create_unclosed_before_table_keywords_use_spark_parse_contract`; `show_create_multi_statement_keeps_spark_invalid_statement_class`; matching facade pins. |
