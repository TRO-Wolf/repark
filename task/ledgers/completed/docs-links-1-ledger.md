...
| C-003 | D-3 baseline: run the script over the tree first, seed `scripts/docs_links_allowlist.txt` with the broken links (one `path:line:link` entry per line, entries only shrink), file the count as residue, fix nothing | PROVEN | Baseline run (before seeding): exit 1 with exactly 10 broken links — 6 missing targets (5 in immutable `task/ledgers/archive/` ledgers, 1 an ellipsis character in a completed ledger) and 4 stale anchors into `docs/spark-sql-iceberg-parity.md` (the hand-written anchors predate the registry headings' FIXED suffixes and drop the `--` a `/` renders as). The allowlist carries those 10 entries one `path:line:link` per line under a `#` header the script tolerates; a malformed entry fails closed (exit 2, pinned); an entry drops exactly its own finding and a stranger link still reds (pinned). Nothing fixed, nothing repaired. Runtime measured 0.82 s whole-tree (0.83/0.81 s repeats; 692 files, 4478 links) — inside the 10 s budget. Audit round 1 turned "only shrinks" into an enforced ratchet: an allowlist entry that matched no finding this run is a gate failure (`scripts/docs_links_allowlist.txt: stale entry <path:line:link> — the link is no longer broken; remove this row`, exit 1, `docs-links: FAIL — 0 broken link(s), 1 stale allowlist entry`), a fully-matched allowlist still exits 0 with the counts line, and the reseeded real tree measured all 10 entries matched (`docs-links: 693 files, 4482 links checked — clean`) — no row removed, none stale. Red first for the new pin: `VIRTUAL_ENV=$PWD/.venv uv run --no-project python -m pytest python/repark-parity/tests/test_dl_6_docs_links.py -q` on the pre-ratchet script → `1 failed, 8 passed in 1.31s` (`test_stale_allowlist_entry_fails` red: the stale entry exited 0); green after: same command → `9 passed in 1.28s`. Audit round 2 (D-9, C-005) then dropped the line from the key: entries are `path:link`, the baseline was re-measured on the merged tree (the same 10 links) and reseeded as 9 entries, and the merged tree's counts line is `docs-links: 694 files, 4489 links checked — clean`. |
| C-004 | D-8 (orchestrator ruling, 2026-09-09): the allowlist ratchets down and a stale entry is a gate failure — an entry that matched no broken link in the run is itself a finding, so a fixed link takes its row away under gate pressure, not by memory | PROVEN | The ratchet is enforced in `run()`: entries are consumed by matching findings, and `allowlist - matched` prints one stale-entry line per survivor before the FAIL summary, exit 1. Pinned red-first by `test_stale_allowlist_entry_fails` (all-good fixture links, one allowlist entry → exit 1 with the stale entry's text and the `0 broken link(s), 1 stale allowlist entry` summary), and the zero-stale half is pinned by the existing allowlist tests plus the real-tree run, which exits 0 with all 10 seeded entries matched. Audit round 2 (D-9) dropped the line from the key, so the stale message names the entry as written (`path:link`) and a line shift can no longer manufacture a stale entry. |
| C-005 | D-9 (orchestrator ruling, 2026-09-09): the allowlist key drops the line number — an entry is `path:link`, a finding is allowlisted when its file path and its link text as written match an entry, the line number stays in the failure output but leaves the key, and the ratchet is unchanged: an entry that matches no finding is still a stale entry and still fails the gate; two identical broken links on different lines of one file collapse to a single entry | PROVEN | The defect was real: merging `origin/main` moved three allowlisted rows in `python/repark/tests/map.md` (1974→1984, 2289→2299, 2302→2312) and the line-keyed gate printed `3 broken link(s), 3 stale allowlist entries` on a tree where nothing about those links changed. Red first for the new pin: `VIRTUAL_ENV=$PWD/.venv uv run --no-project python -m pytest python/repark-parity/tests/test_dl_6_docs_links.py -q` against the line-keyed script → `2 failed, 8 passed in 2.28s` (`test_allowlist_survives_a_line_shift` red: the `path:link` entry is rejected by the old key, and the real-tree pin reds on the moved lines); green after: same command → `10 passed in 1.28s`. The reseed was measured on this merged tree, not hand-edited: with a header-only allowlist the run exits 1 with exactly the same 10 links as the original D-3 baseline, which collapse to 9 `path:link` entries (the ST-1 anchor is broken twice in `python/repark/tests/map.md` and is one entry); the gate then exits 0 with the counts line `docs-links: 694 files, 4489 links checked — clean`. |

## Residue

| Count | Where recorded | Disposition |
|---|---|---|
| 10 broken links measured 2026-09-09 (4 stale anchors into the divergence registry, 5 dead targets in immutable archive ledgers, 1 ellipsis target in a completed ledger), re-measured unchanged on the merged tree under the D-9 reseed | `scripts/docs_links_allowlist.txt`, one `path:link` entry per line — the line is not part of the key, and the 10 findings collapse to 9 entries because the ST-1 anchor is broken twice in one file | Deferred by card D-3 — seeded, not fixed; the allowlist only shrinks as each entry's link is repaired in a unit that owns the file. |
...
Audit round 2 (2026-09-09, one finding): the line-keyed allowlist reded an unrelated PR's line
shift — merging `origin/main` moved three allowlisted rows and the gate printed 3 broken links
plus 3 stale entries on a tree where nothing about those links changed. Ruled D-9 and
remediated as C-005 records: the key is `path:link`, the baseline was re-measured and reseeded
(10 findings, 9 entries after the duplicate collapse), and the merged tree runs green. The
ledger itself moved to `completed/` with the unit's departure; this edit is in place per the
...
COVERAGE_ATTESTATION:
  ...
  categories:
    ...
    - id: AT-6
      status: ATTACKED
      evidence: The holes that would fake a green gate were attacked — a target that exists on disk but is untracked still reds, a stranger broken link stays red beside an allowlisted one, a malformed allowlist entry fails closed (exit 2) instead of silently passing, and, under D-9, the line-shift attack: the same broken link moved to a different line by inserted blank lines still exits 0 under its path:link entry (test_allowlist_survives_a_line_shift), while an entry whose link no longer breaks anywhere still reds as stale.
      artifacts: [python/repark-parity/tests/test_dl_6_docs_links.py]
    - id: AT-7
      status: N/A
