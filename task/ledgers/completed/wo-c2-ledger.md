...
|---|---|---|---|---|
| C-001 | Once `SHOW CREATE TABLE` is recognized, a missing target, bad target, bad `AS` word, trailing text, or tokenizer failure returns `INVALID_STATEMENT_OR_CLAUSE` / `42601` as a `DataFusionError::SQL(ParserError::ParserError(_))` parse path. | Parser unit pins and execute-path pins. | PROVEN | `show_create.rs` and `tests/show_create.rs` pin bare, trailing, invalid-`AS`, and four malformed identifiers. |
