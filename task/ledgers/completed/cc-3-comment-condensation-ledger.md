...
(the D-001 correction applied from the first edit). The two `Cargo.toml` banner blocks in
`crates/repark-ml` and `crates/repark-ta` were condensed to one line each in this tail commit.
