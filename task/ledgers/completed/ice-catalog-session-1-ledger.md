...
COVERAGE_ATTESTATION:
  ...
  categories:
    ...
    - id: AT-10
      status: ATTACKED
      evidence: The cell file pins every clause end to end (41 passed); mechanism pins stand under each slice (USE 16/16, bare_nullary 11/11, q14 88 passed); the suite caught real defects red (DESCRIBE temp-view shadow, catalog_surface_1 refusal shape, premature ALTER token gates); every added branch has a named input — USE spellings, SHOW forms, CACHE forms, both kind aliases, all three table-default/override legs.
