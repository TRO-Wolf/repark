...
| **On main at the run's end** | **22** |
| After #629 FNP-GEN-1 merges (gated green, handed over) | 20 |

**Twelve names are closed on `main`**, and two more are closed on a gated-green branch (#629): `current_time`, `make_time`, `time_diff`, `time_trunc`, `to_binary`, `to_char`, `to_number`, `to_varchar`, `to_timestamp_ltz`, `to_timestamp_ntz`, `typeof`, and `make_timestamp`'s full PySpark signature — plus `window`, `window_time` and `session_window` from the carried-over #618.
...
| **#629** | FNP-GEN-1 | `posexplode`, `posexplode_outer`, `inline`, `inline_outer`; `from_xml` / `schema_of_xml` declared | **Devin SWE-2** ×3 (one round lost to a turn-stall) | critic-logic 4 P1 / Python perf 1 P1 / Rust perf 2 P2; **verification critic: all CLOSED**, 2 new P2 (one raised to P1 by me and fixed) | **gated fully green at `0aa3d69b`, handed over — not merged** (§6) |
...
- **Q-17a-5 — #629 FNP-GEN-1 is gated fully green and handed over, not merged; here is exactly how to finish it.** Final gate on a release native at `0aa3d69b`: **facade 8868 passed / 372 skipped / 34 xfailed, `make verify` 0, clippy 0, the whole parity suite 757 passed, example coverage executed, sizes clean, 0 added comment lines**, pushed. It was not merged because a CI cycle (~45 min) plus a rebase onto #644 and #645, which landed while the gate ran, finishes past this run's 07:15 stop. *To finish it:* in `/tmp/pa-build`, `git rebase --onto origin/main $(git merge-base HEAD origin/main)` — expect conflicts only in `map.md` files, the EX-0 count, `BACKLOG_BASELINE` and the registry, and **take both counts from measurement, not arithmetic** (run the EX-0 test and read its assert; run `check_example_coverage.py` and read its reported backlog). Then re-run `/tmp/oc-worker/ra-gen2/chain629.sh` and re-queue. Verify the diff-of-diffs against `wip/fnp-gen-1-pre-640` before pushing, and check `task/ledgers/staging/map.md` for union-duplicated blocks, which the diff-of-diffs cannot see (§8).
