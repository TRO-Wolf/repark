...
| **MW-4b — S3 Tables MOR leg** (delivered as **MW-10** — measured **allow** 2026-08-30, [ledger](../../ledgers/archive/2026-08/2026-08-30-mw-10-s3tables-mor-ledger.md); OD-3b IAM applied 2026-08-28) | The same helper against S3 Tables. **`[OWNER]` OD-3b:** table-bucket `DeleteObject` on the acceptance role's scratch table bucket. Without it the unit cannot exist; with it the unit is the Glue leg's twin plus the service-compaction conflict-retry path exercised for real (DML-5 / fork contract §8). | S/M | OD-3b |
...
| **DML-B — partition-scoped overwrite** | `INSERT OVERWRITE … PARTITION (…)` static + dynamic and `writeTo().overwritePartitions()` over fork `ReplacePartitions`. **Unblocked (2026-08-29):** fork F-5 answered in #217, already inside the engine pin; static forms route via `OverwriteFiles.overwrite_by_row_filter`, dynamic via `replace_partitions` — recipe in [../epic-term/release-roadmap-2026-08-29.md](../epic-term/release-roadmap-2026-08-29.md) v0.6. | M | none |
