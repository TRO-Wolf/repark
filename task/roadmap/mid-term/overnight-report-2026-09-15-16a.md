...
| After #618 FNP-WIN-1 (gated green, CI green; **not merged** — see ruling 31) | **34** on merge |
...
31. **#618 hands over green rather than merging past the cutoff.** It reached the front of the queue at 19:51, two minutes after #611 FNP-4B merged, and GitHub still reported `UNKNOWN` mergeability, so the merge chain attempted the squash and was correctly refused. #611 touches 15 of #618's files, including `crates/repark-functions/src/lib.rs` (both PRs restructure the analyzer-rule registration). A fourth rebase plus a full re-gate plus a second CI cycle lands near 21:00, past the owner's 20:15 finish (G-3), so the queue line was removed and #618 is handed over gated green at 05fceb17 with CI green. The build clone `/tmp/pa-build` is kept, holding the branch and the `wip/fnp-win-1-pre-630` / `-pre-632` safety branches.

    **To finish it:** in `/tmp/pa-build`, `git rebase --onto origin/main 19ab726e`; expect conflicts in `lib.rs` (keep #618's move of `analyzer_rules` into `registration.rs` and carry any rule #611 added into that function), the map rows and the CAP-1 / size mirrors (union of both sides); then re-run `/tmp/oc-worker/pa-win/chain618h.sh` with `BASE` and `LEASE` updated, and `merge618h.sh` behind it. Verify the diff-of-diffs against `wip/fnp-win-1-pre-632` reduces to the rebase's own lines, as the three earlier rebases did.
...
| C-029 `F.expr` backtick display; C-026 HOF packing through `selectExpr` (16c hand-offs) | functions.py / HOF rewrite | #611 merged 19:49 — ready now |
| **#618 FNP-WIN-1: one rebase onto #611, re-gate, merge** (gated green at 05fceb17, CI green) | `/tmp/pa-build`, branch `feat/fnp-win-1` | next run, first item — ruling 31 has the recipe |
