...
| 15:3x | S3-PATH-WRITE-1: plain Parquet/CSV/JSON path writes to `s3://` are a **v1.5.1 card** (#801; moved into the 1.5.0 target by the owner on 09-23) — reads reach S3 and Iceberg tables on S3 write, the path writer is local staging-and-rename |
