...
## D. Lessons about the PLANNER (the orchestrating session and the plan packets)

- D1 (2026-09-21) — pattern: a packet design ruling written from source reading was overturned by a lane's Spark measurement, twice (INDEX decision 28 `gc.enabled` purge guard, xo-glmflash 09-20; packet IPI-30/31 D-6 `rewrite_manifests sort_by`, xo-muse4 09-21: Spark sorts then packs, the packet said one manifest per key). Both times the lane's challenge was right and the handbook's RULINGS rule (challenge once, with evidence) is what surfaced it. Instruction for the next packet set: every design ruling in a packet carries a tag MEASURED (with the cell or oracle file) or UNMEASURED; the handbook tells lanes that an UNMEASURED ruling is a hypothesis they must measure before building on it, and that a contradiction is a QUESTION, never a silent deviation.

- A16 (2026-09-21 16:59) — HARNESS DEFECT: the shared claims file was truncated by one lane (a '>' redirect or a write-file tool instead of '>>'); 880 lines and every ruling of the day vanished, and my watcher read the drop in question count as "new question". Culprit NOT established: the first surviving line was xo-grok47's 16:59 line, which is suggestive, not proof (its tick transcript had not been written; the Grok log holds no tool arguments) — do not count it against any engine in the evaluation unless the tick artifact shows the write. Rebuilt from the 09-20 freeze copy + the claims tail inside 679 tick prompts (all rulings recovered; some progress lines lost). Tool fix, not an instruction: `_lib/claims-guard.sh` (unit claims-guard) snapshots every 30 s and restores on shrink; next run gives lanes a `xclaim.sh "<line>"` helper that appends under flock and the handbook names it as the ONLY way to write a claim; the watcher must treat a falling question count as file damage, not as a question.
- A17 (2026-09-21 21:00) — HARNESS DEFECT: the `until-<unit>` deadline file only works for drivers started AFTER the 10:14 patch; a bash function already loaded keeps its old body, so xo-muse7 (started 06:35) ignored its extension, finished at 21:00 mid-rebase and wrote a "0 merges" report while two clerk rounds were still running. Restarted with state intact. Rule: after patching xorch-drive.sh, list drivers older than the patch (`systemctl show -p ExecMainStartTimestamp`) and restart each between ticks — or make the driver `exec` itself when its own mtime changes.
- B12 (2026-09-21, from the Grok 4.7 test) — HANDBOOK RULE: a critic PASS on head H carries over a rebase whose diff-of-diffs against H is empty; queue, do not re-critic. Evidence: xo-grok47 got four consecutive PASS verdicts on #783 and merged nothing in 8 h because main moved 7 times and it re-verified every head. Tool: `xreview.sh --reuse <H>`.
- B13 (2026-09-22 07:5x, PR #796 critic r2) — HARNESS DEFECT: a Grok critic round ended after 2 turns with ZERO tool calls ($0.03) and returned NEEDS_REMEDIATION with five fabricated findings (a `command -v xorch-pr` gate, a `{{DIFF_BASE}}` token, a comment at drive.sh:13 — none exist). Rule: a critic verdict whose out.json shows no tool use is VOID, never a verdict either way; relaunch. Tool fix owed: the lane's verdict read (and a future `review.sh --wait`) must check `num_turns`/tool count before trusting `summary`.

## B14 — Opus 5.5 high exhausts a 60-turn tick before writing state (2026-09-22, xo-opus55 tick 1)
Evidence: tick 1 = 61 turns, $3.36, error_max_turns, three claims lines posted, two clones cut, `state-xo-opus55.md` still NEW → counted as a failed tick. Tool fix: `xorch-start.sh` forwards `XORCH_TURNS`; Opus-family lanes launch with `XORCH_TURNS=150`. Handbook instruction for every engine: the state file is written FIRST on a tick, then the work.
