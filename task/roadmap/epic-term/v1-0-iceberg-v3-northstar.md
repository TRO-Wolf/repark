...
| Read: `_row_id` / `_last_updated_sequence_number` | ✅ V3-4 (2026-08-31): Spark-equal on single-table v3 reads, three doors; JOIN/CTE/subquery/time-travel refuse `V3-ROWID-2`; v1/v2 engine Schema `No field named _row_id` (`V3-ROWID-1` FIXED). Preserve-half stays F-7 | served as columns, Spark-equal (single-table) | V3-4 |
...
| Live: Glue + S3 Tables v3 legs | ❌ nothing measured live on v3 (format-v3-track §7). MW-10 is format v2 — it proves the S3 Tables expire permission, not this v3 leg. Evidence: [mw-10-s3tables-mor-ledger.md](../../ledgers/archive/2026-08/2026-08-30-mw-10-s3tables-mor-ledger.md). Measured 2026-08-30: the permission leg is green — [run 33333274383](https://github.com/TRO-Wolf/repark/actions/runs/33333274383) answered PutTableData **allow**; the v3 live legs themselves stay unmeasured. | every green row re-proven live where the service supports it | evidence (intake) (+OD-3b / MW-10) |
| Nightly oracle: v3 leg | ⚠ V3E-5 (2026-08-27) wired `v3-spark-part-dv` and `v3-spark-eq-dv` as a live triple `repark == Spark` (PySpark 4.1.2 + Iceberg 1.11.0), but the CI leg was **red from its first run** (2026-08-28 → 2026-09-01): the live helpers issued `CALL system.register_table` unqualified, which Spark resolved against `spark_catalog` (`MISSING_CATALOG_ABILITY.PROCEDURES`); the 2026-08-27 ✅ rested on the local run only. **Fixed 2026-09-01** (`CALL local.system.…`, JDK 17 local live run 6/6). ✅ returns with the first green nightly after the fix merges | a v3 fixture leg in the nightly, green | evidence (intake) |
