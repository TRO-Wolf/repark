...
| Write: MOR DML via deletion vectors | ⚠ RP-6 (2026-09-01, fork `fb0cacfa`): plain-`WHERE` DELETE and UPDATE Spark-equal (`_row_id` kept, seq advances on the changed row); 🚫 MERGE stays refused (RePark-owned writer reassigns; V3-3 2026-08-30 keep-refusal remains for MERGE) | full DML including UPDATE/MERGE, round-tripped | V3-4 ← fork F-7 |
| Write: COW DML on an adopted v3 table | ⚠ RP-6 (2026-09-01): first and second DELETE Spark-equal on the single-file layout (`next-row-id` 5 then 6); UPDATE Spark-equal; F-rp3-c7 consumed as a two-file-seed artefact; 🚫 MERGE stays refused (owner ruling 2026-08-25; registry `V3-COW-1`; V3-3 2026-08-30 MERGE keep-refusal) | lineage carried per spec | V3-4 ← fork F-7 |
