...
| Write: MOR DML via deletion vectors | ⚠ V3-7 (2026-09-02): MERGE matched-UPDATE/DELETE/INSERT/NMBS/mixed Spark-equal on created and adopted v3 (`_row_id` kept, seq advances on the changed row; MoR matched-UPDATE `next-row-id` 4); RP-6 already lifted plain-`WHERE` DELETE and UPDATE; 🚫 subquery-`WHERE` DML still `V3-COW-1` | full DML including UPDATE/MERGE, round-tripped | V3-7 |
| Write: COW DML on an adopted v3 table | ⚠ V3-7 (2026-09-02): MERGE matched-UPDATE is Spark-equal `(1,0,1),(2,1,2),(3,2,1)` at `next-row-id` 6; matched-DELETE, INSERT, NMBS DELETE, mixed MERGE match the live oracle; RP-6 already lifted first/second DELETE and UPDATE; F-rp3-c7 consumed as a two-file-seed artefact; 🚫 subquery-`WHERE` DML still `V3-COW-1` (owner ruling 2026-08-25) | lineage carried per spec | V3-7 |
