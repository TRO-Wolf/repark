...
| Write: MOR DML via deletion vectors | ✅ V3-9 (2026-09-02): `V3-MOR-1` FIXED — predicate DML's V2-only delete-file gate is lifted, so `DELETE … WHERE` / `UPDATE … WHERE` on v3 write one file-scoped Puffin DV per touched data file on all three doors, created and adopted (`DELETE` `IN`/`EXISTS`/plain → `(1,0,1),(3,2,1)` next-row-id 3 added 0, 1-record DV; `NOT IN`/`NOT EXISTS` → `(2,1,1)`, 2-record DV; `UPDATE` `IN`/plain → `(1,0,1),(2,1,2),(3,2,1)` next-row-id 4 added 1, 2 data files; `write.delete.granularity` is inert on v3); V3-7 lifted MERGE matched-UPDATE/DELETE/INSERT/NMBS/mixed; RP-6 lifted plain-`WHERE` DELETE and UPDATE. v2 MoR still writes Parquet position deletes | full DML including UPDATE/MERGE, round-tripped | V3-9 |
