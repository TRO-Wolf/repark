...
| Write: MOR DML via deletion vectors | ⚠ V3-7 (2026-09-02): MERGE matched-UPDATE/DELETE/INSERT/NMBS/mixed Spark-equal on created and adopted v3 (`_row_id` kept, seq advances on the changed row; MoR matched-UPDATE `next-row-id` 4); RP-6 already lifted plain-`WHERE` DELETE and UPDATE; V3-8 (2026-09-02) measured the MoR subquery-`WHERE` cells (`DELETE` next-row-id 3 added 0, `UPDATE` next-row-id 4 added 1) — 🚫 not served: predicate DML's pre-existing V2-only delete-file gate, not `V3-COW-1` | full DML including UPDATE/MERGE, round-tripped | V3-7 |
