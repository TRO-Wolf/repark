...
| Write: MOR DML via deletion vectors | ⚠ RP-2 (2026-08-27): plain-`WHERE` DELETE on a **DV-free** v3 table commits Puffin DVs the 4.1.2+1.11.0 oracle reads back; DV-carrying tables refuse (measured resurrection), UPDATE/MERGE refuse | full DML, DV-writing, round-tripped | V3-3 ← fork F-13 |
| Write: COW DML on an adopted v3 table | ⚠ RP-2 (2026-08-27): DELETE on a DV-free table lifted — lineage Spark-equal incl. the `next_row_id` allocate-then-suppress; DV-carrying tables refuse (measured resurrection), UPDATE/MERGE refuse (`V3-COW-1`) | lineage carried per spec | V3-3/V3-4 ← fork F-7 |
