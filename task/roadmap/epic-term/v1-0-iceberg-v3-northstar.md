...
| Scale | ✅ SCALE-v3 (2026-09-02): the MW-7 `1e7 x 50` workload re-measured on format-v3 tables at the same knobs (8 identity partitions, 2 % touch, checkpoints every 10, 7 reps, 4 MiB target), both legs, 2:42:36 wall, peak RSS 4,792 MiB. Against the v2 run of 2026-08-24 the merge-on-read leg holds **96 delete files instead of 400 (0.24x)** — a Puffin deletion vector per data file, rewritten in place, so the count stops growing at the seeded data-file count — **496 data files instead of 1,696 (0.29x)**, and reads the point probe at **2,493 ms against 3,878 ms (0.64x)** and `COUNT(*)` at 0.28x, for **1.59x the merge cost** (41.2 s against 25.9 s mean). The full maintenance sequence ends at **zero delete files and zero delete records** where v2 kept 8 files and 10,000,000 records, and the table comes back to **0.61x the copy-on-write control** where v2 stayed at 2.02x — MW-7's F-MW7-1 residue is a v2 statement. `rewrite_position_delete_files` refuses on live DVs (`B-MOR-3`) and `rewrite_data_files` reclaims all 96 by itself (`removed_delete_files_count` 96). Evidence: [scale-v3-mw7-ledger.md](../../ledgers/completed/scale-v3-mw7-ledger.md) §3, run JSON `/tmp/opus-scale-scratch/full-v3.json` (one host, uncommitted, as MW-7 did); the driver knob is `run_mw7.py --format-version 3` | the same measurement on a v3 table | SCALE-v3 |
