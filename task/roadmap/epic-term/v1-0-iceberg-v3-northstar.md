...
| Write: MOR DML via deletion vectors | ⚠ V3-7 (2026-09-02): MERGE matched-UPDATE/DELETE/INSERT/NMBS/mixed Spark-equal on created and adopted v3 (`_row_id` kept, seq advances on the changed row; MoR matched-UPDATE `next-row-id` 4); RP-6 already lifted plain-`WHERE` DELETE and UPDATE; V3-8 (2026-09-02) measured the MoR subquery-`WHERE` cells (`DELETE` next-row-id 3 added 0, `UPDATE` next-row-id 4 added 1) — 🚫 not served: predicate DML's pre-existing V2-only delete-file gate, not `V3-COW-1`; follow-up unit **V3-9** owns that cell | full DML including UPDATE/MERGE, round-tripped | V3-7 |
...
| Live: Glue + S3 Tables v3 legs | ❌ still nothing measured live on v3 (format-v3-track §7), but LIVE-v3 (2026-09-02) wired both legs: `test_v3_dv_dml_maintenance_against_glue` and `test_v3_dv_dml_maintenance_against_s3tables` drive one shared body — opt-in v3 MoR CTAS partitioned by identity `part`, appends to five files per partition, row-scoped `DELETE` (1 Puffin DV), `MERGE` matched-UPDATE + NOT MATCHED INSERT (2 DVs), a `_row_id` / `_last_updated_sequence_number` read, `rewrite_data_files` (12 rewritten / 2 added / 2 delete files removed / 0 DVs left), `expire_snapshots` (14 → 1), and `register_table` of the final metadata location on a second session (Glue only; S3 Tables is the dated gap `S3T-1` / R126). The local engine's numbers are pinned by `python/repark/tests/test_v3_acceptance_local.py`; registry row `S3T-V3-1`. The first `aws-acceptance` nightly (08:43 UTC) or dispatch on merged `main` answers two questions: does Glue reproduce those numbers, and does S3 Tables accept `format-version = 3` at CREATE — a refusal is classified, its masked text recorded, and the leg passes rather than reds. MW-10 is format v2 — it proves the S3 Tables expire permission, not this v3 leg. Evidence: [mw-10-s3tables-mor-ledger.md](../../ledgers/archive/2026-08/2026-08-30-mw-10-s3tables-mor-ledger.md). Measured 2026-08-30: the permission leg is green — [run 33333274383](https://github.com/TRO-Wolf/repark/actions/runs/33333274383) answered PutTableData **allow**; the v3 live legs themselves stay unmeasured. | every green row re-proven live where the service supports it | evidence (intake) (+OD-3b / MW-10) |
