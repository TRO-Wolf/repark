...
| **1.6** | Window-function performance + a fully optimized, bug-free `dynamicFlatten` | (was 1.2) W-0…W-2 from the 2026-08-23 intake, measured against the v1.2 nested suite and the v1.3 spill matrix. **Added 2026-09-12 (owner):** **Windows, macOS and arm support** — the wheel matrix (Linux x86_64 + aarch64, macOS arm64 + x86_64, Windows x86_64), the facade suite on every platform with Linux-only tiers named, and a five-wheel tag pipeline; cards PLATFORM-1…3 in slate 2 (S2-29). Measured: no platform-specific product code, the fork already builds on all three OSes — a CI and packaging job. |
...
| 2026-09-12 | Windows / macOS / arm support — where? | **Slated for 1.6** (owner): cards PLATFORM-1…3 (S2-29). Not started; the current 1.4/1.5 track finishes first. |
