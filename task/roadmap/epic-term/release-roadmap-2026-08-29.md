...
| **2.3** | **CDC ingestion from the connectors** — additional adapters and external feeds | **Re-scoped 2026-09-13:** the first native capture (Postgres `pgoutput`) and the Bronze contract moved to the 1.7 medallion release; 2.3 keeps SQL Server CDC tables, further adapters and optional external-feed compatibility (Stage 7 of the unified plan). Original scope: Postgres logical replication (`pgoutput`) and SQL Server CDC tables → Iceberg `MERGE` with deletion vectors, declared as a sync in `repark.toml`. Builds on the 1.6 connectors (1.10 from the 2026-09-03 renumber until 2026-09-13; was 1.6 before the 2026-09-03 renumber) + RP-2/RP-3 DV merge + 2.2 change semantics. Replaces a Debezium + Kafka + Spark stack with one process — squarely the "no JVM, one box" thesis. |
