...
of v1.6 (database connections; 1.10 until 2026-09-13) and v1.12 (dbt targets; 1.11 until 2026-09-13), so the schema wants to be right early.
...
| Topic | Decision |
|---|---|
| Owner | The loader lives in **Rust** (`serde` + `toml`), per ADR-0004 everything-through-Session, so the v1.13 Spark Connect server (1.12 until 2026-09-13) and v2.0 Flight SQL get it without Python. **Pydantic v2** models are the Python-facing validation / typed accessor — the same Rust-source / Python-mirror pattern already used for `prop_key_is_secret` (`python/repark/src/repark/spark/_secrets.py`). |
...
| Catalog keys | Accept the facade's existing Java-class spellings 1:1 (`impl`, `catalog-impl`, … map to `spark.sql.catalog.<name>.<key>`) for migration fidelity, **and** a native short form `type = "glue" \| "s3tables" \| "memory" \| "rest"` (REST matters for v1.14, 1.13 until 2026-09-13). |
...
|---|---|---|
| **1.5** | FNP fixes + TA fixes and optimizations **+ the Rust-backed facade + named sources** | (was 1.1) Continues the FNP campaign (FNP-15/16 next) and the golden-safe TA perf campaign. **Added 2026-09-10 (owner):** the facade-to-Rust sequence — FACADE-AUDIT-0, then FACADE-1 the Arrow C-stream boundary (pyarrow optional), FACADE-2 `Column` as a Rust expression, FACADE-3 `createDataFrame` inference, FACADE-4 type conversions, FACADE-5 the display renderer — every public name unchanged under the freeze; and **CFG-2 named sources** (`[<profile>.database.*]` parsed, validated, listed, refusing on use), the precondition for the connectors (1.10 then, 1.6 since 2026-09-13) and for the database-discovery proposal's Stage 1. The discovery / profiling / planning product itself ([database-discovery-lakehouse-planning-2026-09-09.md](database-discovery-lakehouse-planning-2026-09-09.md)) stays a 2.3 consumer, not a 1.5 item. **Added 2026-09-12 (owner):** the **deterministic silver-layer compiler** ([deterministic-silver-layer-compiler-2026-09-12.md](deterministic-silver-layer-compiler-2026-09-12.md)) — a typed `SilverPlan` compiled natively to DataFusion/Arrow over a pinned bronze Iceberg snapshot: accepted / quarantined / superseded rows in one classified table with one publication commit, latest-source-version selection that fails on ambiguous ties, exact row accounting, reproducible plan and run identities. Slices S-0…S-6 of its §17; S-0 (contract + storage feasibility, the stage/validate/conditional-commit primitive in the pinned fork) is the first unit and the ten SIL decisions of its §18 are the owner's. Source ingestion, CDC, deletes, joins and other backends stay deferred (its §3). **Charter sharpened 2026-09-12 (owner): 1.5 is the Rust switchover, isolated — "as much Rust switchover as possible; incredibly performant."** FACADE-2…5 land in order; when FACADE-5 is in, the audit's UNMEASURED roll-call ([facade-audit-2026-09-10.md](facade-audit-2026-09-10.md) §6) is chartered measure-first as FACADE-6+ — the UDF bridge modules (`udf_bridge.py`, `udf_projection.py`, `grouped_udf.py`, `functions_udf.py`), `plan_collapse.py`, `joins_columns.py`, the reader/writer modules, the SQL-UDF modules — each unit opening with an isolating measurement and landing only on a measured win under the freeze (the S2-21 reviewers hold the bar). Platform work stays in 1.6; REST in 1.8. **Re-scoped 2026-09-13 (owner):** the silver compiler's S-0 and S-1 (contract, storage feasibility, the plan contract) stay in 1.5 as the foundation; silver S-2 onward moves to the 1.8 medallion release, behind the 1.6 connectors and 1.7 PySpark parity. **Added 2026-09-14 (owner): one-to-one functions and transformations parity with PySpark 4.1.2 ships in 1.5** — every public name of `pyspark.sql.functions`, `DataFrame`, `Column`, `GroupedData`, `Window`, the readers and writers, `Row`, `types`, `Catalog` and `SparkSession` is present on the facade and answers Spark on both doors, or carries a dated declared refusal in the registry (JVM-only, RDD, Structured Streaming and Spark Connect client surfaces refuse with Spark's error class). Census at `main` `629f3d5f` against PySpark 4.1.2: 64 function names missing and about 20 stubbed, 25 `DataFrame` methods, 7 `Column`, 6 `GroupedData`, 12 reader/writer, 13 `Catalog`, 19 `SparkSession`, 13 `types`; plus the registry BACKLOG rows, the FNP-Z residues and the SQL-door divergences. The FNP campaign closes inside this release (FNP-11 temporal family, FNP-12 `typeof`, FNP-13/14, FNP-4b expression strings, FNP-6d, FNP-Z); the 1.7 row of 2026-09-13 folds in here and the later rows move up one. |
| **1.6** | **Postgres and SQL Server native connectors** + Windows / macOS / arm / Python 3.13–3.14 | **Moved up from 1.10 on 2026-09-13 (owner): the operational databases come before the rest of the medallion builds.** The unified plan's Stage 1 ([unified-database-query-cdc-silver-plan-2026-09-13.md](unified-database-query-cdc-silver-plan-2026-09-13.md) §15, §2 "Session and native connector contract", D-15): `[<profile>.database.postgres.*]` and `[<profile>.database.sqlserver.*]` sources from CFG-2 become live — Session registers them in the one federated namespace, `repark.sql()` reads and joins them with Iceberg tables, `EXPLAIN` shows the pushdown boundary per source, the crawler's discovery surface reads their catalogs without side effects, specifications are versioned and immutable. Pure Rust, no JVM: `tokio-postgres` / `sqlx`, `tiberius` (TDS); the read bar stays ConnectorX's (partitioned parallel reads into Arrow, benchmarked against ConnectorX and pandas+SQLAlchemy); writes keep the 2026-08-29 ruling (bulk path default with a per-call `bulk` / `row` flag). **Standing instruction to every orchestrator on this release (owner, 2026-09-13): reference ConnectorX and Arrow ADBC** — ConnectorX for the partitioned-read design and as the benchmark bar, Arrow ADBC for the driver-level Arrow contract (type mapping, bulk ingest, cursors), each unit's ledger citing which of the two shaped its design; and the no-code-comments ruling stands unchanged. Separate query pools from capture connections from day one, so 1.8's CDC cannot starve a query. Trino moves to 1.11. The platform cards PLATFORM-1…4 (S2-29) stay in 1.6 as ruled on 2026-09-12; PLATFORM-1 already merged in the 1.5 window. |
| **1.7** | **The medallion release** — Bronze ingestion, native CDC, Silver execution and publication | **New on 2026-09-13 (owner); 1.7 for a few hours that evening, then 1.8 when PySpark parity was pulled ahead of it.** The unified plan's Stages 2–5: Silver execution from contract-compliant Bronze fixtures (S-2 onward of the [silver compiler](deterministic-silver-layer-compiler-2026-09-12.md), under the SIL-1…10 rulings); durable sequential historical publication — stage both outputs, publish current, publish history, record the completed snapshot pair (the two-table verification ledger's C-001…C-013); the first native RePark CDC connector (Postgres logical replication, `pgoutput`) and the Bronze ingestion adapter with the versioned event envelope (D-03); operational qualification — Trino and Spark read both tables through the qualified catalog, retention, maintenance, backpressure, restore. SQL Server CDC and further adapters follow as Stage 7 (2.3). Depends on 1.6's connectors and shares their source identity, type maps and credentials. |
| **1.8** | One-to-one **I/O parity with Polars** | (was 1.3; 1.7 then 1.8 on 2026-09-13) Excludes non-native Polars integrations (PyIceberg, Delta via `deltalake`), Hive metastore, Unity, HuggingFace. **Hive-partitioned directory reads are in** (DataFusion provides them). Absorbs two named differentiators — first-class Excel read/write and the smart-CSV / inference readers — name them explicitly in the charter. |
| **1.9** | One-to-one **functions, expressions, transformations parity with Polars** | (was 1.4; 1.8 then 1.9 on 2026-09-13) With 1.7, this is the **native lazy door maturing** far past today (`scan_*` / `sink_*`, expression API). State that scope in the charter — it is the real size of the item. **The Polars leg of the cross-engine function matrix lands here as the proof** (ruled R-2). **Added 2026-09-12 (owner):** the **generic REST catalog campaign** — REST-0…REST-4 and the fork cards F-REST-NSPROPS-1, F-REST-IO-SCHEME-1, F-REST-CREDREFRESH-1 (F-REST-SIGV4-1 deferred without a date) from [../mid-term/rest-catalogs-intake-2026-09-12.md](../mid-term/rest-catalogs-intake-2026-09-12.md) — Lakekeeper and Apache Polaris through the fork's already-complete `iceberg-catalog-rest`; target **1.8 (tentative — the owner said "may")**, moved from 1.13 where only "REST first-class" was named; 1.13 keeps multi-writer. |
| **1.10** | **Trino** integration + window-function performance + `dynamicFlatten` | (was 1.6; 1.10 until 2026-09-13; **Postgres and SQL Server moved up to 1.6 on 2026-09-13**, Trino stays here and inherits the window-function performance + bug-free `dynamicFlatten` item that 1.6 carried — W-0…W-2 from the 2026-08-23 intake, measured against the v1.2 nested suite and the v1.3 spill matrix) **Acceptance includes a federated statement**: one `repark.sql()` joining an Iceberg table, a Postgres table and a Trino table, with `EXPLAIN` showing the pushdown boundary per source. Pure Rust, no JVM: `tokio-postgres`/`sqlx`, `tiberius` (TDS), Trino HTTP. **Read bar** = ConnectorX's design: partitioned parallel reads on a partition column straight into Arrow; acceptance is a benchmark of the same query vs ConnectorX and pandas+SQLAlchemy, within a stated factor of ConnectorX. **Writes**: bulk path default (`COPY … BINARY`, TDS bulk insert, batched Trino `INSERT`) with a per-call `bulk` / `row` flag; row mode is the fallback for types bulk cannot carry. Reference ConnectorX and ADBC. Connections come from v1.4. |
| **1.11** | Full dbt support | (was 1.7; 1.11 until 2026-09-13) `dbt-repark` (sibling repo; M0–M2a merged, AWS gates owner-scheduled). Targets can read v1.4 profiles. |
| **1.12** | **Spark Connect server** | (was 1.8; 1.12 until 2026-09-13) Unmodified `pyspark` clients via `spark.remote(...)` — zero import changes. Built owned, not Sail. This is a gRPC daemon with sessions, cancellation and per-query resource policy, so it **fires the ADR-0005 session-decomposition trigger here, not at 2.0**: build the server core once (session manager, cancellation, resource policy), Spark Connect is protocol #1, Flight SQL at 2.0 is protocol #2 on the same core. PROJECT.md's "no daemon" becomes "no daemon *required*". |
| **1.13** | **Multi-writer Iceberg** + REST catalog first-class | (was 1.9; 1.13 until 2026-09-13) Lift the single-writer-per-table rule (OCC retry policy, serializable isolation done right); REST alongside Glue / S3 Tables. Correctly placed *before* the 2.0 freeze — standing-rule changes go before the API promise, not after. |
...
| Ver | Item | Notes |
|---|---|---|
| **2.0** | **Arrow Flight SQL endpoint** — the headline | Protocol #2 on the 1.13 server core (1.12 until 2026-09-13). Unlocks JDBC/ODBC drivers, BI tools, non-Python clients, a standalone `repark` binary/shell. |
...
| **2.3** | **CDC ingestion from the connectors** — additional adapters and external feeds | **Re-scoped 2026-09-13:** the first native capture (Postgres `pgoutput`) and the Bronze contract moved to the 1.8 medallion release (1.7 for a few hours on 2026-09-13); 2.3 keeps SQL Server CDC tables, further adapters and optional external-feed compatibility (Stage 7 of the unified plan). Original scope: Postgres logical replication (`pgoutput`) and SQL Server CDC tables → Iceberg `MERGE` with deletion vectors, declared as a sync in `repark.toml`. Builds on the 1.6 connectors (1.10 from the 2026-09-03 renumber until 2026-09-13; was 1.6 before the 2026-09-03 renumber) + RP-2/RP-3 DV merge + 2.2 change semantics. Replaces a Debezium + Kafka + Spark stack with one process — squarely the "no JVM, one box" thesis. |
...
| **2.7** | **Observability** | OpenTelemetry traces + metrics from server and engine, a query-history table, `EXPLAIN ANALYZE` with per-source timings and bytes (extends the 1.6 pushdown-boundary `EXPLAIN`; 1.10 until 2026-09-13), spill / memory telemetry from v1.3. Prerequisite for 3.0 quotas and audit — you cannot govern what you cannot measure. |
...
| 2026-09-12 | Silver-layer compiler in 1.5 | **The deterministic silver-layer compiler joins 1.5** as a campaign ([deterministic-silver-layer-compiler-2026-09-12.md](deterministic-silver-layer-compiler-2026-09-12.md)): S-0 opens first (contract, supported-type matrix, and proof that the pinned fork can stage, validate and conditionally commit one classified table); SIL-1…SIL-10 are open owner decisions; the original generator brief is not yet filed. |
| 2026-09-12 | Generic REST catalogs (Lakekeeper, Polaris) — where? | **All REST catalog work joins the roadmap at target 1.8 (tentative).** The intake's slate and its eight decisions stand as recommended pending the owner's word on REST-D-1…7 (Lakekeeper first fixture; vended credentials in slice one; no default delegation header; `catalog-impl` suffix rule; SigV4 out; explicit storage-scheme key; the sixth patch line needs no ruling); REST-D-8 is answered by the 1.8 placement. Not started: the current track (1.4 cut, 1.5 facade + CFG-2 + silver compiler) continues first. |
| 2026-09-12 | Windows / macOS / arm support — where? | **Slated for 1.6** (owner), with **Python 3.13 and 3.14** added the same day: cards PLATFORM-1…4 (S2-29). Not started; the current 1.4/1.5 track finishes first. |
| 2026-09-12 | 1.5 charter | **1.5 stays isolated as the Rust switchover — as much as possible, performance the goal.** FACADE-2…5 first; then the audit's unmeasured roll-call becomes FACADE-6+ measure-first (a unit lands only on a measured win); the silver compiler's core, PERF-UNPIVOT-1 step 2, PERF-CAST-1 step 2 ride the same release. OS/interpreter support stays 1.6; REST stays 1.8. Run 9 (2026-09-12 evening) executes the first slices. |
| 2026-09-13 (evening) | PySpark parity before the medallion (owner) | **One-to-one PySpark functions and transformations parity moves from 1.10 to 1.7**; the medallion release becomes 1.8, Polars I/O parity 1.9, Polars functions parity 1.10; 1.11 onward unchanged (1.10's occupant moved up, so nothing below shifts). FNP-11 timestamp/TIME, FNP-13 collation and the 2,509-pin cohort sweep now sit in 1.7. |
| 2026-09-14 (evening) | Spark parity in 1.5 (owner: "full Spark parity is in the 1.5 release, every function we need, in any shape that we need") | **One-to-one PySpark functions and transformations parity moves from 1.7 into 1.5**, chartered on the 2026-09-14 census; the medallion release is 1.7 again, Polars I/O 1.8, Polars functions 1.9, Trino 1.10, dbt 1.11, Spark Connect 1.12, multi-writer + REST 1.13. Run 15 (four Opus orchestrators over GLM, Muse, Grok and Devin workers) opens the campaign the same night. |
| 2026-09-15 (morning) | Run 15 and run 14 owner questions (owner: "Go with recommendations") | **Every recommendation stands as ruled:** RustCrypto `aes` / `aes-gcm` / `cbc` / `ecb` approved for the AES functions; `orc-rust` approved for a read-only ORC scan in 1.5, XML stays a dated refusal; port JDK 17 `FloatingDecimal` for byte-identical DOUBLE text; runtime `SET` of ANSI mode and the session time zone applies to the live session; facade `degrees` / `radians` move onto the Rust kernel; `make_timestamp` widens to Spark's signature in a freeze-update PR; the `__all__` uniqueness pin; a second build clone per orchestrator above 250 G free; merges across concurrent runs ordered through a shared queue file (the GitHub merge-queue setting is a later owner change); size baselines stay ratchet-only; no global retag wrapper, DOOR-CONVERGE-2 fixes per function with the `concat` / `reverse` / `sequence` / `split` wrong answers first as P1; FNP-13 collation design note after FNP-4B; TYPES-GEO-DDL-1 is a 1.5 Rust card on the type table; `inputFiles` keeps Spark's `[]` on Iceberg with a registry note; `format("jdbc")` URL dispatch is 1.6 connector work with a registry row now; the `CommitStateUnknownException` name stays (Q-R14). Run 16, the 12-hour day run of 2026-09-15, carries them with Muse Spark 1.3 contributor as the builder tier on every actor round (owner). |
| 2026-09-15 (evening) | Run 16 owner questions (owner: "Go with recommendations" / "Proceed with recommendations"), v1.4.2, the actor tier mix | **v1.4.2 cut from main** after DECIMAL-CACHE-1 (#632): decimal arithmetic that overflows 38 digits types like Spark and materializes through eager/cache/persist. Rulings: posexplode's second output is `col` (Q-16a-1); the second build clone opens above 200 G free (Q-16a-2); FNP-4B closes the `semanticHash` literal gap (Q-16b-1); a second build clone may share a cargo target only under a lock (Q-16b-2); LOGICAL-WIDTH-1 scheduled for the facade's smallint/tinyint/float/binary widths (Q-16b-3); IO-TEXT-PART-POOL-1 follow-up (Q-16b-4); `fancy-regex` approved as the fallback engine for Java regex features (Q-16c-1); ICU4X `icu_collator` approved for collation slice 13c (Q-16c-2). Run 17 (the night of 2026-09-15) runs Devin SWE-2 on at least two units per orchestrator and Muse contributor on the rest, with a tier comparison in each report. |
| 2026-09-16 (morning) | Run 17 owner questions (owner: "all of the recommendations are solid"), the actor tier for the day run | **Every run-17 recommendation stands as its report wrote it.** Process: the Rust-first rule gains one sentence — a decision that raises, casts, coerces or branches on a value is a kernel too, and if the SQL door cannot reach it, it is in the wrong place (Q-17a-2); every gate runs the whole facade and parity suites, a chain that runs a subset is fixed before use (Q-17a-4); a worker brief states the commit as its own imperative numbered step (Q-17b-1); mixed-tier slates split by card clarity, clearest card on Devin (Q-17b-5). Box: `target/debug` dropped after every gate that pushes (Q-17a-3); `LimitNOFILE=65536` on every launch (Q-17b-2); the lane launcher proceeds after ten minutes of box-wide cargo activity with a logged line (Q-17c-6). Product, 1.5: #625's two door-split P1s open run 18a (Q-17a-1); BL-20, the SQL door's integral literal typing, is a 1.5 card and run 18c's first unit (Q-17c-2); BL-19, `UNRESOLVED_ROUTINE` as the blanket unknown-function refusal, is the second (Q-17c-3); the Rust panic that reached the Python boundary under memory pressure is the defect card NEVER-OOM-PANIC-1 (Q-17c-7); SUBQ-CELLS-1 is a 20-minute follow-up (Q-17b-3); #629 finished by the orchestrating session and merged 33c87cbf (Q-17a-5); the stale 40 G clone removed (Q-17b-4). Deferred: sub-minute fixed offsets stay a dated DECLARED divergence in 1.5 with a 1.6 card carrying offsets as seconds east of UTC (Q-17c-1); a shared leaf crate waits for a third duplicated case (Q-17c-4); the BL-11 encoder cost is a perf card, not a blocker (Q-17c-5). **Day run 18 (2026-09-16, about 09:00 to 21:15 EDT) runs Muse Spark 1.3 contributor as the actor tier on every unit** (owner: "switch the worker agents back to muse for today"); Grok stays the reviewer tier. Morning board: `docs/artifacts/parity-run-17-morning-2026-09-16.html`. |
| 2026-09-13 | The operational databases before the rest of the medallion (owner, with the [unified plan](unified-database-query-cdc-silver-plan-2026-09-13.md)) | **Postgres and SQL Server native connectors move from 1.10 to 1.6** (the plan's Stage 1: Session sources, live and federated query, the crawler's read surface, versioned specifications; Trino stays behind). **1.7 becomes the medallion release** (re-cut to 1.8 the same evening, see the row above; Stages 2–5: Silver S-2 onward, sequential historical publication, the first native CDC + Bronze ingestion, Trino/Spark qualification); silver S-0/S-1 stay in 1.5 as its foundation. Former 1.7–1.13 renumber to 1.8–1.14 (superseded the same evening by the row above); former 1.6's window-function performance + `dynamicFlatten` rides with Trino at 1.11; PLATFORM-1…4 stay in 1.6. The 2026-09-10 standing recommendation (Spark Connect first after 1.5) is superseded. **Open at the 1.6 cut:** where REST catalogs sit now that the tentative 1.8 slot is the Polars-functions release; SQL Server *capture* (2.3) versus SQL Server *query* (1.6) is the intended split. |
