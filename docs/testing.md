...
> **Row-2 spelling note (re-home landed, 2026-08-14).** `repark.sql()` is the shipped ANSI-door
> *callable* (`from repark import sql`; `import repark.sql` is not a module). The PySpark facade
> and the `pyspark` → `repark.spark` sed-swap alias live at `repark.spark` / `repark.spark.sql`
> (docs/design/python-facade.md Q1). The native lazy DataFrame API is not this phase.
