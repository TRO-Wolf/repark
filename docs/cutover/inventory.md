...
| # | Question | Ruling | Follow-through |
|---|---|---|---|
| 5 | Gold rebuild on Glue: the fork's Glue catalog has no replace-publish path, so a second `dbt run` fails | **Block C6 on the fork fix and a repin.** No interim route. | `F-GLUE-REPLACE-1` + `RP-20` — **FIXED at pin `edc38c6a`, live pending**: `GlueCatalog::publish_replace_table` is a version-id-checked `UpdateTable`; `ICE-GOLD-TWICE-1` put the replace-twice legs and the gold module (unique stem, two `dbt run` passes, `dbt test`) in `aws-acceptance.yml`. The dispatch's run id and per-leg counts record here when it lands. |
