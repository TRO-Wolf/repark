...
| # | Question | Ruling | Follow-through |
|---|---|---|---|
| 5 | Gold rebuild on Glue: the fork's Glue catalog has no replace-publish path, so a second `dbt run` fails | **Block C6 on the fork fix and a repin.** No interim route. | `F-GLUE-REPLACE-1` + `RP-20` — **FIXED at pin `edc38c6a`, live-proven 2026-09-14 (aws-acceptance run 34901483202)**: `GlueCatalog::publish_replace_table` is a version-id-checked `UpdateTable`; `ICE-GOLD-TWICE-1` put the replace-twice legs and the gold module (unique stem, two `dbt run` passes, `dbt test`) in `aws-acceptance.yml`. Run 34901483202 (`workflow_dispatch` on `main` `0b33f5b7`, success): silver module 10 passed in 308.55 s, including `test_create_or_replace_twice_against_glue` and `…_against_s3tables`; dbt gold module 1 passed in 36.19 s (two `dbt run`, `dbt test` 10 blocks). |
| 6 | Who creates the shadow tables | **Spark**, from the production DDL and properties, so RePark meets Spark-created metadata before C4. Writing into Spark-created tables is a standing requirement beyond this cutover. | `SHADOW-1` (pipeline side) creates the shadow tables with Spark; `ICE-SPARK-TABLE-1` pins the shape here — landed 2026-09-14, hardened by a Grok critic-logic round the same day: adopt + two MERGEs + maintenance CALLs + Spark read-back green, with the stale-cache `refreshTable` requirement after repark commits (catalog-agnostic; C4 is Glue, not the Hadoop version hint) and the transform-sort refusal named in the registry. |
| 7 | Candidate revision and live acceptance | `main` after the Glue replace fix lands; aws-acceptance dispatch approved, the dbt Glue module joins the workflow. | Dispatched on `main` `0b33f5b7` after the Glue replace fix: aws-acceptance run 34901483202, 2026-09-14 21:55:59Z → 22:27:04Z, success, no environment wait. |
| 8 | Readers, optimizers, retries | Spark and Trino read silver and gold; Glue optimizers off; Airflow may retry MERGE and CTAS IF NOT EXISTS only, never an append, and alerts on `CommitStateUnknown`. | Check 4 re-scoped above; `ICE-COMMIT-UNKNOWN-1` landed the alert class: an ambiguous commit raises `repark.errors.CommitStateUnknownException` (a `PySparkException` subclass) carrying the commit's `engine.operation-id` as `exc.operation_id` (`None` on the staged Glue publish path, which mints no id), and a service-managed CTAS leaves the possibly-landed table in place rather than abort-dropping it. |
...
and the dbt gold module to the workflow; the first run, 34901483202 on `0b33f5b7`, passed every leg:
10 silver legs (308.55 s) and the gold module (36.19 s).
