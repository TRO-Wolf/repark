...
| Id | Capability | Evidence | Qualification | Status |
|---|---|---|---|---|
| R1 | **Scans with deletes.** v2 and v3 tables, partitioned or not; position deletes, equality deletes and Puffin deletion vectors applied; `count(*)` folds on delete-free snapshots. | North star §3.1 rows 1–2: `crates/repark-spark/src/tests/v3e3.rs`, `python/repark/tests/test_v3e3_fixtures.py`. Nightly `python/repark/tests/test_v3_live_oracle.py` over Spark-written fixtures. TORTURE-1 full tier: 1,000,000 rows written, 200,000 deleted, 800,000 read on both doors ([report](../perf/torture-1-2026-09-11.md)). `PERF-ICE-COUNTSTAR-1` FIXED (RP-14). | The Iceberg scan takes no memory-pool reservation (row X2). The nightly live leg was red from 2026-09-05 to 2026-09-11 for a harness reason (NIGHTLY-LIVE-1). | PROVEN |
...
| R4 | **Metadata tables and DESCRIBE.** `snapshots`, `files`, `delete_files`, `manifests`, `history`, `refs`, `partitions`, `entries`, `all_data_files`; `DESCRIBE [EXTENDED\|FORMATTED]` returns Spark's rows. | V3-COV matrix (`python/repark/tests/test_v3_statement_coverage.py`); `crates/repark-spark/src/tests/metadata_tables.rs`; SQL-DESCRIBE-1 (`7db61bfa`, `crates/repark-spark/src/tests/describe_table.rs`). | A `position_deletes` scan refuses (`V3-COV-6`, DECLARED, fork-routed). `SHOW TABLES` hides metadata tables by design (ADR-0006). | PROVEN |
| R5 | **Incremental and changelog reads.** | The facade refuses `start-snapshot-id` and `end-snapshot-id` (`python/repark/src/repark/spark/session/reader.py:950`). The north star (§6) rules both out of the v1.0 gate. | The fork has whole-data-file `IncrementalAppendScan` and `IncrementalChangelogScan` (GAP_MATRIX R122, R123); RePark serves neither. The admitted workloads do not use them. | NOT ESTABLISHED |
...
| W3 | **INSERT OVERWRITE.** Whole table, static `PARTITION (k=v)`, dynamic `PARTITION (k)`, `writeTo().overwritePartitions()`, and `saveAsTable(mode="overwrite")` on an existing table; the files are staged, then swapped in one commit. | `crates/repark-iceberg/src/write/overwrite.rs`, `partition_overwrite.rs`. `DML-1` and `V3-COV-1` FIXED. S4, S8 and S9 `overwritePartitions` rows equal Spark on memory, Glue and S3 Tables (2026-09-04). | The default `write.overwrite.isolation-level` is `snapshot` (`overwrite.rs:22-50`). A concurrent append is not refused, and the overwrite replaces its file. The SILVER-S0 probes show this (C-003, C-007); they ran on a fork probe branch, not the pin. Transform-field static partitions, mixed static and dynamic lists, and an empty dynamic overwrite refuse (`DML-1`, DECLARED residue). | PROVEN |
...
| W6 | **Write distribution and order.** Hash distribution by partition-transform value on CTAS, INSERT OVERWRITE and MERGE inserts; `write.distribution-mode` `none`/`hash`/`range`; `ALTER TABLE … WRITE ORDERED BY / DISTRIBUTED BY / UNORDERED` with a per-writer sort. | `WRITE-DISTRIBUTION-1`, `WRITE-DISTRIBUTION-2` and `WRITE-ORDER-DIST-1` (all FIXED 2026-09-06); `V3-COV-5` FIXED. `crates/repark-iceberg/src/write/distribution/tests.rs`; `python/repark/tests/test_write_distribution_1.py`, `test_write_distribution_2.py`, `test_write_order_dist_1.py`, whose live halves compare with Spark. | `range` is the hash layout plus a per-writer sort, not a global range shuffle (`WRITE-RANGE-1`, BACKLOG). Transform sort orders refuse at DDL, and a write into a transform-ordered table refuses (`WRITE-ORDER-TRANSFORM-1`, BACKLOG). The plain `INSERT INTO` path is row W1. | PROVEN |
...
| Id | Capability | Evidence | Qualification | Status |
|---|---|---|---|---|
| V1 | **Format v3.** Opt-in CREATE and CTAS behind `repark.sql.allowCreateFormatVersion3` (default false); in-place v2→v3 upgrade; row lineage on every served write; merge-on-read DML through deletion vectors; legacy position deletes merged into DVs; `timestamp_ns` and column defaults. | North star §3.1 rows 3–11 and 16–19 with their pins (`python/repark/tests/test_v3_create_opt_in.py`; `crates/repark-spark/src/tests/v3_upgrade.rs`, `v3_mor_dml.rs`, `v3_legacy_delete.rs`, `v3_types.rs`). Live legs `test_v3_dv_dml_maintenance_against_glue` and `_s3tables` in runs 33635288918 and 33699342417. v3 statement coverage: 81 programs ([v3-statement-coverage.md](../design/v3-statement-coverage.md)). | DECLARED exclusions: `V3-GEO-1`; `V3-VARIANT-SHRED-1`; `ENC-1` (encryption keys are stored, never applied); `V3-UPGRADE-V4-1`; `V3-FILEORDER-1`; `V3-ROWID-2`. Athena does not support v3, per the AWS documentation as read on 2026-08-23 (north star §5). The admitted silver tables are v2, so v3 is outside this cutover. | PROVEN |
| E1 | **Schema and partition evolution.** `ALTER TABLE` schema changes through the fork's `UpdateSchema`, partition-spec changes through `UpdatePartitionSpec` (`crates/repark-iceberg/src/write/alter.rs`), and compaction after evolution. | Fork GAP_MATRIX R94 and R95 ✅, with bidirectional Java interop. `RDF-SCHEMA-EVO-1` FIXED at RP-15 with 11 pins (`python/repark/tests/test_rdf_schema_evo_1.py`: add plus spec change, add only, drop, rename, type promotion, v3 with a DV). `V3-COV-2` FIXED (lineage read after a widening ALTER). | This review did not audit a pin for every ALTER spelling; the claim rests on the fork contract and the named RePark pins. `ALTER COLUMN … COMMENT` refuses (`DBT-COLCOMMENT-1`). No cell measures silver MERGE absorbing a new bronze column. | PROVEN |

### Maintenance

| Id | Capability | Evidence | Qualification | Status |
|---|---|---|---|---|
| M1 | **`rewrite_data_files`** (binpack, `where`, options). | North star §3.1 row 12 (`crates/repark-spark/src/tests/call_v3.rs`, `call_v3_dv.rs`). `RDF-1` FIXED for a delete file that names one data file (`crates/repark-spark/src/tests/call_rewrite_dangling.rs`). `RDF-SCHEMA-EVO-1` FIXED. RP-17 and RP-18: rewrite output carries the table codec and no dead dictionary pages; live outputs were 1,839,168 B and 1,755,749 B against 2,840,672 B of zstd-3 input ([re-measure 3](../perf/adapt-part-ap1-remeasure-3-2026-09-12.md)). | `sort` and `sort_order` refuse (`RDF-SORT-1`, DECLARED fork ceiling). A position-delete file that names two or more data files is not selected (`RDF-1` residue, fork F-16 residue 2); that shape exists only on merge-on-read v2. The AP-1 numbers come from synthetic beds on the memory catalog. | PROVEN |
...
| K3 | **An ambiguous commit that an operator can act on.** | `classify_iceberg_error` (`crates/repark-core/src/error_map.rs`) folds `CommitStateUnknown`, `CatalogCommitConflicts`, `DataInvalid`, `PreconditionFailed` and `Unexpected` into one `Error::Iceberg`. That becomes the base `PySparkException` (`crates/repark-common/src/lib.rs:61`, `crates/repark-python/src/exceptions.rs`). `engine.operation-id` is a fresh UUID per commit (`write/append.rs:308`, `write/merge/snapshot_commit.rs:122` and `:334`, `write/overwrite_commit.rs:24`, `write/partition_overwrite.rs:379` and `:413`). No API that returns it was found. | A caller can tell an unknown outcome from a conflict or a validation refusal only by message text. It cannot look up its own commit by operation id. No restart or lost-response drill is recorded. | NOT ESTABLISHED |
| K4 | **Concurrent writers.** | The single-writer rule (inventory §2). Release-roadmap row 1.14 plans multi-writer. `DML-5`. Fork `ENGINE_CONTRACT.md` §8 on S3 Tables service commits. SILVER-S0 C-003 on the overwrite rebase. | No multi-writer acceptance exists. Whether AWS-managed table optimizers write to the production Glue tables was not assessed (Q-ICE-6). | NOT ESTABLISHED |
...
| Id | Capability | Evidence | Qualification | Status |
|---|---|---|---|---|
| I1 | **Spark 4.1.2 reads a RePark-written table.** | The V3-0 audit of 2026-08-21: Spark read RePark's v3 append (858 rows, `_row_id = 1000`) ([format-v3-track.md](../design/format-v3-track.md) §2). The fork's Java interop scenarios run both ways at fork revisions (GAP_MATRIX R94, R95, R98, R114, R158). | No pinned or recorded cell has Spark read the admitted shapes, RePark-written v2 copy-on-write CTAS and MERGE snapshots. The parity-live cutover cells run the same SQL on each engine separately and do not cross-read. Inventory check 4 is unmet. | NOT ESTABLISHED |
...
|---|---|---|
| G-1 | Glue replace of an existing table has no publish path (row C2) — **FIXED 2026-09-14 at `edc38c6a`** (F-GLUE-REPLACE-1 + RP-20); the twice legs and gold twice in `aws-acceptance.yml` (ICE-GOLD-TWICE-1) are the live proof | Was: C6 (gold only per Q-ICE-2). Closed |
...
**RESOLVED 2026-09-14 (F-GLUE-REPLACE-1 + RP-20):** fork #282 landed
