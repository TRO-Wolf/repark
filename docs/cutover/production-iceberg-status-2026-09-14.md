...
"(corrected 2026-09-17 from the 2026-09-16 rating)". The corrected rows, the
rollup line and §11 below are the only changes.
...
|---|---|---|---|---|
| C1 | **Glue.** Register a Glue catalog, resolve `glue_catalog.ns.table`, then create, read, write, run DML and run the maintenance CALLs through the session. | `crates/repark-core/src/session.rs` (Glue arm of `register_catalog_spec`). Runs 33635288918 (2026-09-02) and 33699342417 (2026-09-03). SQL-HARDEN-2 AWS legs, 2026-09-04 ([ledger](../../task/ledgers/completed/sql-harden-2-cow-shapes-ledger.md) §5): S1–S9 replayed on Glue; copy-on-write MERGE left 0 delete files and 1 data file. | The evidence is for 1.0.1-era revisions only. Glue got no metadata or manifest cache at the time (`PERF-CATALOG-AWS-CACHE-1`, then BACKLOG, performance only); since 2026-09-19 (RP-37, ICE-CATALOG-CACHE-1) it receives the session's caches — wired, unmeasured on AWS. Credentials resolve inside the fork through the AWS SDK default chain. Replacing an existing table is row C2. | PROVEN |
...
**Row counts:** 36 rows — 20 PROVEN, 2 PROVEN (run 34901483202: C2, D2), 6 PROVEN in part
(R1, W3, W6, V1, E1, M1), 1 PROVEN offline (K3), 1 PROVEN on local catalogs (I1),
2 DECLARED (R5, K1), 3 NOT ESTABLISHED (C4, X2, I2) and 1 measured failing on the
adopted-Hadoop path only (K4) (corrected 2026-09-17 from the 2026-09-16 rating).
...
| C-8 | `statistics` / `partition-statistics` keys are absent from repark commits. | `docs/spark-sql-iceberg-parity.md:6843` | Holds for tables without prior statistics; on a Spark table with stats both entries carry forward through INSERT, and expiring the owning snapshot removes the entries and files exactly as Spark does (`(0, 0, 0, 0, 1, 2)` on both). Registry sentence rewritten. |
