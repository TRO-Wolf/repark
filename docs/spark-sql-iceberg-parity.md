...
### TZ-3 — a runtime `conf.set` of the session zone validates and applies — FIXED 2026-09-15

- **repark** — the call validates in Rust (Java `ZoneId` semantics) and writes the live
  session: `conf.get` and fresh `current_timezone()` / `from_unixtime` / timestamp-cast
  queries answer the new zone at once. An unresolvable zone refuses
  `[INVALID_CONF_VALUE.TIME_ZONE] … SQLSTATE: 22022` before anything is stored, so the
  session keeps answering the previous zone. `conf.unset` falls back to the registered
  default (`UTC`), applied; `RESET` restores the builder value through the lenient restore
  entry. Frames built before the set keep the snapshot they were analysed under
  (residue SET-ANSI-RUNTIME-2). The pre-fix contract was: accepted, warned once per process,
  neither validated nor stored (extended to the SQL door by SQL-SET-DOOR-1, which did validate).
...
- **Pin** — `python/repark/tests/test_session_timezone_parity.py::test_runtime_conf_set_of_the_session_zone_applies`
  ...
  the reuse path, unchanged); the SQL-door half is pinned by
  `python/repark/tests/test_sql_set_door_1.py::test_set_session_time_zone_key_applies`,
  `…::test_set_time_zone_literal_returns_the_pair`,
  `…::test_set_time_zone_literal_america_new_york_applies`,
  and `python/repark/tests/test_set_ansi_runtime_1.py` (S16-6…S16-11, BTZ5-2/3/5, S5-tz-*)
- **Rationale** — FIXED by SET-ANSI-RUNTIME-1 round 1 (per-query config snapshot through the
  live `SessionContext` carriers plus the session zone snapshot; owner Q-15c-3). The old
  accept-and-warn shape was evidence-driven: refusing the call reds the pinned Apache drop-in
  test `test_create_dataframe_from_pandas_with_dst`, which sets this key through PySpark's own
  `sql_conf` helper. Applying keeps that test green with the engine following each step.
...
- **Rationale** — DECLARED. Serving `LOCAL` would require reading the host machine's zone, a
  ...
  query path. A loud refusal naming the explicit spellings beats a host-dependent silent apply.
  Revisit if a facade-level "name the local zone" facility ever exists.
...
- **Residuals (FNP-6D-FOLLOWUP-1, 2026-09-15).** The followup closes the L-001/L-003
  ...
  to 1) are not re-pinned — the numeric→BINARY leg waits on the runtime ANSI carrier's
  round-2 consumer (SET-ANSI-RUNTIME-1 FIXED the switch; BL-11 is its round 2).
  `concat(BINARY, BINARY)` answers BINARY on the SQL door as Spark does
...
> set and `spark.sql.ansi.enabled` validate in Rust and apply to the live session
> ([TZ-3](#tz-3--a-runtime-confset-of-the-session-zone-validates-and-applies--fixed-2026-09-15)
> FIXED, [SET-ANSI-RUNTIME-1](#set-ansi-runtime-1--a-runtime-set-of-sparksqlansienabled-applies--fixed-2026-09-15)
> FIXED — a stale frame keeps its analysis snapshot), and `SET TIME ZONE LOCAL` is a dated
> DECLARED refusal
...
### SET-ANSI-RUNTIME-1 — a runtime `SET` of `spark.sql.ansi.enabled` applies — FIXED 2026-09-15

- **repark** — `SET spark.sql.ansi.enabled = false` (and the `conf.set` spelling) validates
  in Rust (case-insensitive `true`/`false` only; `1`/`yes`/padded values refuse
  `[INVALID_CONF_VALUE.TYPE_MISMATCH] … SQLSTATE: 22022` before anything is stored) and writes
  the live session's `SparkAnsiConfig`: a fresh `SELECT 1/0` answers NULL double, nullable,
  on both doors. `true` restores `DIVIDE_BY_ZERO` for fresh queries. The flag binds when a
  frame is analysed — a frame built before the SET keeps the raise (oracle S16-0). `RESET`
  restores the builder value through the lenient restore entry; `conf.unset` falls back to the
  registered default (`true`), applied. The pre-fix contract was: stored but never applied.
- **Apache Spark** — `SET spark.sql.ansi.enabled = false` flips the live session's ANSI mode;
  ...
  PySpark 4.1.2; re-measured 2026-09-15 in `fixtures-batch16-dc2rest-setansi.json` S16-0…S16-5.)*
- **Pin** —
  `python/repark/tests/test_sql_set_door_1.py::test_set_ansi_enabled_applies`,
  `python/repark/tests/test_set_ansi_runtime_1.py` (S16-0…S16-5, C-001 F-API, BTZ5-7…10, S5-bool-*)
- **Rationale** — FIXED by SET-ANSI-RUNTIME-1 round 1 (the live path to the built engine's
  `SparkAnsiConfig` this row's BACKLOG named, paired with the runtime-applied session zone;
  owner Q-15c-3). Two narrow residues stay open: a stale frame's `current_timezone()`
  (SET-ANSI-RUNTIME-2) and unparsable string casts under ANSI off (SET-ANSI-RUNTIME-3).

### SET-ANSI-RUNTIME-2 — `current_timezone()` on a pre-SET frame answers the build zone

- **repark** — a frame built under one zone and collected after `SET TIME ZONE` answers
  `current_timezone()` with the build zone. Value-bearing expressions on the same frame
  (`from_unixtime`, timestamp casts) also keep the frame-build snapshot.
- **Apache Spark** — answers `current_timezone()` with the NEW zone on the stale frame (the
  call folds at collect), while the value-bearing expressions keep the analysis snapshot.
  *(oracle: measured — `fixtures-batch16-dc2rest-setansi.json` cell S16-6, PySpark 4.1.2.)*
- **Pin** —
  `python/repark/tests/test_set_ansi_runtime_1.py::test_s16_6_stale_frame_split_binding`
  (pins `UTC` with the residue named in the docstring)
- **Rationale** — DECLARED narrow residue. DataFusion `DataFrame`s hold the `sql()`-time
  `SessionState` clone, so writing the live state gives analysis-time binding for free and
...
- **repark** — **FIXED 2026-09-17** on fork branch `fix/nested-evo-1` (fork PR #292,
  ...
  `Arrow Schema Error … Incorrect number of arrays for StructArray fields, expected 2 got 1`;
  the same for a list element struct (`ADD COLUMN arrs.element.y INT`) and a map value struct
  (`ADD COLUMN m.value.q STRING`), v2 and v3. After the fix every cell answers Spark's rows
  and types on the SQL door and the DataFrame door: `[(1,{a:1,b:None}),(2,{a:2,b:'y'})]`,
  `[(1,1,None),(2,2,'y')]`, `[1]`, `[(1,[{x:1,y:None}]),(2,[{x:2,y:20}])]`,
  `[(1,{k:{p:1,q:None}}),(2,{k:{p:2,q:'z'}})]`. The unaliased `s.a` projection name
  (`<table>.s[a]` where Spark says `a`) is EX-COL-2 and the dotted `col("s.a")` is
  COL-DOTTED-FIELD-1, both BACKLOG; the pins read aliased and through `getField`.
- **Apache Spark** — reads the row written before the evolution with the new child `NULL`.
  *(oracle: recorded — PySpark 4.1.2 + iceberg-spark-runtime-4.1_2.13:1.11.0, Hadoop catalog,
  2026-09-17, `python/repark-parity/fixtures/torture/data/ice_nested_evo_1/oracle.json`, with
  the four Spark-written tables beside it; v2 and v3 identical.)*
- **Pin** — `python/repark/tests/test_ice_nested_evo_1.py::test_adopted_spark_table_reads_match_spark`
  (8 `fork292` cells, both doors), `…::test_nested_ddl_rows_match_spark` (`fork292` ids),
  `crates/repark-spark/src/tests/nested_column_ddl.rs::fork292_nested_add_reads_null_for_rows_written_before`.
- **Rationale** — FIXED. A spec-legal Spark schema evolution made the table unreadable.

### ICE-NESTED-DDL-1 — nested `CREATE TABLE` and `ALTER TABLE … ADD / RENAME / DROP COLUMN` on a nested path — **FIXED 2026-09-17**

- **repark** — `CREATE TABLE t (id INT, s STRUCT<a: INT, b: STRING>, arrs ARRAY<STRUCT<x: INT>>,
  m MAP<STRING, STRUCT<p: INT>>)` creates the Iceberg struct, list and map types, and `DESCRIBE`
  answers Spark's `struct<a:int,b:string>`, `array<struct<x:int>>`,
  `map<string,struct<p:int>>`. `ALTER TABLE t ADD COLUMN s.c BIGINT`, `ADD COLUMN
  arrs.element.y INT`, `ADD COLUMN m.value.q STRING` (also `ADD COLUMNS (…)` lists,
  `COMMENT`, `FIRST` / `AFTER sibling`), `RENAME COLUMN s.a TO a2` (field id kept) and
  `DROP COLUMN s.b` (also `DROP COLUMNS … `, `IF EXISTS`) evolve the schema through the
  fork's `UpdateSchema` — `add_column_to` with the parent path, `rename_column`,
  `delete_column` — never a RePark-side schema model; reads and `DESCRIBE` then answer
...
- **Apache Spark** — creates and evolves the nested columns; the same `DESCRIBE` strings and
  ...
  `IllegalArgumentException`: `Unsupported table change: Incompatible change: cannot add
  required column: r`. *(oracle: the same recording.)*
- **Pin** — `python/repark/tests/test_ice_nested_evo_1.py::test_nested_ddl_schema_matches_spark`
  (every DDL cell, inserts skipped, both doors), `…::test_nested_ddl_rows_match_spark`,
  `…::test_required_nested_child_refuses_like_spark`,
  `crates/repark-spark/src/tests/nested_column_ddl.rs` (CREATE / DESCRIBE, evolution by
  field id, required refusal, malformed paths, recognizer fall-through),
  `crates/repark-sql/tests/alter_nested_column.rs::nested_add_rename_and_drop_on_the_ansi_door`,
...
- **repark** — `DELETE FROM t WHERE xs IS NULL`, `xs IS NOT NULL`,
  ...
  cells answer Spark's ok, ids and operation. Every seed runs verbatim, the `map_int`
  seed's `CAST(map() AS MAP<STRING, INT>)` row included (CAST-MAP-SPELL-1, FIXED
  2026-09-19). Two residues, both pinned: copy-on-write DELETE with a
...
- **repark** — **FIXED 2026-09-15 (DOOR-CONVERGE-2).** `concat` is one UDF with
  ...
  any NULL array nulls the row, all-`BINARY` stays `BINARY`, any other mix with
  `BINARY` answers `STRING` (each binary read as UTF-8 text), and an array/string
  mix refuses `DATATYPE_MISMATCH.DATA_DIFF_TYPES` (oracle cells Q12-0…Q12-16;
  round-3 cells Q15-5…Q15-8).
- **Apache Spark** — `concat(array(1), array(2))` → `[1, 2]` (`array<int>`,
  non-null); the full cell table is Q12-0…Q12-16. `concat('A', CAST(X'42' AS BINARY))`
  → STRING `'AB'` (Q15-5…Q15-8). *(oracle: recorded PySpark 4.1.2
  fixtures-batch12, 2026-09-15; fixtures-batch15-critic-622, 2026-09-15.)*
- **Pin** — `python/repark/tests/test_door_converge_2.py::test_sql_cell` (`Q12-0…Q12-16`
  legs), `test_q15_cell` (`Q15-5…Q15-8` legs), `test_facade_concat_arrays`,
  `test_facade_concat_string_binary`, and the `concat_array` / `string` Rust unit pins.
...
- **repark** — **FIXED 2026-09-15 (DOOR-CONVERGE-2).** `sequence` is a registered Spark
  ...
  `DATATYPE_MISMATCH.SEQUENCE_WRONG_INPUT_TYPES`, the plan-time
  `ArrayCardinalityCeiling` still fires for literal stops, and the same cap now fires
  at runtime inside the row builders (closed-form cardinality up front, same refusal
  text) so column stops obey RePark's own `repark.sql.maxArrayElements` policy too
  (oracle cells Q12-26…Q12-40; round-3 cells Q15-0…Q15-4 month stepping from the start,
  Q15-18/Q15-19 cap refusals). Month steps compute element `i` from the start
  (`start + months × i`, Spark `daysAddMonths`), never by chaining. The oracle
...
- **Pin** — `python/repark/tests/test_door_converge_2.py::test_sql_cell` (`Q12-26…Q12-40`
  legs), `test_sql_error_cell` (`Q12-31`, `Q12-32`, `Q12-40`), `test_q15_cell` (`Q15-0…Q15-4`
  legs), `test_q15_error_cell` (`Q15-18`, `Q15-19` legs), `test_facade_sequence_column_stop`,
  `test_facade_sequence_literals`, `test_facade_sequence_month_step`,
  `test_facade_sequence_column_cap`,
...
- **repark** — **FIXED 2026-09-15 (DOOR-CONVERGE-2).** `split` is a registered Spark
  ...
  16a, recorded in the unit ledger. Round 3: `\Q…\E` literal quoting translates in the
  shared compiler (Q15-9); lookahead/lookbehind, backreferences and possessive
  quantifiers refuse loudly naming the feature (Q15-10…Q15-12, shared with
  `regexp_*`/`rlike`); an invalid pattern keeps the `regex`-crate text beside Spark's
  `Unclosed character class` (Q15-13, class under the 16b export hand-off).
...
- **Pin** — `python/repark/tests/test_door_converge_2.py::test_sql_cell` (`Q12-41…Q12-55`
  legs), `test_q15_cell` (`Q15-9`, `Q15-14`, `Q15-16` legs), `test_q15_error_cell`
  (`Q15-10…Q15-13` legs) and the `spark_split` Rust unit pins;
  `test_facade_split_refusal_handoff_16a` guards the Python refusal until run 16a
  wires it.
...
- **Apache Spark** — keeps the header text verbatim as the column name and the read
  completes: the same 10k-row file answers
  `struct<Order Total:string,Qty:int,Note:string,PureBlank:string,StrictFlag:boolean,LooseFlag:string,Small:int,Wide:bigint,Overflow:decimal(20,0)>`
  with `count() = 10000`. *(oracle: live PySpark 4.1.2, zulu-17-amd64, `local[1]`, ANSI on,
  2026-09-10, the torture `smartcsv` CSV at 10k rows, measured by the orchestrator on the
  TORTURE-1 step-2 audit.)*
- **Pin** —
...
- **Apache Spark** — raises on the same file, for `SUM` itself and on both decimal
  columns: `ARITHMETIC_OVERFLOW` / SQLSTATE 22003, "Overflow in sum of decimals. Use
  'try_sum' to tolerate overflow and return NULL instead." *(oracle: live PySpark 4.1.2,
  zulu-17-amd64, `local[1]`, ANSI on, 2026-09-10, the torture `decimal_overflow` parquet at
  10k rows, measured by the orchestrator on the TORTURE-1 step-2 audit; the `avg`/`try_avg`
  half stays recorded in `AVG-DEC-SUMWRAP-1`, oracle 2026-09-05.)*
- **Pin** —
...
### DATE-INTERVAL-NSBOUND-1 — `date + INTERVAL` raises at the whole-day edge; Spark adds days at date width and completes — **BACKLOG 2026-09-10**
...
- **Apache Spark** — completes through the whole-day edge, and **stays `date`**: over the
  same file `day + INTERVAL 1 DAY` has type `date`, `count()` answers all 10,000 rows,
  `max(day)` is `+294247-01-10` and `max(day + INTERVAL 1 DAY)` is `+294247-01-11`. Spark
  adds whole days at **date** width; it does not promote a day-only interval to a timestamp,
  so no microsecond bound is involved on its side at all. *(oracle: live PySpark 4.1.2,
  zulu-17-amd64, `local[1]`, ANSI on, 2026-09-10, the torture `temporal` parquet at 10k rows,
  measured by the orchestrator on the TORTURE-1 step-2 audit. Read the result as a string:
  collecting it into Python raises `ValueError: year 294247 is out of range` from
  `datetime`, which is a Python limit, not a Spark one.)*
- **Pin** —
  ...
  promotion).
- **Rationale** — BACKLOG (2026-09-10), filed by TORTURE-1 step 2; the Spark half measured
  by the orchestrator on the same day, which **narrowed the finding**. The first reading was
  that Spark promotes to timestamp and repark's promotion window ends early; the measurement
  refutes the promotion half — Spark keeps `date` and adds days at date width. What is left is
...
### TYPES-GEO-DDL-1 — `geometry(n)` / `geography(n)` parse in DDL schema strings; column use stays V3-GEO-1 — **FIXED 2026-09-15**

- **repark** — `DataType.fromDDL("g geometry(4326)")` answers
  `StructType([StructField('g', GeometryType(4326), True)])`. `geometry(0|3857|4326|any)` parse and
  `geography(4326|any)` parse (case-insensitive keyword and `ANY`, inner whitespace allowed, nested in
  `struct<…>` / `array<…>` / `map<…>` / field lists); `geography(0)` / `geography(3857)`, the bare tokens,
  unknown SRIDs, `-1` and CRS strings keep the `ValueError: cannot parse datatype` refusal. The spatial arm
  lives in the Rust type table (`parse_atomic_token`); the supported-SRID sets live there once
  (`GEOMETRY_SRIDS`, `GEOGRAPHY_SRIDS` — Python keeps only the JSON door's CRS spellings); the bridge carries
  both tags, and the Arrow mapping refuses spatial variants naming the type instead of the `Utf8` catch-all.
  Column use (`createDataFrame` / `cast` / reader schema / SQL CREATE) still refuses per V3-GEO-1.
- **Apache Spark** — every form above answers `_parse_datatype_string` as pinned, or raises
  `ParseException` `PARSE_SYNTAX_ERROR` for the refusal cells. *(oracle: recorded, PySpark 4.1.2 on JDK 17,
  2026-09-15, cells `G13-0 … G13-20`, copied verbatim to `python/repark/tests/fixtures-batch13-geo.json`.)*
- **Pin** — `python/repark/tests/test_types_geo_ddl_1.py::test_geo_ddl_parse_cells`,
  `::test_geo_ddl_error_cells`, `::test_geo_ddl_bridge_tags_both_ways`,
  `::test_geo_ddl_create_schema_string_refuses`, `::test_geo_ddl_reader_schema_string_refuses`;
  `python/repark/tests/test_types_bases_1.py::test_geometry_ddl_door_answers`,
  `::test_spatial_ddl_door_answers`; `crates/repark-spark/src/type_table/tests.rs`
  (`spatial_ddl_accept_cells_parse`, `spatial_ddl_refuse_cells_keep_parse_error`,
  `spatial_mixed_form_uses_spark_any_marker`, `spatial_arrow_mapping_refuses_naming_the_type`)
...
- **PERF-ICE-SCANPART-1** — surfaced 2026-09-04, PERF-ANALYSIS-1 §2 row 5.
  ...
  **IO-3 round 2 (2026-09-05):** the "measured bounded" claim above is withdrawn. The
  32 MiB budget then bound the fork's ESTIMATED manifest weight (768 B per manifest entry,
  ...
  was unexercised at the estimate (8 MB charged of 32 MB at 8,000 tables). The
  under-count closed as `PERF-CATALOG-CACHE-WEIGHT-1` / `F-CATIO-WEIGHT`. This row
  stays BACKLOG for the metadata-cache LRU only (`F-CATIO-BOUND`).
- **PERF-CATALOG-CACHE-WEIGHT-1** — surfaced 2026-09-05, PERF-ICE-CATALOG-IO-3 round 2.
  **FIXED 2026-09-11 (RP-16)** at pin `090bc821` (fork `#274` F-CATIO-WEIGHT): the shared
  manifest `ObjectCache` charges each retained parsed object graph instead of
  `entries × 768 B` (lists 256 B). Measured on this tree with the pin's 256-table
  `_RSS_ROWS_EACH` shape (bisection over `repark.iceberg.manifestCacheBytes`,
  `/tmp/rp16_weight_bisect.py`): the smallest round budget that retains every table
  after `_delete_manifests` is **1,071,000**; the largest round budget that evicts the
  coldest table is **1,070,000** (~4,184 B charged per small table). The retain pin
  uses **1,250,000** (headroom above the 1,071,000 edge: r0 is a single TinyLFU
  victim and 1,071,000 flaked once under pytest after three isolated retains). The
  old 280000 budget still evicts (192 of 256 missed in the probe). Pins:
  `python/repark/tests/test_perf_ice_catalog_io_1.py::test_a_budget_sized_to_the_charged_weight_retains_every_table`
  (budget 1250000 retains r0 and r255) and
  `python/repark/tests/test_perf_ice_catalog_io_1.py::test_the_old_estimated_weight_budget_evicts_the_coldest_table`
  (budget 280000: r0 misses and the miss names the manifest). History: **BACKLOG**
  behind a fork change. Fork ask **F-CATIO-WEIGHT**: the fork's
  ...
  per table) before any parsed-form overhead. So the 32 MiB default then budgeted
  estimated weight, not resident bytes: a session filling the bound holds 617.5 MB
  resident (602.9 MB peak) against 338.8 MB with the cache off — ~265–278 MB of
  entries above a ~340–352 MB base, ~8× charged (the 32,768-table at-bound run).
  Red-when-fixed pin was
  `test_a_budget_sized_to_the_charged_weight_retains_every_table` (256 tables fit a
  280000 budget at estimated weight, so the coldest table still hits after every
  manifest is deleted; true weights evict it and the leg reds — it redded at this
  bump as designed). Today's ratio is a documented number, not an assertion —
  peak-RSS deltas ride ±20 MB of allocator/phase noise, so no numeric pin could hold
  it. Tables: `docs/perf/iceberg-catalog-io-baseline.md` §6.3.
