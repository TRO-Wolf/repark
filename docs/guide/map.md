...
- [session-and-conf.md](session-and-conf.md) — the `ReparkSession` builder; `getOrCreate` reuse
  ...
  `metadataCacheEntries` / `manifestCacheBytes`, all build-time and session-scoped, reaching memory, Glue and S3 Tables catalogs since ICE-CATALOG-CACHE-1; the entry knob's 64 KiB-per-entry byte budget),
