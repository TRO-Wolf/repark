...
def main() -> None:
    ...
    try:
        ...
        camel = Path("ex26_camel_part")
        frame.write.mode("overwrite").partitionBy("name").option("header", "false").csv(str(camel))
        ...
        snake = Path("ex26_snake_part")
        frame.write.mode("overwrite").partition_by("name").option("header", "false").csv(str(snake))
