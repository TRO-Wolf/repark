...
def main() -> None:
    """Run the measured format and load arms over csv, json, and parquet."""
    repark = ReparkSession.builder.appName("ex26-reader-load").master("local[1]").getOrCreate()
    try:
        with tempfile.TemporaryDirectory() as workdir:
            root = Path(workdir)
            csv_path = root / "ex26_load.csv"
            ...
            by_path_option = (
                repark.read.format("csv")
                .option("path", str(csv_path))
                .option("header", "true")
                .load()
            )
            ...
            parquet_path = root / "ex26_load_pq"
