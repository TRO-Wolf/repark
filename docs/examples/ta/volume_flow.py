"""Demonstrate the volume-flow ``ta`` kernels against the recorded TA-Lib goldens.
...
def main() -> None:
    """Run ``ta.mfi`` and ``ta.obv`` over the ordered fixture window and check bit-for-bit."""
    repark = ReparkSession.builder.appName("ex-ta-volume-flow").master("local[1]").getOrCreate()
    try:
        ...
        rows = frame.select(
            ta.mfi("high", "low", "close", "volume", timeperiod=14).over(window).alias("mfi")
        ).collect()
        expect_bit_exact("ta.mfi", column(rows, "mfi"), golden("mfi_14"))
        rows = frame.select(ta.obv("close", "volume").over(window).alias("obv")).collect()
        expect_bit_exact("ta.obv", column(rows, "obv"), golden("obv"))
    finally:
        repark.stop()
