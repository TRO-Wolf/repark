"""Demonstrate the volatility ``ta`` kernel against the recorded TA-Lib golden.
...
def main() -> None:
    """Run ``ta.atr`` over the ordered fixture window and check the golden bit-for-bit."""
    repark = ReparkSession.builder.appName("ex-ta-volatility").master("local[1]").getOrCreate()
    try:
        ...
        rows = frame.select(
            ta.atr("high", "low", "close", timeperiod=14).over(window).alias("atr")
        ).collect()
        expect_bit_exact("ta.atr", column(rows, "atr"), golden("atr_14"))
    finally:
        repark.stop()
