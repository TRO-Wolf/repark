"""Demonstrate the parabolic SAR ``ta`` kernels against the recorded TA-Lib goldens.
...
COVERS: list[str] = ["ta.sar", "ta.sarext"]
...
def main() -> None:
    """Run ``ta.sar`` and ``ta.sarext`` over the ordered fixture window and check bit-for-bit."""
    repark = ReparkSession.builder.appName("ex-ta-sar").master("local[1]").getOrCreate()
    try:
        ...
        rows = frame.select(
            ta.sar("high", "low", acceleration=0.02, maximum=0.2).over(window).alias("sar")
        ).collect()
        expect_bit_exact("ta.sar", column(rows, "sar"), golden("sar"))
        rows = frame.select(
            ta.sarext(
                "high",
                "low",
        ...
        expect_bit_exact("ta.sarext", column(rows, "sarext"), golden("sarext"))
    finally:
        repark.stop()
