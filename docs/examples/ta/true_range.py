"""Demonstrate the true-range ``ta`` kernels against the recorded TA-Lib goldens.
...
COVERS: list[str] = ["ta.natr", "ta.trange"]
...
def main() -> None:
    """Run ``ta.natr`` and ``ta.trange`` over the ordered fixture window and check bit-for-bit."""
    repark = ReparkSession.builder.appName("ex-ta-true-range").master("local[1]").getOrCreate()
    try:
        ...
        rows = frame.select(
            ta.natr("high", "low", "close", timeperiod=14).over(window).alias("natr")
        ).collect()
        expect_bit_exact("ta.natr", column(rows, "natr"), golden("natr_14"))
        rows = frame.select(
            ta.trange("high", "low", "close").over(window).alias("trange")
        ).collect()
        expect_bit_exact("ta.trange", column(rows, "trange"), golden("trange"))
    finally:
        repark.stop()
