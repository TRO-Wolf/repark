...
def main() -> None:
    ...
    try:
        ...
        if flipped != flipped_expected:
            raise SystemExit(f"Catalog.currentCatalog after register {flipped!r} != {flipped_expected!r}")
        ...
        if cleared != cleared_expected:
            raise SystemExit(f"Catalog.setCurrentCatalog return {cleared!r} != {cleared_expected!r}")
        ...
        if snake_current != flipped_expected:
            raise SystemExit(
                f"Catalog.currentCatalog after set_current_catalog {snake_current!r} != {flipped_expected!r}"
            )
