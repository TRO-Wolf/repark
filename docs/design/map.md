...
- [sql-harden-cutover-matrix.md](sql-harden-cutover-matrix.md) — **SQL-HARDEN-1 (2026-09-04):**
  ...
  v2/v3 copy-on-write of S1/S2/S4 (6 programs; 15 total, 0 EQUAL, 15 DIVERGES). CoW MERGE
  `delete_files` empty and data-file count 1 on both engines; remaining DIVERGES are
  `CUTOVER-CTAS-REQ-1` / `V3-COV-7`. Harness:
  ...
  pins: sql-harden-2-cow-shapes/C-004
...
- [v1-0-api-freeze.json](v1-0-api-freeze.json) — **the frozen-surface register (2026-09-02,
  **U7 PR1 (2026-09-24):** the register gains one Python name, `NumberFormatException` (`python/repark/src/repark/errors.py`, the `IllegalArgumentException` leaf a non-integer `output-spec-id` raises, as Spark does); frozen names 890 → 891, regenerated with `scripts/build_api_freeze.py --write`.
  ...
  members (651 carrying the required-parameter list an AST walk can read), 85 door surfaces with
  their per-door disposition, 5 conf keys, 6 packaging facts, 12 error classes. Generated by
