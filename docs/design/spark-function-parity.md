...
| **FNP-11** | **Timestamp / TIME.** | 18 | Half the kernels exist and RePark owns `TimeType` and the session-zone carrier — but the family is entangled with the open TZ-4 residues and TZ-6/TZ-7/TZ-8, so it needs a design pass, not a wiring pass. **2026-08-31 (FNP-7 critic):** owns `try_avg(INTERVAL)` (and interval-mean overflow). FNP-7 refuses that cell loud rather than emitting a silent NULL or a half interval engine. |
