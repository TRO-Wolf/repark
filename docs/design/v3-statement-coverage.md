...
|---|---|---|---|---|---|---|---|---|
| `create-v3-flat` | create | `CREATE TABLE t (id INT, name STRING) USING iceberg TBLPROPERTIES ('format-version' = '3')` | — | 1 | as Spark | as Spark | **EQUAL** | — |
| `create-v3-partitioned` | create | `CREATE TABLE t (id INT, name STRING, part INT) USING iceberg PARTITIONED BY (part) TBLPROPERTIES ('format-version' …` | — | 1 | as Spark | as Spark | **EQUAL** | — |
| `create-v3-bucket-transform` | create | `CREATE TABLE t (id INT, name STRING, part INT) USING iceberg PARTITIONED BY (bucket(4, id)) TBLPROPERTIES ('format-…` | — | 1 | as Spark | as Spark | **EQUAL** | — |
...
| `create-v3-properties` | create | `CREATE TABLE t (id INT, name STRING) USING iceberg TBLPROPERTIES ('format-version' = '3', 'write.delete.mode' = 'me…` | — | 1 | as Spark | as Spark | **EQUAL** | — |
| `ctas-v3` | create | `CREATE TABLE t USING iceberg TBLPROPERTIES ('format-version' = '3') AS SELECT 1 AS id, 'a' AS name` | — | 1 | as Spark | as Spark | **EQUAL** | — |
...
| `delete-all-rows-mor` | delete | `DELETE FROM t WHERE id > 0` | flat MoR v3 | 3 | one PUFFIN DV covering all 4 rows | drops the data file, no delete file | **DIVERGES** | `V3-COV-4` |
...
| `V3-COV-3` | partitioned `INSERT INTO` on v3 | `_row_id` is assigned by an unstable data-file order — two permutations measured across twelve runs of the same statement | DECLARED, fork TRIGGER | fork `IcebergTableProvider::insert_into` |
| `V3-COV-4` | `DELETE FROM t WHERE id > 0` (MoR) | repark writes one Puffin DV covering every row; Spark drops the data file and leaves no delete file | BACKLOG | repark |
...
## 6. What is deliberately not a row
...
- **Error text is not compared across engines.** A cell where both engines refuse is `REFUSED`, not
  `DIVERGES`; each engine's own message is pinned on its own side of the golden.
