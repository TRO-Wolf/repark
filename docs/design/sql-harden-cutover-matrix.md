...
| **EQUAL** | **1** (s3, 2026-09-04 CUTOVER-SCHEMA-1) |
| **DIVERGES** | **14** |
...
| Defects FIXED | **3** (`CUTOVER-DATE-1` FIXED 2026-09-04 DATE-FN-1; `CUTOVER-CTAS-REQ-1` + `CUTOVER-DEDUP-SCHEMA-1` FIXED 2026-09-04 CUTOVER-SCHEMA-1) |
...
the second MERGE) match Spark. Schema requiredness and parquet-read `string_view` match
Spark since 2026-09-04 (CUTOVER-SCHEMA-1). MoR delete-file packing and Spark's stamped `zstd`
codec still do not.
...
|---|---|---|---|---|---|---|---|
| `s1-ctas-if-fresh` | S1 parquet temp-view CTAS IF NOT EXISTS, v2 MoR | Spark SQL | DIVERGES | PASS | PASS | **DIVERGES** | `V3-COV-7` (`CUTOVER-CTAS-REQ-1` FIXED) |
| `s2-merge-idempotent` | S2 MERGE UPDATE SET * / INSERT * twice | Spark SQL | DIVERGES | PASS | PASS | **DIVERGES** | `V3-COV-7`, `CUTOVER-MERGE-FILES-1` (`CUTOVER-CTAS-REQ-1` FIXED) |
| `s3-dedup-coalesce-cast` | S3 row_number dedup + coalesce/cast | facade DF | EQUAL | PASS | PASS | **EQUAL** | (`CUTOVER-DEDUP-SCHEMA-1` FIXED) |
| `s4-overwrite-partitions` | S4 `writeTo.overwritePartitions` | facade DF | DIVERGES | PASS | PASS | **DIVERGES** | `V3-COV-7` |
| `s5-maintenance-calls` | S5 expire / rewrite_data_files / remove_orphan_files / rewrite_position_delete_files | Spark SQL CALL | DIVERGES | PASS | PASS | **DIVERGES** | `V3-COV-7` (`CUTOVER-CTAS-REQ-1` + `CUTOVER-DEDUP-SCHEMA-1` FIXED) |
| `s6-gold-incremental` | S6 gold join + agg + INSERT OVERWRITE | Spark SQL | DIVERGES | path | path | **DIVERGES** | `V3-COV-7` (`CUTOVER-DATE-1` FIXED; `COUNT(*)` nullability FIXED) |
| `s7-ctas-if-fresh` | S7 = S1 at format-version 3 | Spark SQL | DIVERGES | PASS | PASS | **DIVERGES** | `V3-COV-7` (`CUTOVER-CTAS-REQ-1` FIXED) |
| `s7-merge-idempotent` | S7 = S2 at format-version 3 | Spark SQL | DIVERGES | PASS | PASS | **DIVERGES** | `V3-COV-7`, `CUTOVER-MERGE-FILES-1` (`CUTOVER-CTAS-REQ-1` FIXED) |
| `s7-overwrite-partitions` | S7 = S4 at format-version 3 | facade DF | DIVERGES | PASS | PASS | **DIVERGES** | `V3-COV-7` |
| `s8-ctas-cow` | S1 at v2 copy-on-write | Spark SQL | DIVERGES | PASS | PASS | **DIVERGES** | `V3-COV-7` (`CUTOVER-CTAS-REQ-1` FIXED) |
| `s8-merge-idempotent-cow` | S2 at v2 copy-on-write; MERGE twice | Spark SQL | DIVERGES | PASS | PASS | **DIVERGES** | `V3-COV-7` (`CUTOVER-CTAS-REQ-1` FIXED) |
| `s8-overwrite-partitions-cow` | S4 at v2 copy-on-write | facade DF | DIVERGES | PASS | PASS | **DIVERGES** | `V3-COV-7` |
| `s9-ctas-cow` | S1 at v3 copy-on-write | Spark SQL | DIVERGES | PASS | PASS | **DIVERGES** | `V3-COV-7` (`CUTOVER-CTAS-REQ-1` FIXED) |
| `s9-merge-idempotent-cow` | S2 at v3 copy-on-write; MERGE twice | Spark SQL | DIVERGES | PASS | PASS | **DIVERGES** | `V3-COV-7` (`CUTOVER-CTAS-REQ-1` FIXED) |
...
| S2 / S7-merge (MoR) | row set, second-MERGE idempotence, delete-file kinds, schema requiredness | packing count (host-dependent; ≥ Spark's 2), codec (`V3-COV-7`) |
| S8 / S9-merge (CoW) | row set, second-MERGE idempotence, `delete_files` empty, data-file count 1, snaps `append,overwrite,overwrite`, v3 `next-row-id` 6, schema requiredness | codec (`V3-COV-7`) |
| S8 / S9-ctas (CoW) | row set, snaps `append`, `delete_files` empty, v3 `next-row-id` 3, schema requiredness | codec (`V3-COV-7`) |
| S5 | CALL tuples: expire six zeros, `rewrite_data_files` five zeros, `remove_orphan_files` empty, `rewrite_position_delete_files` four zeros, schema requiredness | codec as S1 |
...
Schema requiredness is FIXED 2026-09-04 (CUTOVER-SCHEMA-1): s3 is EQUAL
(`CUTOVER-DEDUP-SCHEMA-1` FIXED) and the CTAS-REQ programs still DIVERGE only on `V3-COV-7`.
S6 `COUNT(*)` nullability matches Spark since the same unit.
