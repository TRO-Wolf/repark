...
| [`orc_io.rs`](orc_io.rs) | **IO-ORC-1 (2026-09-16):** the ORC read binding — one free `#[pyfunction]` `read_orc` (the `session_sources` shape): session, path, mergeSchema/pathGlobFilter/recursive/modified bounds/basePath/ignoreCorruptFiles bools and strings, and the user schema as name/simpleString pairs, driving `ReparkSession::read_orc` with the GIL detached. pins: io-orc-1/C-002 |
