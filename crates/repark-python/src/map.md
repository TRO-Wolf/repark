...
| [`session.rs`](session.rs) | Shared runtime, session doors, readers, catalogs, and temp views. **FNP-4B round 8 (2026-09-15):** `shared_runtime` is `pub(crate)` so `F.expr` blocks on it. **CAST-MAP-SPELL-1 (2026-09-19):** `sql` prepares its text through `session_runtime::prepare_session_sql` and `native` registers through `session_runtime::register_native_door_functions` (file 1127 → 1126, ceiling ratcheted down). pins: cast-map-spell-1/C-005 **ICE-WAP-BRANCH-1 (2026-09-19):** the gate serves the two `spark.wap.*` keys into `repark_spark::wap::WapSessionConfig` through `write_wap_value`; the value is parsed in Rust (`parse_runtime_wap_value` trims, and an empty string clears the key, which is what the facade's `unset` forwards). pins: ice-wap-branch-1/C-007 |
| [`session_write_options.rs`](session_write_options.rs) | **ICE-WRITE-OPTIONS-1 round 3 (2026-09-17):** `session_sql_with_write_options(session, query, options)` runs writer SQL with the out-of-band option map (free `#[pyfunction]`: one `#[pymethods]` block per type, so it cannot be a method; split from `session.rs` under the file-size gate). **Run 22b rebase (2026-09-18, Q-22b-WO-1):** a fourth argument `force_static_overwrite=False` carries ICE-DYN-OVERWRITE-1's typed static flag (the `saveAsTable` pin), so the retired `static_overwrite.rs` / `sql_static_overwrite` entry folds into this one native call and options plus the flag travel on one statement. pins: ice-dyn-overwrite-1/L-001; ice-write-options-1/C-014 **ICE-OVERWRITE-MODE-1 (2026-09-19):** a fifth argument `force_dynamic_overwrite=False` carries `writeTo.overwritePartitions`' typed dynamic intent; the two flags map to `repark_core::OverwriteIntent` (`Session` / `Static` / `Dynamic`) and both set raises `ValueError`. pins: ice-overwrite-mode-1/C-007 **U7 PR2 slice-2 round 2 (2026-09-25, critic r4 V-001..V-007):** a sixth argument `source_by_name=False` carries `writeTo.overwrite(condition)`'s by-name source binding to `EngineContext::source_by_name`; `tests.rs` passes `false` on the intent pins. pins: u7-write-df-2/C-013 |
| [`writer_layout.rs`](writer_layout.rs) | **U7 PR1 (2026-09-24), round 2:** two free `#[pyfunction]`s for the DataFrameWriter, registered from `lib.rs`. `writer_plan(session, action, target, qualified, mode, explicit_format, frame_schema, layout)` reads the frame's analyzed Arrow schema (`cdf_infer::read_schema`), the session's `spark.sql.caseSensitive`, and runs `ReparkSession::plan_table_write`, returning `ctas` / `rtas` / `append` / `overwrite` / `skip` or raising the Spark-shaped refusal; a missing bucket column becomes `AnalysisException` with `_spark_error_class` `_LEGACY_ERROR_TEMP_3060` and parameters `i` (the name as `missing_column_name` renders it), `schema` (the tree from `repark_spark::spark_tree_string`). `writer_target_is_table(target, explicit_format)` is the `save()` name-or-path test. pins: u7-write-df/C-003, C-005, C-006, C-009, C-015, C-018 |
...
| [`orc_io.rs`](orc_io.rs) | **IO-ORC-1 (2026-09-16):** the ORC read binding — one free `#[pyfunction]` `read_orc` (the `session_sources` shape): session, path, mergeSchema/pathGlobFilter/recursive/modified bounds/basePath/ignoreCorruptFiles bools and strings, and the user schema as name/simpleString pairs, driving `ReparkSession::read_orc` with the GIL detached. **Round 2:** `paths` is a list (one scan for multi-path reads) and the binding attaches the native file conditions (`PATH_NOT_FOUND`, `UNABLE_TO_INFER_SCHEMA`, `FAILED_READ_FILE.CANNOT_READ_FILE_FOOTER`, `CANNOT_MERGE_SCHEMAS` with class/SQLSTATE/params) at the raise site. pins: io-orc-1/C-002 |
| [`subquery.rs`](subquery.rs) | DF-SUBQUERY-1 (2026-09-15): the subquery bindings — free `#[pyfunction]`s (the `session_sources` shape): `scalar_subquery` / `exists_subquery` scope-resolve a frame's expressions then wrap its plan in `Expr::ScalarSubquery` / `Expr::Exists`, `lateral_join` builds a `Join` whose right arm is `LogicalPlan::Subquery` over scope-resolved exprs (a `Column` `on`, a named-key list with Spark's drop-right-key projection, or neither), `subquery_alias` wraps a plan in `SubqueryAlias` so qualified `e.dept` survives analysis, and `column_outer` marks a `PyColumn` `OuterReferenceColumn`. `PyDataFrame.bound` gains a line-neutral scope-resolution funnel so outer refs resolve through select/filter/join. pins: df-subquery-1/C-001, C-002, C-003, C-004 |
| [`dataframe.rs`](dataframe.rs) | Lazy plans, actions, transforms, schema, and Arrow C Stream export. **FNP-4B (2026-09-15):** `filter_sql` parses through `column::expr_build::parse_canonical_predicate` (shared Databricks canonicalization). pins: fnp-4b/C-003 **U11-EDGE-1 round 2 (2026-09-26):** `drop`, `join_on_names`, `union` (by name) and the new `union_by_name(other, allow_missing)` hand their name decisions to `repark_core::frame_names` (`drop_named_columns`, `join_on_named_keys`, `union_by_folded_name`); `select` binds each column through `bind_projection_expr`. `join_keeps_only_left_columns` and `spark_join_projection` moved into the core binder (their comment shed). The file ceiling ratchets 1017 → 1016. pins: u11-edge-1/C-017, C-018, C-019, C-020 |
| [`dataframe_names.rs`](dataframe_names.rs) | **U11-EDGE-1 round 4 (2026-09-26):** free `#[pyfunction]`s over `&PyDataFrame` (the `dataframe_stats` shape, one `#[pymethods]` block per type). `drop_frame_columns(frame, names, references)` hands `DataFrame.drop` to `repark_core::frame_names::drop_named_columns` — string targets and Column targets travel apart, so a qualified Column binds through its relation and an unmatched target is a no-op. It replaces the `PyDataFrame.drop` method (`dataframe.rs` 1016 → 1005, ceiling ratcheted down). pins: u11-edge-1/C-022 **V-002 (same round):** `refuse_ambiguous_join_condition(left, right, condition_sql)` refuses a bare condition reference that matches two fields of the two sides ignoring case, and `requalify_join_sides(joined, left, right)` (`right=None` for semi/anti) re-projects the condition join's output under each side's relations, both through `repark_core::frame_names`. pins: u11-edge-1/C-023 **Round 5 (2026-09-26):** `attribute_column(name)` returns a `PyColumn` over `frame_names::attribute_reference`, the exact-bind column the facade uses for its own re-projections and origin Columns; `drop_frame_columns` gains `attributes` (exact engine names from an origin Column). pins: u11-edge-1/C-024, C-025, C-026 **Round 6 (2026-09-26, V-001):** `attribute_copies(frame)` and `attribute_copy_name(name)` bind `repark_core::frame_names::with_attribute_copies` / `attribute_copy_name` for the facade's SQL select route. pins: u11-edge-1/C-027 **Round 7 (2026-09-26, V-003):** `attribute_copy_name(frame, name)` takes the frame and returns `frame_names::attribute_copy_name_in(frame.schema(), name)`, the collision-free copy name `with_attribute_copies` gave that field. pins: u11-edge-1/C-030 |
...
| [`dataframe_stats.rs`](dataframe_stats.rs) | **DF-RUST-3 (2026-09-15):** two free `#[pyfunction]`s over `&PyDataFrame` (the `session_sources` shape, since pyo3 allows one `#[pymethods]` block per type): `freq_items(frame, engines, capacity)` runs `repark_core::freq_items` over the named engine columns, and `transpose(frame, index_engine, key_names, max_values)` runs `repark_core::transpose_frame` and formats the index→string display names via `repark_functions::java_double`. pins: df-rust-3/C-001, C-003, C-004 |
...
| [`dataframe_fill.rs`](dataframe_fill.rs) | **LOGICAL-WIDTH-1 (2026-09-16, round 2, R-12):** free `fill_expr_for_column` binds `repark_core::na_fill_expr` — the cast decision and coalesce land in Rust; the binding renders the literal's `CAST(... AS ...)` texts via `wrap_cast` from the core-returned target. Free `#[pyfunction]` over `&PyDataFrame` (one `#[pymethods]` block per type; `dataframe.rs` stays on its exact line baseline), registered in `lib.rs`; `analyzed_arrow_schema_native` widens to `pub(crate)` line-neutrally so the free function reads the plan schema. The literal texts travel as one `LitParts` unit (clippy `type_complexity`, no exception). pins: logical-width-1/C-012 |
...
| [`session_runtime.rs`](session_runtime.rs) | **SET-ANSI-RUNTIME-1 (2026-09-15):** two free `#[pyfunction]`s taking `PyRef<'_, PyReparkSession>` (the `session_sources` shape): `set_runtime_config` (strict gate) and `restore_runtime_config` (builder-lenient, for `RESET`), each writing the live `SessionContext` carrier plus the session zone snapshot. Comment-free per R-17c-3. pins: set-ansi-runtime-1/C-001, C-002, C-003 **R-17c-4:** plus `session_zone_canonical` (reads the snapshot companion for the facade cache; infallible, no lock beyond the snapshot read). pins: set-ansi-runtime-1/C-002 **ICE-RTAS-BYNAME-1 round 2 (2026-09-17):** `set_runtime_config` also serves `spark.sql.caseSensitive` (strict booleans) into the case-sensitivity carrier. pins: ice-rtas-byname-1/C-010 **ICE-DYN-OVERWRITE-1 (2026-09-17):** the gate serves a third key, `spark.sql.sources.partitionOverwriteMode`, writing `PartitionOverwriteModeConfig` (parse refuses with Spark's `OUT_OF_RANGE_OF_OPTIONS` class before anything is stored). pins: ice-dyn-overwrite-1/C-007, C-010, C-014 **CAST-MAP-SPELL-1 (2026-09-19):** `prepare_session_sql` runs the declared-function refusal then `cast_map::rewrite_map_casts`, so the native `repark.sql` door (core `DataFusionDialect`, no router) spells `CAST(… AS MAP<…>)`; `register_native_door_functions` registers `spark_log1p` and the cast-map UDFs on that door. pins: cast-map-spell-1/C-005 **ICE-SESSION-WRITE-CONF-1 (2026-09-19):** `unset_runtime_config` plus the `spark.sql.iceberg.*` write-conf bridge in `apply_runtime_config` — the facade forwards the keys, Rust validates. |
| [`session_runtime.rs`](session_runtime.rs) | **SET-ANSI-RUNTIME-1 (2026-09-15):** two free `#[pyfunction]`s taking `PyRef<'_, PyReparkSession>` (the `session_sources` shape): `set_runtime_config` (strict gate) and `restore_runtime_config` (builder-lenient, for `RESET`), each writing the live `SessionContext` carrier plus the session zone snapshot. Comment-free per R-17c-3. pins: set-ansi-runtime-1/C-001, C-002, C-003 **R-17c-4:** plus `session_zone_canonical` (reads the snapshot companion for the facade cache; infallible, no lock beyond the snapshot read). pins: set-ansi-runtime-1/C-002 **ICE-RTAS-BYNAME-1 round 2 (2026-09-17):** `set_runtime_config` also serves `spark.sql.caseSensitive` (strict booleans) into the case-sensitivity carrier. pins: ice-rtas-byname-1/C-010 **ICE-DYN-OVERWRITE-1 (2026-09-17):** the gate serves a third key, `spark.sql.sources.partitionOverwriteMode`, writing `PartitionOverwriteModeConfig` (parse refuses with Spark's `OUT_OF_RANGE_OF_OPTIONS` class before anything is stored). pins: ice-dyn-overwrite-1/C-007, C-010, C-014 **CAST-MAP-SPELL-1 (2026-09-19):** `prepare_session_sql` runs the declared-function refusal then `cast_map::rewrite_map_casts`, so the native `repark.sql` door (core `DataFusionDialect`, no router) spells `CAST(… AS MAP<…>)`; `register_native_door_functions` registers `spark_log1p` and the cast-map UDFs on that door. pins: cast-map-spell-1/C-005 **ICE-CATALOG-SESSION-1 S6 (2026-09-20):** plus `session_defaults` (engine default catalog + namespace for the facade USE/SET sync) and `register_late_catalog_block` (one runtime block in, newly-registered out). pins: ice-catalog-session-1/C-024, C-028 |
| [`session_runtime.rs`](session_runtime.rs) | **SET-ANSI-RUNTIME-1 (2026-09-15):** two free `#[pyfunction]`s taking `PyRef<'_, PyReparkSession>` (the `session_sources` shape): `set_runtime_config` (strict gate) and `restore_runtime_config` (builder-lenient, for `RESET`), each writing the live `SessionContext` carrier plus the session zone snapshot. Comment-free per R-17c-3. pins: set-ansi-runtime-1/C-001, C-002, C-003 **R-17c-4:** plus `session_zone_canonical` (reads the snapshot companion for the facade cache; infallible, no lock beyond the snapshot read). pins: set-ansi-runtime-1/C-002 **ICE-RTAS-BYNAME-1 round 2 (2026-09-17):** `set_runtime_config` also serves `spark.sql.caseSensitive` (strict booleans) into the case-sensitivity carrier. pins: ice-rtas-byname-1/C-010 **ICE-DYN-OVERWRITE-1 (2026-09-17):** the gate serves a third key, `spark.sql.sources.partitionOverwriteMode`, writing `PartitionOverwriteModeConfig` (parse refuses with Spark's `OUT_OF_RANGE_OF_OPTIONS` class before anything is stored). pins: ice-dyn-overwrite-1/C-007, C-010, C-014 **CAST-MAP-SPELL-1 (2026-09-19):** `prepare_session_sql` runs the declared-function refusal then `cast_map::rewrite_map_casts`, so the native `repark.sql` door (core `DataFusionDialect`, no router) spells `CAST(… AS MAP<…>)`; `register_native_door_functions` registers `spark_log1p` and the cast-map UDFs on that door. pins: cast-map-spell-1/C-005 **ICE-CATALOG-SESSION-1 S6 (2026-09-20):** plus `session_defaults` (engine default catalog + namespace for the facade USE/SET sync) and `register_late_catalog_block` (one runtime block in, newly-registered out). pins: ice-catalog-session-1/C-024, C-028 S7 strips the `///` docs (comment-ban zero); the semantics above stand. |
...
**FNP-AGG-1 slice (d) (2026-09-21):** `lib.rs` maps DataFusion unknown-function plan
errors to Spark's `[UNRESOLVED_ROUTINE]` condition at the binding door, and registers
the `grouping_id_column` binding for the thin `F.grouping_id` facade.
pins: fnp-agg-1/C-002, C-004
