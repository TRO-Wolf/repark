...
| [`tests.rs`](tests.rs) | Unit pins for module registration and exception/type identity, and the overwrite-intent binding pins: `session_sql_with_write_options` driven with each `force_static_overwrite` / `force_dynamic_overwrite` pair on a real `PARTITION (cat)` table in a static and a dynamic session (Dynamic keeps the untouched partition, Static replaces the table, Session follows the session mode, both flags refuse). |
