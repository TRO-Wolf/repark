...
| [`dataframe_fill.rs`](dataframe_fill.rs) | **LOGICAL-WIDTH-1 (2026-09-16, round 2, R-12):** `fill_expr_for_column` binds `repark_core::na_fill_expr` — the cast decision and coalesce land in Rust; the binding renders the literal's `CAST(... AS ...)` texts via `wrap_cast` from the core-returned target. Free `#[pyfunction]` over `&PyDataFrame` (one `#[pymethods]` block per type; `dataframe.rs` stays on its exact line baseline), registered in `lib.rs`; `analyzed_arrow_schema_native` widens to `pub(crate)` line-neutrally so the free function reads the plan schema. pins: logical-width-1/C-012 |
