...
| [`session.rs`](session.rs) | Shared runtime, session doors, readers, catalogs, and temp views. **FNP-11B step 3 (2026-09-15):** the shared-rules install call retargets to `bool_decimal::install_shared_analyzer_rules` (same line, count held). pins: fnp-11b/C-007 |
