...
| [`exceptions.rs`](exceptions.rs) | PySpark-shaped exception types. **ICE-COMMIT-UNKNOWN-1 (2026-09-14):** `CommitStateUnknownException(PySparkException)` — the ambiguous-commit class; `to_py_err` sets `operation_id` on the instance from `Error::CommitStateUnknown`. pins: ice-commit-unknown-1/C-002 |
