...
| [`session_write_options.rs`](session_write_options.rs) | **ICE-WRITE-OPTIONS-1 round 3 (2026-09-17):** `session_sql_with_write_options(session, query, options)` runs writer SQL with the out-of-band option map (free `#[pyfunction]`: one `#[pymethods]` block per type, so it cannot be a method; split from `session.rs` under the file-size gate). |
