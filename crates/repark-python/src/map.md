...
| [`writer_layout.rs`](writer_layout.rs) | **U7 PR1 (2026-09-24):** two free `#[pyfunction]`s for the DataFrameWriter, registered from `lib.rs`. `writer_check_layout(session, parts, partition_columns, num_buckets, bucket_columns, sort_columns)` runs `ReparkSession::check_writer_layout` (Spark's partitioning-match refusal surfaces as `IllegalArgumentException`); `writer_save_target(session, target, qualified, mode, explicit_format)` returns `create` / `append` / `overwrite` / `skip` from `ReparkSession::writer_save_target` or raises its Spark-shaped refusal. pins: u7-write-df/C-003, C-005, C-006, C-009 |
