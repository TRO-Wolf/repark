...
TABLE, and the DML passthrough. **Change-rename refusal (2026-09-21):** the Hive
`CHANGE COLUMN` intercept refuses the two-name rename form at parse time
(`[PARSE_SYNTAX_ERROR]`, SQLSTATE 42601) and keeps the same-name type/COMMENT
form ([router/hive_change_column.rs](src/router/hive_change_column.rs)). The unit
battery is under `src/tests/` (navigation
