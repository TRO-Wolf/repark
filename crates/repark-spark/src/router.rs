...
//!
//! **PR-2 spine scope (TEMPORARY refuse arms).** This PR ports the router spine — normalize,
//! passthrough Spark-AST defaults, DESCRIBE/SHOW namespace, metadata tables, the time-travel
//! scanner, and the P11/MoR/SEC-02 guards. The handler modules for CTAS, column-def CREATE
//! TABLE, DROP TABLE, namespace DDL, ALTER, MERGE INTO, INSERT OVERWRITE, CALL, and branch/tag
//! ref DDL land in phase-2 PR-3a/PR-3b; until then their router arms refuse loudly with
//! [`DataFusionError::NotImplemented`] naming the construct (see [`refuse_pending`]). The v1
//! write-to-branch sniff (`ref_ddl::sniff_write_to_branch`) is a declared TEMPORARY omission
//! restored with `ref_ddl` in PR-3b.
//!
...
use crate::{describe_show, metadata_tables, spark_ast, time_travel};
...
///
/// PR-2 spine: interception is live for DESCRIBE/SHOW namespace forms, metadata tables, time
/// travel, TRUNCATE (targeted refuse), and the DML passthrough guards; the CTAS / CREATE /
/// DROP / ALTER / MERGE / INSERT OVERWRITE / CALL / ref-DDL handlers refuse loudly until
/// phase-2 PR-3a/PR-3b restores them (see the module docs).
/// ===========================================================================================
///
/// # Errors
...
pub async fn execute_with_read_only<S: std::hash::BuildHasher>(
    ...
    // TEMPORARY (PR-2): v1 runs the r25 T2 write-to-branch STOP here via
...
/// TEMPORARY (PR-2 refuse-arm class): a loud `NotImplemented` for a router arm whose handler
/// module arrives in phase-2 PR-3a/PR-3b. Every call site names the construct and the restoring
/// PR; each arm carries a refuse test in `router/tests.rs`.
...
async fn execute_inner(
    ...
    // Pre-parse recognizers + the PR-2 pre-parse refuse set (see `try_preparse_intercepts`).
    ...
    let Some((statement, _partitioning)) = parse_single_normalized(sql)? else {
        return execute_unparsable_fallthrough(ctx, catalogs, sql).await;
    };
    match &statement {
        // TEMPORARY refuse arms (PR-2): handlers land in PR-3a (DDL) / PR-3b (DML + refs).
        Statement::CreateTable(create) if create.query.is_some() => Err(refuse_pending(
            "CREATE TABLE … AS SELECT (CTAS)",
            "PR-3a (ctas)",
        )),
        Statement::CreateTable(_) => Err(refuse_pending(
        ...
        Statement::Drop {
            ...
            ..
        } => Err(refuse_pending("DROP TABLE", "PR-3a (namespace_ddl)")),
        Statement::Drop {
...
/// Pre-`parse_single_normalized` intercepts (DESCRIBE/SHOW namespace — Groups Z + AB) plus the
/// PR-2 pre-parse refuse set: the v1 pre-parse recognizers for ALTER (I6/I7), CREATE NAMESPACE,
/// and ref DDL (I5) live in modules landing in PR-3a/PR-3b, so their statement shapes refuse
/// loudly here (TEMPORARY refuse-arm class — leading-token sniffs, same recognition boundary).
async fn try_preparse_intercepts(
    ctx: &SessionContext,
    catalogs: &CatalogRegistry,
    sql: &str,
) -> Option<Result<DataFrame>> {
    // ALTER … — v1 routes ALTER TABLE (I6), the iceberg ALTER DDL forms (I7), and the residual
    // unsupported-ALTER refusals through the `alter` module (PR-3a).
    if starts_with_keywords(sql, &["ALTER"]) {
        return Some(Err(refuse_pending(
            "ALTER TABLE / ALTER …",
            "PR-3a (alter)",
        )));
    }
    // Snapshot-ref DDL (I5) — top-level `CREATE|DROP|REPLACE BRANCH|TAG` forms (PR-3b). Checked
    // before CREATE NAMESPACE so `CREATE BRANCH` names the ref-DDL construct. The bare
    // `REPLACE BRANCH|TAG` form (v1: `ref_ddl::try_parse_ref_ddl`) is sniffed explicitly — the
    // normalize sniff covers only the CREATE/DROP and ALTER-scoped shapes.
    ...
    // `CREATE {NAMESPACE|SCHEMA|DATABASE}` — the v1 pre-parse recognizer (Spark `LOCATION` /
    // `COMMENT` / `WITH DBPROPERTIES` clauses) lives in `namespace_ddl` (PR-3a). Refused here
    // pre-parse so the bare form cannot fall through to a DataFusion in-memory schema.
    if starts_with_keywords(sql, &["CREATE", "NAMESPACE"])
        || starts_with_keywords(sql, &["CREATE", "SCHEMA"])
        || starts_with_keywords(sql, &["CREATE", "DATABASE"])
    {
        return Some(Err(refuse_pending(
            "CREATE NAMESPACE | DATABASE | SCHEMA",
            "PR-3a (namespace_ddl)",
        )));
    }
...
/// leading whitespace/comments — the `starts_with_merge` pattern). PR-2 refuse-arm recognizer.
