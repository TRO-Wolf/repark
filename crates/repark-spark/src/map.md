...
- `ctas.rs` — CTAS staged create/replace (fork `StagedTableTransaction`, one catalog publish),
  ...
  **SE-1 PR-D1:** refuses Iceberg CREATE when any `TableScan` source is tagged
  `repark.tighten_nulls` (source walk, SQM F1) or the output schema still carries the
  tag (the write-boundary relax is PR-D2).
