...
- `window_range.rs` — **G5b (2026-08-11) + G5b-R (Y-1, 2026-08-12):** Spark's rules for a
  **unit-less** `RANGE` frame offset over a datetime order key, plus two residuals that share
  the same seam. DataFusion coerces a unit-less bound to `Interval(MonthDayNano)`, where Arrow
  reads a bare `"1"` as one **month**; Spark refuses it on a `TIMESTAMP` key
  ...
  for **R3** (negative interval over TIMESTAMP → Spark-empty frame; wrapping `count(*)` = -1
  is gone) and **R2** (`DAY TO SECOND` → Arrow-accepted interval text). A cheap AST probe
  (`statement_has_bare_range_bound`) still keeps ordinary statements on the single-plan path
  (now also fires for a negative or field-qualified interval). R1 / R4 / R5 stay recorded.
