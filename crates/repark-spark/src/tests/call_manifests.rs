...
async fn call_rewrite_manifests_no_op_returns_zeros_and_commits_nothing() {
    ...
    // The seeding call must be asserted, not just run.
    let seeded = execute(
        &ctx,
        &catalogs,
...
/// Seed a merge-on-read table whose DATA manifest count stays at one.
...
/// Seed `appends` single-row appends under a 4 KB manifest target.
