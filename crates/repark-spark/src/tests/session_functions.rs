...
#[tokio::test]
async fn spark_door_session_scalars_refuse_window_use() {
    let (ctx, catalogs, _holder) = session_ctx().await;
    for (name, sql) in [
        ("user", "SELECT user() OVER () FROM src"),
        (
            "current_user",
            "SELECT current_user() OVER (PARTITION BY id) FROM src",
        ),
        (
            "session_user",
            "SELECT session_user() OVER w FROM src WINDOW w AS (ORDER BY id)",
        ),
        ("version", "SELECT concat(version() OVER (), 'x') FROM src"),
        ("user", "SELECT \"user\"() OVER () FROM src"),
        ("version", "SELECT \"version\"() OVER () FROM src"),
        (
            "user",
            "SELECT transform(array(1), x -> x), user() OVER () FROM src",
        ),
    ] {
        let error = execute(&ctx, &catalogs, sql).await.unwrap_err().to_string();
        assert!(error.contains("[UNSUPPORTED_EXPR_FOR_WINDOW]"), "{error}");
        assert!(error.contains(&format!("\"{name}()\"")), "{error}");
        assert!(error.contains("SQLSTATE: 42P20"), "{error}");
    }
}

#[tokio::test]
async fn spark_door_session_scalar_window_arguments_reach_planner() {
    let (ctx, catalogs, _holder) = session_ctx().await;
    for name in ["user", "current_user", "session_user", "version"] {
        let sql = format!("SELECT {name}(1) OVER () FROM src");
    ...
    let error = execute(&ctx, &catalogs, "SELECT user(DISTINCT) OVER () FROM src")
        .await
        .unwrap_err()
        .to_string();
    assert!(!error.contains("[UNSUPPORTED_EXPR_FOR_WINDOW]"), "{error}");
...
#[tokio::test]
async fn spark_door_window_functions_and_quoted_shadow_columns_still_work() {
    let (ctx, catalogs, _holder) = session_ctx().await;
    let batches = execute(
        &ctx,
        &catalogs,
        "SELECT row_number() OVER (ORDER BY id) AS row_number FROM src",
    )
    .await
    .unwrap()
    .collect()
    .await
    .unwrap();
    let row_numbers: Vec<i32> = batches
        .iter()
        .flat_map(|batch| {
            batch
                .column(0)
