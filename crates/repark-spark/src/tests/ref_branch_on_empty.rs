...
async fn branch_and_tag_on_a_v1_table_refuse_until_the_fork_keeps_v1_refs() {
    const V1: &str = "This feature is not implemented: BRANCH|TAG on the format v1 table \
                      sales.be is not supported: the Iceberg fork writes v1 metadata without its \
                      refs, so the new ref would be lost";
    let wh = TempDir::new().unwrap();
    let (ctx, catalogs) = setup(&wh).await;
    ...
    for seeded in [false, true] {
        ...
        for sql in [
            ...
            assert_eq!(error.to_string(), V1, "{sql}");
        }
    }
    let table = load_sales_table(&catalogs, "be").await;
    assert_eq!(table.metadata().snapshots().count(), 1);
