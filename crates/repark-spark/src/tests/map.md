...
- `v3_cow.rs` — **V3R-1 (2026-08-25):** adopted-v3 copy-on-write DELETE / UPDATE / MERGE refuse
  (registry `V3-COW-1`, both guard seats — resolver and passthrough valve), the CCC regressions
  (default-catalog short names, a padded merge-on-read spelling), the merge-on-read DELETE +
  MERGE refusals, a v2 control; keeps V3E-2's `V3_MAINTENANCE_ORACLE` constant and
