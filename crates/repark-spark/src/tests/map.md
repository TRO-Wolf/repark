...
- `call_rewrite_options.rs` — **rewrite_data_files options:** `where => 'part = 0'` keeps the
  other partition's files byte-identical; unknown strategy and bad where use Spark's text;
  `sort_order` refuses without compacting; named `BINPACK` still compacts v2.
