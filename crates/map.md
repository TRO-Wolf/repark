...
| [repark-ta](repark-ta/map.md) | Technical-analysis kernels (tier 3): bit-exact TA-Lib 0.4.0 hand-ports, golden-gated, plus the optional `datafusion` feature's window-UDF layer. |
...
`repark-ta` is a tier-3 leaf with no internal deps (kernel core: `thiserror` only);
