...
[cc-3-comment-condensation-ledger.md](../task/ledgers/archive/2026-08/2026-08-30-cc-3-comment-condensation-ledger.md).
...
| [repark-ml](repark-ml/map.md) | Native ML kernels (tier 3): hand-rolled Cholesky and streaming OLS, IRLS, and Lloyd KMeans. A capability leaf with no internal dependencies; models hold parameters only. |
| [repark-distributed](repark-distributed/map.md) | Distributed execution (tier 3, role `runtime`): the `DistributedExecutor` seam plus the local DataFusion executor, with the Ballista-backed cluster executor behind the off-by-default `cluster` feature. Seeded by BALLISTA-M1-A. |
