...
- `declared_sorted.rs` — SE-1 declared-sorted temp views: plan pins (window `SortExec`
  ...
  EAGER-BUDGET-1 step 2 (2026-09-14): the cache-materialize call sites pass `None` for the
...
- `temp_view_doors.rs` — **SQM round 6.** `qualified_temp_view_name_refuses_and_persists_nothing`
  ...
  This map owns scenario history. Source comments keep only each assertion's invariant or mutation discriminator.
