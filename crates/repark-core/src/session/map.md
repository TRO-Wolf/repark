...
- `temp_views.rs` — **SQM round 6 (R6-1):** the temp-view family, split out of `session.rs` when
  ...
  **DECIMAL-CACHE-1 (2026-09-15):** `register_collected_memtable` analyzes the plan with
  ...
  analyzed Spark type — pinning the former refused every materializing action). Collected
  batches then conform to the analyzed schema (`conform_batches_to_schema`: same-type
  pass-through, `cast_with_options` with `safe: false`, logical nullability and metadata;
