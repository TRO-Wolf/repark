...
- `writer_layout.rs` — **U7 PR1 (2026-09-24), round 2:** `ReparkSession::plan_table_write`,
  ...
  pins: u7-write-df/C-005, C-009, C-018
...
- `df_guards.rs` — the DataFusion **54.1 regression guards**, owned and constructed by CORE
  ...
  * **Guard 2, a scoped optimizer RULE** (since **DEFECT-2, 2026-08-18**):
    ...
    **U8 WRITE-SQL PR2 fix round 4 (2026-09-25):** outside that path the wrapper also declines
    on a `Projection` whose rewrite fails with the rule's own alias collision
    (`SchemaError::AmbiguousReference` on a `__datafusion_extracted…` name: "qualified field
    name s.__datafusion_extracted_1 and unqualified field name …"). The shape is a
