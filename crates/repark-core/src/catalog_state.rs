...
pub enum LocationPolicy {
    ...
    /// (`ReparkSession::register_memory_catalog` resolves `std::env::temp_dir()` there) — the
    /// CTAS consumer reads `root` and never touches the process environment at query time.
    TempFallbackAllowed {
        /// The temp root a location-less staged CTAS resolves table locations under.
        root: PathBuf,
    },
}
...
impl<const N: usize> From<[(String, Arc<dyn Catalog>); N]> for CatalogRegistry {
    ...
    /// S3 Tables gets [`LocationPolicy::ServiceManagedLocation`]). E-4: the temp `root` is
    /// resolved here, at registration time — the construction-time analogue of
    /// `register_memory_catalog`'s resolution.
