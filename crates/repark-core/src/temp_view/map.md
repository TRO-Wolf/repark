...
caller's temp-view name becomes a `TableReference` (R6-1).
...
The home is TWO things, not one: the build-time `catalog.schema` NAME and the
...
| Reading `SELECT * FROM v` fails while `tableExists("v")` is true — in a RAW SQL body on the native door | Known, measured, and a real CHANGE from BASE, not "current behaviour" (R6-1 disclosure; pinned by `set_to_a_plain_catalog_keeps_the_write_home_and_moves_only_the_read`): the WRITE is pinned to the home, the READ is still DataFusion's live-default resolution. Name the home (`datafusion.public.v`) or clear the `SET`. **R7-1:** the facade's PRODUCT read paths no longer hit this — they ask `resolve_temp_view_home_ref` and emit the home spelling — so a facade `spark.table` / cache / scratch-view read that misses under a `SET` is a BUG, not this row. |

First checks: `cargo test -p repark-core temp_view`. Escalate to: [../map.md#debug](../map.md).
**R7-1:** `tests.rs` pins
`the_sessions_own_home_spelling_is_the_home_not_a_qualified_refusal` — `temp_view_ref` accepts
`<home.catalog>.<home.schema>.<view>` (quoted or not) as the SAME session-local view, because that
is the spelling product read paths must emit to survive a `SET
datafusion.catalog.default_catalog`. Neighbouring three-part names (`ice.public.v`,
`datafusion.other.v`) still refuse, which is what that pin's second half kills.
