...
- **Allowed internal deps:** **none internal** — speaks `datafusion::error::Result` (no `repark-core`
  dep). Third-party: datafusion + datafusion-spark + arrow + chrono + regex + fancy-regex
  (JAVA-REGEX-FEATURES-1, 2026-09-16: fallback engine for Java lookaround, backreferences,
  possessive quantifiers and atomic groups, under a backtrack limit).
