...
fn product_string_to_double(arg_types: &[DataType]) -> Result<Vec<DataType>> {
    arg_types
        .iter()
        .map(|arg| match arg {
            DataType::Utf8 | DataType::LargeUtf8 | DataType::Utf8View => Ok(DataType::Float64),
            _ => Ok(arg.clone()),
        })
        .collect()
}
...
impl AggregateUDFImpl for SparkProduct {
    ...
    fn coerce_types(&self, arg_types: &[DataType]) -> Result<Vec<DataType>> {
        product_string_to_double(arg_types)
    }
...
mod tests {
    ...
    #[tokio::test]
    async fn numeric_strings_coerce_to_double() {
        let batch = batch(
            &ctx(),
            "SELECT __repark_product(s) FROM (VALUES ('2.0'), ('3.0')) AS t(s)",
        )
        .await;
        assert_eq!(double(&batch, 0), Some(6.0));
    }
