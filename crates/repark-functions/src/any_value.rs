...
fn check_flag_literal(exprs: &[Arc<dyn PhysicalExpr>]) -> Result<()> {
    if exprs.len() < 2 {
        return Ok(());
    }
    let erased: &dyn std::any::Any = exprs[1].as_ref();
    let literal = erased.downcast_ref::<Literal>().map(Literal::value);
    if matches!(literal, Some(ScalarValue::Boolean(Some(_)))) {
        return Ok(());
    }
    Err(DataFusionError::Plan(
        "[_LEGACY_ERROR_TEMP_1210] The second argument in any_value should be a boolean literal."
            .to_string(),
    ))
}
...
impl AggregateUDFImpl for SparkAnyValue {
    ...
    fn accumulator(&self, acc_args: AccumulatorArgs) -> Result<Box<dyn Accumulator>> {
        ...
        check_flag_literal(acc_args.exprs)?;
...
mod tests {
    ...
    async fn flag_error(ctx: &SessionContext, sql: &str) -> String {
        match ctx.sql(sql).await {
            Err(failure) => failure.to_string(),
            Ok(frame) => frame
                .collect()
                .await
                .expect_err("flag must fail")
                .to_string(),
        }
    }

    #[tokio::test]
    async fn non_literal_flag_is_refused() {
        let ctx = ctx();
        let refused = flag_error(
            &ctx,
            "SELECT any_value(v, v > 0) FROM (VALUES (10), (20)) AS t(v)",
        )
        .await;
        assert!(refused.contains("_LEGACY_ERROR_TEMP_1210"), "{refused}");
        let null_flag = flag_error(
            &ctx,
            "SELECT any_value(v, CAST(NULL AS BOOLEAN)) FROM (VALUES (10), (20)) AS t(v)",
        )
        .await;
        assert!(null_flag.contains("_LEGACY_ERROR_TEMP_1210"), "{null_flag}");
    }
