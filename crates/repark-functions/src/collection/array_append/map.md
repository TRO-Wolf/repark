...
- `coerce.rs` — **ARRAY-NULL-1 round 4 (2026-09-14):** Spark's recursive
  ...
  `array<float>` + int stays float), recurses through list children, map keys and
  values, and struct fields (same count, positional order, names matched
  case-insensitively, result keeps the array's field names), and resolves the
