...
- `tests.rs` — the `#[cfg(test)] mod tests;` declared in `../guards.rs`. The text/plan guards are
  ...
  still-refused NOT IN / UPDATE IN), the end-to-end refuse, the IN-DELETE execute pin
  (`dml_subquery_in_delete_executes_and_deletes_exactly_the_match`), the **valve-ORDER** pin
  against BUG-001 (`mor_valve_runs_after_the_g3e8_valve`, restated over NOT IN), and
...
| A delegated DELETE/UPDATE with a subquery `WHERE` was refused | By design (G3-E8 residual). Uncorrelated `DELETE … col IN (SELECT …)` is the PR-1 product hole and now executes. Other spellings stay refused. `task/z1-g3e8-pr1-ledger.md` |
