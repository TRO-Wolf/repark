...
- `v3_cow.rs` — **V3E-1 (test-only):** ANSI adopted-v3 copy-on-write DELETE/UPDATE/MERGE
  ...
  refuses `CALL`.
- `v3e3.rs` — **V3E-3 (test-only):** ANSI live-row twins of the Spark-written
  partitioned DV and equality-delete + DV fixtures, plus `$delete_files`
  content 1/2 (`Model: Grok 4.6 xHigh`, `CodeQuality:S`).
