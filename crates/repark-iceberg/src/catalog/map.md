...
- `caches.rs` — **PERF-ICE-CATALOG-IO-1 (2026-09-05):** the session-scoped Iceberg cache handles
  ...
  **PERF-ICE-CATALOG-IO-3 (2026-09-05):** the default-ON flip IO-2's round-2 ruling named
  ...
  default agrees with the fork instead of inventing a second number. The budget is moka
  `max_capacity` over entry weight, TinyLFU admission with rejection. Through IO-3 the
  weigher was the fork's estimate (768 B per manifest entry, 256 B per list entry,
  floored at one entry), so the 193-manifest `t_many` weighed roughly 150 KB. **RP-16
  (2026-09-11)** at pin `090bc821` (fork `#274` F-CATIO-WEIGHT): the weigher charges
  each retained parsed object graph. Registry `PERF-CATALOG-CACHE-WEIGHT-1` is FIXED.
  Measured on the 256-table pin shape: smallest retaining round budget 1,071,000,
  largest evicting round budget 1,070,000 (~4,184 B charged per small table); the
  retain pin uses 1,250,000 (headroom above the 1,071,000 edge); the old 280000
  budget now evicts. The 32 MiB is NOT a resident-bytes ceiling: round 2
  measured ~7.5 KB resident per cached small manifest+list at 2,000–8,000 tables
  against the old estimate (the estimate sat below the objects' own file bytes). The
  ceiling is per memory
  ...
  cleared at the statement door). Size the budget to the working set (about 4 KiB of
  charged weight per small-manifest table as measured): a budget far under it churns —
  repeat reads
  ...
  no retention outside the cache, not the weight bound, which at the old estimate was
  unexercised (8 MB charged of 32 MB at 8,000 tables). The flip is safe
...
- `metadata_columns.rs` — **ICE-METADATA-COLS-1 (2026-09-20):** `MetadataColumnsTableProvider`
  serves `_file` (Utf8) and `_pos` (Int64) on current-snapshot reads by handing the
  ...
  hard-coded `UNSERVED_METADATA_COLUMN_NAMES` names the three PR-2 columns. An empty
...
| Serve `_file` / `_pos` on a Spark-door read | `metadata_columns.rs` (`MetadataColumnsTableProvider`); the Spark door calls `repark_core::prepare_metadata_column_sql` |
