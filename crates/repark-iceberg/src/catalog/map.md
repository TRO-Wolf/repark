...
| Serve `_file` / `_pos` / `_spec_id` on a Spark-door read | `metadata_columns.rs` (`MetadataColumnsTableProvider`); the Spark door calls `repark_core::prepare_metadata_column_sql` |
