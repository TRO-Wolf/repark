...
DataFusion window-UDF wrappers for the TA kernels (feature `datafusion`). Shared
machinery — TLS multi-output cache, densify, period/scalar checks,
`PartitionEvaluator::evaluate_all`, SPECS, `TaFn`, statistic + math_operator
dispatch, `register_all` / `window_udfs` / `window_udf` — lives in `mod.rs`.
...
this wave. Kernel math is **not** here — it stays in `../overlap.rs` etc.
...
- `mod.rs` — spec table (81 entry points), `TaFn` metadata (arity, multi-family
  ...
  plus `compute_routes_every_spec_to_a_family_or_shared_arm` (router vs family
  table cannot drift into `family_dispatch_miss`).
...
| Three BBANDS columns recompute | TLS cache in `mod.rs` (scout #8); pin via `evaluate_all` sibling tests |
