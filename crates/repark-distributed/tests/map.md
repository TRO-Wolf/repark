...
- `codec.rs` (`feature = "cluster"`) — BALLISTA-M2-B: the installed physical codec is the
  ...
  through `try_encode`/`try_decode`. Encode reads the scan through typed accessors;
  predicates travel as `datafusion-proto` `Expr`s. A vanilla session refuses encode and
  decode loud; an unowned custom node refuses encode loud and passes the file-group rewrite
