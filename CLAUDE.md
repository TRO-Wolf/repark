...
| Gate roster (`verify` / `preflight` / facade) | [AGENTS.md `## Verify before "done"`](AGENTS.md#verify-before-done) — `verify` is Rust-only; `preflight` adds `py-test-facade` + `py-test-dbt` + audit + workflow lint. Keep this pointer in lockstep with AGENTS.md; do not invent a second roster. |
