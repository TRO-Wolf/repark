...
- **Iceberg rating residue (2026-09-16)** — scheduled; each unit deletes its clause:
  DML after `ADD`/`RENAME COLUMN` refuses (V2-10e); `write-default` fills NULL (V3-03b, #678);
  serializable MERGE aborts on any concurrent commit (V2-20a); a Spark-added nested child is
