...
- **Iceberg rating residue (2026-09-16)** — scheduled; each unit deletes its clause:
  `write-default` fills NULL (V3-03b, #678); a plain-`WHERE` UPDATE aborts on a disjoint
  commit (V2-20a); a Spark-added nested child is unreadable (V2-10d); mixed-case columns fail
  (V2-27, #676).
