...
| Done gate | [docs/testing.md](../../../docs/testing.md) + the gates the `green_commands` row below names (the scoped local gate at R2, CI's required checks at R7); test with `cargo test --workspace`, **never** `--all-features`; new behavior pins per the entry-point matrix (native DataFrame / ANSI SQL / Spark facade) once those surfaces exist | BIND | Delivery invokes |
...
| Sub-agent / tier policy | [AGENTS.md](../../../AGENTS.md) "Delegated work" (the neutral rule: single-agent default) + [CLAUDE.md](../../../CLAUDE.md) "capability tiers and sub-agents" (Claude-specific launch mechanics only; the tier policy — approved executor tiers, no Sonnet or Haiku in any role, **Opus sub-agents need an explicit user command naming Opus** — is AGENTS.md's) | BIND | Orchestrator's AC mode follows it (see `context_break_mechanics` below + Debug) |
...
| `green_commands` | **Unit gate (R2 exit):** the scoped local gate — the comment gate, the release native, `cargo test -p` for the crates the unit touched, the unit's own pytest files offline and live (AGENTS.md "Delegated-agent standing rules", Gates). **Pre-merge gate (R7 `ci_green`):** CI's required checks green on the PR head — they run `make verify`'s content, the facade suite, the parity harness and the security/workflow gates (taplo, typos, cargo-deny, zizmor, workflow-parse); tool versions pinned identically in the Makefile and the workflows; nothing silently skips (uvx provisions). `make preflight` mirrors that roster for a developer with a box to spare; it is never a delegated unit's local gate. **CI-only exceptions (recorded per R7, residual gap stated):** the whole-workspace roster on the shared build box — justification: memory (the workspace test link exhausted the box on 2026-09-18); residual gap: a red first seen at CI costs one ~25-minute cycle. The record grows only when a later phase adds another check that cannot run locally, each entry naming the check, the justification, and the residual gap. **Parity guard:** pinned tool versions are enforced by construction (uvx `tool@version`); a pin that appears in a workflow but not the Makefile (or vice versa) is a binding defect. | **Never `cargo test --all-features`** — see [AGENTS.md](../../../AGENTS.md) "PyO3 build notes". |
