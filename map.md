...
- `Cargo.toml` — **also the version SSOT (release PR, 2026-08-14):** `[workspace.package] version` (1.4.2) is the single release version; maturin injects it into the wheel (pyproject `dynamic`); bump here, nowhere else (internal deps are path-only — no version requirements to chase). With `Cargo.lock`, `rust-toolchain.toml`, `rustfmt.toml`, `clippy.toml`, `deny.toml`,
  ...
  **RP-49 nested-null reads + binary partition values (2026-09-24):** `d57974d4` (`#351` — the metadata tables' partition tuple projects a BINARY partition value through `LargeBinaryBuilder`, matching `type_to_arrow_type`; `#352` — a MAP or LIST column, or a struct holding one, added by schema evolution reads as typed nulls for files written before the ALTER). The workspace built unchanged; `cargo update` touched only the six fork packages. Unblocks D-ADD-COL-STRUCT, D-X-ADD-COL-MAP-KEY-STRUCT and D-CREATE-PART-TRUNC-BIN (U5).
  **RP-50 v1 metadata refs + replace fresh ids (2026-09-25):** `b2698e56` (`#354` — `begin_replace` assigns fresh field ids by name as Java's `buildReplacement` does and rebinds the spec and sort order by source name; `schemas`/`partition-specs`/`sort-orders` serialize in ascending id order; `#353` — `TableMetadataV1` carries `refs` like V2, written for every v1 table (`{}` when empty, Java's shape) and read when present; legacy v1 files without the key still reconstruct `main` from `current-snapshot-id`; Java's three ref rules validated on the v1 path). The workspace built unchanged; `cargo update` touched only the six fork packages. Unblocks D-CREATE-V1 `md.refs` (U5 PR2b), lets U5 drop `refuse_ref_on_format_v1`, and lets U7 drop its `replacement_schema` re-keying call (residue R-10).
  **RP-51 unknown columns write + uuid as string (2026-09-25):** `08735de9` (`#356` F-UNKNOWN-WRITE-1 — the Parquet writer omits `unknown` (v3) fields, top level and inside structs, values read NULL, metrics skip them, `readable_metrics` answers an all-null struct, copy-on-write rewrites keep the omission; a lone all-unknown struct and unknown under list/map refuse; `#355` F-UUID-STRING-1 — `IcebergTableProvider::with_uuid_as_string(bool)`, default off: `Utf8` advertised for every uuid field, canonical lower-case text in the scan with byte pushdown kept, text parsed on insert and the row-level paths (`Invalid UUID string: …` on bad text), DELETE refuses an unbindable literal only when the whole condition converts like Spark's `canDeleteWhere`; dated divergences ILIKE, Unicode digits, metadata-delete and optimizer-shape live in the fork's `table/map.md`). The workspace built unchanged; `cargo update` touched only the six fork packages. Unblocks U9 PR2: `TY-UNKNOWN-VOID` (VOID → `unknown` on v3) and `TY-UUID-READ` (the provider option on in the catalog path).
  **RP-53 uuid flag on deregistered providers (2026-09-26):** `96582bc2` (`#358` F-UUID-STATIC-2 — `IcebergSchemaProvider::deregister_table` returns the provider WITH the schema provider's `uuid_as_string` flag through the same helper `resolve_table` uses; static-provider pins for nested uuid text at every depth, text-vs-byte pushdown parity and the flag-off byte path; the duplicated `schema()` advertisement body folded into `scan_knobs::advertised_schema`, no behaviour change with the flag off). The workspace builds unchanged; `cargo update` touches only the fork packages. Closes the three follow-ups the #357 verifier recorded.
  **RP-52 uuid text on every provider (2026-09-26):** `b61c82b8` (`#357` F-UUID-STATIC-1 — `IcebergStaticTableProvider::with_uuid_as_string(bool)` and `IcebergCatalogProvider::with_uuid_as_string(bool)` (plus the schema provider's switch), default off, reusing #355's text rendering and byte pushdown, so a snapshot-pinned read (`VERSION AS OF`, `TIMESTAMP AS OF`, tag, branch, a staged WAP snapshot) and every catalog-resolved table present uuid as text on request; metadata tables pass through). The workspace built unchanged; `cargo update` touched only the six fork packages. Unblocks the U9 PR2 static-provider slice (the `IcebergStaticTableProvider` sites in repark-core `time_travel.rs` / `iceberg_path.rs` and repark-spark `time_travel.rs` / `wap.rs`) and lets RePark drop the `UuidTextSchemaProvider` shim for the catalog option.
