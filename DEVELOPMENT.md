...
| `make check-example-coverage` | v0.7 public-surface example drift gate. AST-enumerates `F.*` / DataFrame / TA / reader-writer / session doors, compares `docs/examples` `COVERS` (must be used in the script) to the backlog and exceptions ratchets, executes examples when `repark._native` imports. Dual-wired ci.yml python job; wheels.yml smoke runs `--require-execute`. Seeded 2026-08-31 at **679** names, **658** backlog, **2** exceptions. In `make ci`. |
