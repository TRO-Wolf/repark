...
| `make check-example-coverage` | v0.7 public-surface example drift gate. AST-enumerates `F.*` / DataFrame / TA / reader-writer / session doors, compares `docs/examples` `COVERS` to the backlog ratchet, executes examples when `repark._native` imports. Seeded 2026-08-31 at **679** names, **658** backlog. In `make ci`. |
