...
| `python-conventions: FAIL` | A `def` inside a `def`, or a `dataclasses`/`attrs` import. Lift the function to module or class level, or add an inline `# nested-def: <reason>` pragma if it is one of the three sanctioned cases; use a Pydantic `BaseModel` instead of a `dataclass`. The EXCEPTIONS tables in `scripts/check_python_conventions.py` ratchet **down** only. Reasoning: [.agent/skills/code-quality/SKILL.md](.agent/skills/code-quality/SKILL.md). |
