...
| `make install-hooks` | Wire the pre-commit hook (map.md lockstep warning + map.md link validity + crate dependency policy + thinness guards + rust file-size + docstring presence + structural manifest + fmt/taplo/typos). Do this once per clone. |
| `make check-map-md` | The map.md **lockstep** guard over the pull-request diff (`BASE ?= origin/main`): changed code requires the directory's `map.md` in the same branch diff. This is the CI-held form; the pre-commit hook only warns. |
| `make check-map-sync` | The map.md **content** guard: every relative link in every tracked `map.md` resolves, and no two list rows in one map share a first link (`duplicate row` — the shape a `merge=union` resolution leaves behind). Measured n=5 median **0.08 s** over 143 maps, so it is on pre-commit. The coverage rule (every mappable file mentioned by its directory's map) is behind `--strict` and **not** armed — 24 pre-existing findings at 2026-08-22, a floor rather than an exact debt; run `python3 scripts/sync_map_md.py --check --strict` by hand. |
...
| `make check-example-coverage` | 1.1 (was v0.7) public-surface example drift gate. AST-enumerates ten doors — `F.*`, DataFrame, TA, reader-writer, session, Column, Window/WindowSpec, Catalog, `types` + Row, `ml` — compares `docs/examples` `COVERS` (must be used in the script) to the backlog and exceptions ratchets, executes examples when `repark._native` imports. Dual-wired ci.yml python job; wheels.yml smoke runs `--require-execute`. Seeded 2026-08-31 at 763 names / 742 backlog; widened by EX-1 the same day to **913** names, **892** backlog, **2** exceptions. In `make ci`. |
| `make check-docs-links` | The **markdown link gate** over every tracked `*.md`: each inline relative link (`[t](p)`, `[t](p#anchor)`) resolves to a tracked file relative to the linking file (a directory by anything tracked beneath it), an anchor matches the target's GitHub-style heading slug (lower-case, spaces to `-`, punctuation dropped, duplicates suffixed `-1`), and `docs:` evidence cells under `task/ledgers/**` follow the same rule with repo-root-relative paths. `http(s)` links, bare `#fragments` and anchors into non-markdown targets are out of scope. One line per broken link `path:line: <link> -> <reason>`, exit 1; the counts of files and links checked print on exit 0. Whole-tree run measured **0.82 s** (692 files, 4478 links, 2026-09-09). The measured baseline of pre-existing broken links lives in `scripts/docs_links_allowlist.txt`, one `path:link` entry per line — the line number is not part of the key, so an unrelated edit that shifts lines never reds the gate, and two identical broken links in one file collapse to one entry (D-9, 2026-09-09) — ratchet down only: an entry that matched no finding is itself a gate failure, so a fixed link takes its row away (D-8, 2026-09-09). In `make ci` beside `check-docs-compaction`, so it runs in `make preflight` through `verify` (DOCS-LINKS-1), and dual-wired into `ci.yml`'s guards job beside `check_docs_compaction` (RF-8, 2026-09-10). |
...
| Pre-commit hook warns on map.md lockstep | The touched directory's `map.md` must ride in the same pull request — ci.yml's `map.md guard` (`make check-map-md`) holds it; the staged warning never blocks. `make install-hooks` if the hook is not wired. |
