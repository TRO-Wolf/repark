...
|---|---|
| `make ci` | **The canonical fast gate.** fmt-check + clippy + panic/async bans + crate-DAG + lib.rs/lib.py thinness + `cargo check` + ruff lint/format + uv-lock check + toml + spell. Mirrors the CI `ci.yml` job. |
...
| `make install-hooks` | Wire the pre-commit hook (map.md lockstep + crate-DAG + thinness guards + fmt/taplo/typos). Do this once per clone. |
...
- `ci.yml` — the `make ci` chain (fmt, clippy, panic/async bans, crate-DAG, thinness,
  `cargo check`, ruff, uv-lock, toml, spell) plus the workspace tests and the Python parity step.
