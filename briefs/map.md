...
- [next-sequence.md](next-sequence.md) — **the next-sequence slate (2026-08-21, rolling):** one
  ...
  just the order — V3-1 merged as #203 and left; PYC-1 is in flight; MW-4 still preempts
  everything the moment OD-3 lands; A13 sits last while its guard holds. Carries the PYC unit
  definitions and the two hazards a pure-refactor campaign has to name in advance. Unlike the
  campaign slates below, it is rolling: a unit leaves when it merges.
